...
class GhrecordController extends CControl{
	...
	public function getmyclasscreditlist() {
		...
		if(!empty($this->myclass)){
			...
			//获取该班级10个学生。因为有可能取到自己，而且可能部分学生无积分，所有夺取5条
			$randcount = 10;
			if($students){
				$othercreditlist = array();
				$students_count = count($students);
				if($students_count>$randcount){
					$keys = array_rand($students, $randcount);
					foreach ($keys as $key)
					{
						$uids[] = $students[$key]['uid'];
						$othercreditlist[$students[$key]['uid']] = $students[$key];
					}
				}elseif ($students_count>1){
					foreach ($students as $stu)
					{
						$uids[] = $stu['uid'];
						$othercreditlist[$stu['uid']] = $stu;
					}
				}

				if (!empty($uids)) {
					$othercreditarr = $credit->getCreditByUids(array('uids'=>$uids));
					foreach($othercreditarr as $othercredit) {
						if(isset($othercreditlist[$othercredit['toid']])) {
							$othercredit['uid'] = $othercreditlist[$othercredit['toid']]['uid'];
							$othercredit['username'] = $othercreditlist[$othercredit['toid']]['username'];
							$othercredit['realname'] = $othercreditlist[$othercredit['toid']]['realname'];
							$othercredit['face'] = $othercreditlist[$othercredit['toid']]['face'];
							$othercredit['sex'] = $othercreditlist[$othercredit['toid']]['sex'];
							$othercreditlist[$othercredit['toid']] = $othercredit;
						}
					}
					$showcount = 5;	//实际显示数量，如果超出则unset
					$truecount = 0;
					foreach($othercreditlist as $otherkey=>$othervalue) {
						if(empty($othervalue['credit'])) {
							unset($othercreditlist[$otherkey]);
						} else {
							$truecount ++;
						}
						if($truecount > $showcount) {
							unset($othercreditlist[$otherkey]);
						}
					}
				}
