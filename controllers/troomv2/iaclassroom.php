...
class IaclassroomController extends CControl{
	...
	public function index(){
		$this->display('troomv2/iaclassroom_nav');
	}
	public function view(){
        ...
        if(empty($classes)){
            $this->assign('isclasses',1);
        }else{
            $this->assign('isclasses',2);
        }
		//组织参数
        ...
        foreach($iaList as &$v){
            $par['icid'] = $v['icid'];
            $classes = $iaclassroomModel->getIaClass($par);
            $classinfomation = '';
            $classinfo = array();
            if(!empty($classes)){
                foreach($classes as $val){
                    $classinfo[]=$val['classname'];
                }
                $classinfomation = implode(',',$classinfo);
            }
            if(!empty($classinfomation)){
                $v['classname'] = $classinfomation;
            }
        }
		$iaListCount = $iaclassroomModel->getListCount($param);
		$pagestr = show_page($iaListCount,$param['pagesize']);
        $classname = $this->model('appmodule')->getClassnameByCode(array('modulecode'=>'iaclassroom'));
		...
		$mnlib->getmodulename($this,array('modulecode'=>'iaclassroom','tors'=>1,'crid'=>$roominfo['crid']));
		$this->display('troomv2/iaclassroom');
	}
	/**
	 *添加一条记录
	 */
	public function add(){
		...
		if(empty($rec)){
			...
			$this->display('troomv2/iaclassroom_add');
		}else{
			$result = array();
	...
	public function edit(){
		...
		$this->assign('Upcontrol',$Upcontrol);
        $this->assign('ia',$ia);
		$this->display('troomv2/iaclassroom_edit');
	}
	...
	public function detail(){
		...
		$this->display('troomv2/iaclassroom_detail');
	}
	/**
	 *删除一条记录
	 */
