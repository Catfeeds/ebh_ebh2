<?php
	...
	class MyonlineController extends CControl{
		...
		public function index(){
			...
			$this->assign('key',$key);
			$this->display('troomv2/myonline_my');
		}
		// /**
		//  *所有直播
		//  */
  ...
  //      		$pagestr = show_page($count,10);
  //      		$this->assign('pagestr',$pagestr);
		// 	$this->display('troomv2/myonline_all');
		// }
		/**
		 *添加直播
		 */
		public function add(){
			...
			if(empty($rec)){
				...
				$this->display('troomv2/myonline_add');
			}else{
		...
		public function edit(){
			...
			if(empty($rec)){
				...
				if($op == 'view'){
					...
					$this->assign('folderlist',$folderlist);
					$this->display('troomv2/myonline_view');
				}else{
					$folderlist = $this->model('folder')->getTeacherFolderList1(array('uid'=>$user['uid'],'crid'=>$roominfo['crid'],'limit'=>100,'power'=>'0,1'));
					$this->assign('folderlist',$folderlist);
					$this->display('troomv2/myonline_edit');
				}
			}else{
				parse_str($rec,$querystr);
