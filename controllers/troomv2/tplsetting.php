...
class TplsettingController extends CControl {
	...
	*/
	public function index() {
		...
		$this->display('troomv2/tplsetting_announcement');
	}
	/**
	*删除公告
	*/
	...
	*/
	public function announcement_add() {
		...
		if(NULL !== $message) {	//处理表单
			...
			$this->display('troomv2/tplsetting_announcement_add');
		}
	}
	/**
	*修改公告
	*/
		...
		if(NULL !== $infoid) {	//处理表单提交
			...
			if(is_numeric($infoid) && $infoid > 0) {
				...
				if(!empty($send)) {
					...
					$this->display('troomv2/tplsetting_announcement_edit');
				}
			}
		}
	}
	...
	public function teacherteam() {
		...
		$this->display('troomv2/tplsetting_teacherteam');
	}
	public function teacherteam_add() {
		if($this->input->post()){
			...
			$this->display('troomv2/tplsetting_teacherteam_add');
		}
	...
	public function teacherteam_edit() {
		if($this->input->get('itemid')){
			...
			$Upcontrol = Ebh::app()->lib('UpcontrolLib');
			$this->assign('Upcontrol',$Upcontrol);
			$this->display('troomv2/tplsetting_teacherteam_edit');
	...
	public function information(){
		...
		$this->assign('tag',$tag);
		$this->display('troomv2/tplsetting_information');
	}
	public function information_add(){
		...
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('troomv2/tplsetting_information_add');
	}
	...
	public function information_edit(){
		...
		if($this->input->get('itemid')){
			...
			if($information['crid']==$roominfo['crid']){
				...
				echo '<script>location.href="'.geturl('troomv2/tplsetting/information').'"</script>';
				exit;
			}
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('troomv2/tplsetting_information_edit');
		exit;
		}
		if($this->input->post()){
			$rec = safeHtml($this->input->post(),array('message'));
	...
	public function advertisement(){
		...
		$this->assign('tag',$tag);
		$this->display('troomv2/tplsetting_advertisement');
	}
	public function advertisement_add(){
		...
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('troomv2/tplsetting_advertisement_add');
	}
	public function advertisement_edit(){
		...
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->display('troomv2/tplsetting_advertisement_edit');
		exit;
		}
		if($this->input->post()){
			$rec = safeHtml($this->input->post(),array('message'));
	...
	public function contact(){
		...
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->display('troomv2/tplsetting_contact');
	...
	private function check_information($param){
		...
		if($message['code']===false){
			$this->goback(implode('\r\n',$message),geturl('troomv2/tplsetting/information'));
		}
	}
	...
	private function check_advertisement($param){
		...
		if($message['code']===false){
			$this->goback(implode('\r\n',$message),geturl('troomv2/tplsetting/advertisement'));
		}
	}
	private function check_teacherteam($param){
		...
		if($message['code']===false){
			$this->goback(implode('\r\n',$message),geturl('troomv2/tplsetting/teacherteam'));
		}
	}
