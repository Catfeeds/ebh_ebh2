...
class SettingController extends CControl {
        ...
        if($this->input->post() !== NULL) {
            ...
            if(!empty($param)) {
                ...
                if($result !== FALSE) {
                    echo 'success';
					if($message !== NULL){
						$classroommess = $roommodel->getdetailclassroom($roominfo['crid']);
						$classroomchachekey = $this->cache->getcachekey('classroom',$roominfo['crid']);
    ...
    public function upmessage() {
        ...
        $this->display('troomv2/upmessage');
    }
	/*
	普通上传头像
	*/
	public function avatarold(){
		...
		if($op == 'teacher'){
			...
			$this->display('troomv2/customlogo');
		}
