<?php
	...
	class OnlineController extends CControl{
		...
		public function allcourse(){
			...
			$this->display('troomv2/online_all');
		}
		/**
		 *我的直播
		 */
		public function index(){
			$this->display('troomv2/onlinechoose');
		}
		public function my(){
			$roominfo = Ebh::app()->room->getcurroom();
	        $user = Ebh::app()->user->getloginuser();
			...
			$pagestr = show_page($count,10);
       		...
       		$this->assign('key',$key);
			$this->assign('roominfo',$roominfo);
			$this->display('troomv2/online_my');
		}
		/**
		 *添加直播
		 */
		public function add(){
			...
			if(empty($rec)){
				...
				$this->assignClassInfo();
				$this->display('troomv2/online_add');
			}else{
		...
		public function edit(){
			...
			if(empty($rec)){
				...
				$op = $this->input->get('op');
				if($op == 'view'){
					$this->display('troomv2/online_view');
				}else{
					$this->display('troomv2/online_edit');
				}
			}else{
				parse_str($rec,$querystr);
