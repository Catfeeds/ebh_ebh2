...
class FullcourseController extends CControl {
    ...
    public function __construct() {
		...
		if($moduleid != $this->helpcrid) {
			...
			$moduleid = $this->uri->uri_attr(0);
			$check = $rpmodel->checkmodule($roominfo['crid'],$moduleid);
			if(!$check) {
				exit();
			}
		}
    ...
    public function index() {
        ...
        if(is_numeric($moduleid) && $moduleid > 0) {
                ...
                $this->display('troomv2/fullcourse');
        }
    }
    /**
     * 全科复习课程
     * 由于list与PHP系统方法冲突，方法名改成list
     */
    public function lists() {
        ...
        if(is_numeric($moduleid) && $moduleid > 0) {
			...
			$q = $this->input->get('q');
				...
				$this->assign('q',$q);
				$this->assign('pagestr',$pagestr);
				$this->display('troomv2/fullcourse_list');
		}
    }
	/**
	*全科复习课件详情页
	*/
	public function view() {
		...
		if($type == 'flv' || empty($course['cwurl'])){
			...
			$this->display('troomv2/fullcourse_view');
		}
		//$this->display('troomv2/fullcourse_view');
	}
}
