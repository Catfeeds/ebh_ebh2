...
class StudentController extends CControl{
	...
	public function edit(){
		...
		/**写日志结束**/
		//var_dump($param);
		header('location:'.geturl('aroom/student'));
	...
	public function edit_view(){
		...
		$this->assign('room',$roominfo);
		$this->assign('classlist',$classlist);
		$this->display('aroom/student_edit');
	}
	...
	public function add(){
		...
		if($this->input->post()){
			...
			$classid = $this->uri->uri_attr(0);
			$this->assign('classid',$classid);
			$this->display('aroom/student_add');
		}
	}
	/*
	修改密码
	*/
	...
	public function input(){
		...
		$this->display('aroom/student_input');
	}
	/*
	 * 获取存储附件路径
	 */
	...
	*/
	public function input_scb(){
		...
		$this->display('aroom/student_input_scb');
	}
	...
	//	姓名	登录账号	性别	学校
		...
		for ($i = $titlerownum + 1; $i <= $reader->sheets[0]['numRows']; $i++) {
			...
			if(!preg_match('/^[a-z0-9A-Z_]{6,20}$/',$username)) {
				$result['errormsg'] = '第 '.$i.' 行登录账号格式不正确，登录账号只能为6-20位英文、数字、“_”的组合字符，请调整。';
				break;
			}
			if(trim($sex) == '女')
				$sex = 1;
	...
	*/
	public function input_upgrade(){
		...
		$this->display('aroom/student_input_upgrade');
	}
