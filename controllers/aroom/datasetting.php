<?php
/**
 * 教师网校模版设置控制器 datasettingController
 */
class DatasettingController extends CControl {
	...
	*/
	public function index() {
		...
		$this->display('aroom/datasetting_announcement');
	}
	/**
	*删除公告
	*/
	...
	*/
	public function announcement_add() {
		...
		if(NULL !== $message) {	//处理表单
			...
			$this->display('aroom/datasetting_announcement_add');
		}
	}
	/**
	*修改公告
	*/
		...
		if(NULL !== $infoid) {	//处理表单提交
			...
			if(is_numeric($infoid) && $infoid > 0) {
				...
				if(!empty($send)) {
					...
					$this->display('aroom/datasetting_announcement_edit');
				}
			}
		}
	}
	...
	public function teacherteam() {
		...
		$this->display('aroom/datasetting_teacherteam');
	}
	public function teacherteam_add() {
		if($this->input->post()){
			...
			$this->display('aroom/datasetting_teacherteam_add');
		}
	...
	public function teacherteam_edit() {
		if($this->input->get('itemid')){
			...
			$Upcontrol = Ebh::app()->lib('UpcontrolLib');
			$this->assign('Upcontrol',$Upcontrol);
			$this->display('aroom/datasetting_teacherteam_edit');
	...
	public function information(){
		...
		$this->assign('tag',$tag);
		$this->display('aroom/datasetting_information');
	}
	public function information_add(){
		...
		if($this->input->post()){
			...
			if($this->model('item')->_insert($param)==false){
				...
				echo 'success';
				updateRoomCache($param['crid'],'item');
				exit;
			}
		}
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('aroom/datasetting_information_add');
	}
	...
	public function information_edit(){
		...
		if($this->input->get('itemid')){
			...
			if($information['crid']==$roominfo['crid']){
				...
				echo '<script>location.href="'.geturl('aroom/datasetting/information').'"</script>';
				exit;
			}
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('aroom/datasetting_information_edit');
		exit;
		}
		if($this->input->post()){
			$rec = safeHtml($this->input->post(),array('message'));
	...
	public function advertisement(){
		...
		$this->assign('tag',$tag);
		$this->display('aroom/datasetting_advertisement');
	}
	public function advertisement_add(){
		...
		$this->assign('editor',$editor);
		$this->assign('tag',$tag);
		$this->display('aroom/datasetting_advertisement_add');
	}
	public function advertisement_edit(){
		...
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->display('aroom/datasetting_advertisement_edit');
		exit;
		}
		if($this->input->post()){
			$rec = safeHtml($this->input->post(),array('message'));
	...
	public function contact(){
		...
		$editor = Ebh::app()->lib('UMEditor');
		$this->assign('editor',$editor);
		$this->display('aroom/datasetting_contact');
	...
	private function check_information($param){
		...
		if(mb_strlen($param['note'],'UTF-8')>100||mb_strlen($param['note'],'UTF-8')<5){
			$message['code'] = false;
			$message[] = '资讯摘要长度不对!';
		}
	...
	private function check_teacherteam($param){
		...
		if($message['code']===false){
			$this->goback(implode('\r\n',$message),geturl('aroom/datasetting/teacherteam'));
		}
	}
