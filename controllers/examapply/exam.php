...
class ExamController extends CControl{
	...
	public function examlist() {
		...
		foreach($exams as $key => $exam){
			$folderids .= $exam['folderid'].',';
			if(empty($exam['itemid'])){
				$exams[$key]['isfinish'] = $this->_getfinishstatus($exam['folderid']);
			}
		}
		//获取课程封面
		...
		$folderimg = array();
		if (!empty($folderids)){
			$folders = $this->model('folder')->getfolderlist(array('folderids'=>$folderids,'limit'=>$queryarr['pagesize']));
			foreach ($folders as $folder){
				$folderimg[$folder['folderid']] = empty($folder['img'])?'http://static.ebanhui.com/ebh/images/nopic.jpg':$folder['img'];
			}
		}
		...
		$this->assign('folderimg',$folderimg);
		$this->assign('roominfo',$roominfo);
		$this->assign('pagestr',$pagestr);
        $this->display('examapply/examlist');
	...
	public function examresult() {
		...
		$this->assign('roominfo',$roominfo);
		$this->assign('pagestr',$pagestr);
        $this->display('examapply/examresult');
	}
	/**
	*更新新作业用户状态时间
	*/
	...
	/**
	 * 获取课程学习进度
	 * @param  integer $folderid 课程编号
	 * @return boolean           是否完成课程
	 */
	private function _getfinishstatus($folderid){
		$isfinish = TRUE;
		$user = Ebh::app()->user->getloginuser();
		$param['uid'] = $user['uid'];
		$param['folderid'] = $folderid;
		$cwlist = $this->model('progress')->getCWProgressByFolderid($param);
		if (!empty($cwlist) && is_array($cwlist)){
			foreach ($cwlist as $cw){
				if(!empty($cw['cwid'])){
					if(empty($cw['ctime']) || ($cw['ltime']/$cw['ctime']) < 0.9){
						$isfinish = FALSE;
						break;
					}
				}
