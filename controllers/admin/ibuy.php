...
class IbuyController extends CControl {
	...
	*生成订单信息
	*@param $payfrom 来源
	*@param $detail array 订单信息
	*@param $isorder bool 如果为TRUE则表示$detail参数为订单数据，否则未订单详情
	*/
	private function buildOrder($detail,$isorder = FALSE) {
		// $user = Ebh::app()->user->getloginuser();
		// if(empty($user))
			// return FALSE;
	...
	*/
	...
	/**
	 * 手动生成订单
	 * @param $orderdetail array 自己构造的订单数据
	 * @param $isorder bool $orderdetail数据是否为订单数据，而非订单详情数据，如果传过来的是多个订单详情，则需要设置为TRUE
	 */
	public function manualnotify($orderdetail = array(),$isorder = FALSE){
		...
		//生成订单
		$orderparam = $this->buildOrder($orderdetail,$isorder);
		if(empty($orderparam)){
			if(empty($tag)){
				echo 0;exit;
			}else{
		...
		//处理订单
		$res = $this->notifyOrder($orderparam);

		/**********日志逻辑开始**********/
		if(!empty($orderdetail)){
			$message = "";
			if($isorder) {
				$message .= '[uid:'.$orderdetail['uid'].']';
				foreach($orderdetail['itemidlist'] as $itemid) {
					$message .= '[itemid:'.$itemid.']';
				}
			} else {
				foreach ($orderdetail as $rkey => $rvalue) {
					$message .= '['.$rkey.':'.$rvalue.']';
				}
			}
			/**写日志开始**/
	...
	public function refund(){
		...
		//判断是否冻结，冻结不给退款
		$paytime = $oldpayorder['paytime'];
		$parameters['crid'] = $oldpayorder['crid'];
		$frozeDay = Ebh::app()->getApiServer('ebh')->reSetting()->setService('Settleset.FreeznTime.getFreeznTime')->addParams($parameters)->request();
		if (!empty($frozeDay)) {
			$today = strtotime('today')+86400;
			if (($today - $frozeDay*86400) >= $paytime) {
				$info['msg'] = '该订单已经冻结';
				$info['status'] = '-5';
				echo json_encode($info);
				exit;
		...
		$payorder['sharefee'] = -$payorder['sharefee'];
		$payorder['remark'] = '退款'.$money.'元';
		$payorder['status'] = 0;
		$payorder['dateline'] = SYSTIME;
		...
		foreach ($payorder['detaillist'] as $detail) {
			...
			$detail['sharefee'] = -$detail['sharefee'];
			$detail['osummary'] = '退款';
		...
		if($orderid){
			...
			if($payorder['payfrom']==8){
				if($real == 1){
					$usermodel = $this->model('user');
					$user = $usermodel->getuserbyuid($oldpayorder['uid']);
					if(!empty($user)){
						$balance['balance'] = floatval($user['balance']) + $money;
						$result = $usermodel->update($balance,$oldpayorder['uid']);
					}
				}
					if(empty($result)){
					$info['msg'] = '余额退款失败';
			...
			if($res){
				...
				$info['status'] = '1';
				//分销退款情况
				if (!empty($oldpayorder['isshare']) && !empty($oldpayorder['sharefee']) && !empty($oldpayorder['shareuid']) ) {
					$shareuser = $usermodel->getuserbyuid($oldpayorder['shareuid']);
					if(!empty($shareuser)){
						$balance['freezebalance'] = floatval($shareuser['freezebalance']) - $oldpayorder['sharefee'];
						if ($balance['freezebalance'] < 0 ) {
							log_message('分享退款失败，uid为'.$shareuser['uid'].' 冻结余额不足，分享的钱扣款失败');
						} else {
							if (empty($user))
								$user = $usermodel->getuserbyuid($oldpayorder['uid']);
							$payorder['sharedetail'] = empty($user['realname'])?$user['username']:$user['realname'].' '.$payorder['ordername'].'  价格: <em>'.$payorder['totalfee'].'</em>';
							$payorder['isfreeze'] = 0;//退款的不是冻结的
							$result = $this->model('Share')->addCharge($payorder);
							if (empty($result)) {
								log_message('分享退款失败，uid为'.$shareuser['uid'].'分享的钱扣款失败');
							} else  {
								log_message('分享退款成功，uid为'.$shareuser['uid'].'分享的钱扣款成功');
							}
						}
	...
	*导入开通处理
	*/
	public function input($order) {
		$orderid = $this->manualnotify($order,TRUE);
		return $orderid;
	}
}
?>
