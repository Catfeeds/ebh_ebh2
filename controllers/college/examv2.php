...
class Examv2Controller extends ApiControl {
	private $user = NULL;
	private $room = NULL;
	const SOURCE   = '/data0/uploads/examcourse';//examcourse/';源文件路径
//	const SOURCE   = '/opt/nginx/html/upload/data0/uploads/img/examcourse';//examcourse/';

	const DEST     = '/data0/htdocs/img/schimages';//目标路径
	private $k = "";
	...
	public function elistAjax() {
		...
		if(empty($folderid) && $action != 'hasdo'){//待做，免费，全校免费，开通未过期
			...
			$cwids = $this->getcwids();
			if (empty($folderids) && empty($cwids))
	...
	public function doneexam_view() {
		...
		$this->assign('eid',$eid);
		$this->assign('user',$this->user);
		$this->display('college/exam_done');
	}
...
// var_dump($userAnswer);exit;
	...
	public function myerrorbook() {
		...
		// }
		// $this->assign('q',$q);
		$this->display('college/exam_errorlist');
	}
	...
	public function efenxi_view(){
		...
		$this->assign('eid',$eid);
		$this->assign('user',$this->user);
		$this->display('college/analysis_exam');
	}
	...
	*/
	public function errsituation_view(){
		...
		$this->display('college/errorSituation');
	}
