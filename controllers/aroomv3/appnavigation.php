 ...
 * Time: 10:10
 */
class AppnavigationController extends ARoomV3Controller {
    public function add(){
        ...
        $result =  $this->apiServer->reSetting()->setService('App.Navigation.add')->addParams($parameters)->request();
        if($result['status'] == 0){
    ...
    public function edit(){
        ...
        $result =  $this->apiServer->reSetting()->setService('App.Navigation.edit')->addParams($parameters)->request();
        if($result['status'] == 0){
            $this->renderjson(1,$result['msg']);
        }
    ...
    public function lists(){
        ...
        $result = $this->apiServer->reSetting()->setService('App.Navigation.list')->addParams($parameters)->request();
    ...
    public function del(){
        $id = intval($this->input->post('id'));
        if($id <= 0){
            $this->renderjson(1, '请选择要删除的导航');
        }
        $parameters['crid'] = $this->roominfo['crid'];
        $parameters['id'] = $id;

        $result =  $this->apiServer->reSetting()->setService('App.Navigation.del')->addParams($parameters)->request();
        if($result['status'] == 0){
            $this->renderjson(1,$result['msg']);
        }
