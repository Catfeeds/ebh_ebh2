...
class WeixinController extends CControl{
	...
	{
		$this->display('troom/weixin/index');
	}
	/*
	*班级发信
 ...
 //        $classlist = $this->model('Classes')->getTeacherClassList($roominfo['crid'],$user['uid']);
	...
	// 	$this->display('troom/weixin/class_send');
	// }
	public function student_send_msg(){
		...
		$isenterprise = Ebh::app()->room->getroomtype() == 'com';
		if($isenterprise){//企业版
			...
			if(!empty($detail)){
				...
				$classlist = $this->apiServer->reSetting()->setService('Aroomv3.Enterprise.teacherDepts')->addParams($tdparam)->request();
				foreach($classlist as $department){//该部门 选择了该老师或者是所选部门的子部门
		...
		if(count($classlist) > 0 && !$verify && !$isenterprise) {
			$classid = $classlist[0]['classid'];
		}
		if($classid>0){
	...
	*/
	public function class_send_msg()
	{
		...
		$this->display('troom/weixin/some_classes');
	}
	//家长回复
	public function parent_send()
	{
		...
		$this->assign('classlist', $classlist);
		$this->assign('studentlist', $studentlist);
		$this->display('troom/weixin/parent_send');
	}
	//历史信息
 ...
 //        $classlist = $this->model('Weixin')->getTeacherClassList($roominfo['crid'],$user['uid']);
	...
	// 	$this->assign('classlist',$classlist);
	// 	$this->display('troom/weixin/list');
	// }
	/**
	*根据条件获取发信记录时间列表
	*/
	...
	*/
	...
	public function showDetail(){
        ...
        $this->display('troom/weixin/wxdetail');
	}
}
?>
