...
<div class="lefrig" style="border:solid 1px #cdcdcd;background:#fff;float:left;margin-top:15px;width:786px;">
	...
	<?php foreach ($courseList as $value) {?>
		<tr style="border-bottom:1px solid #d9d9d9;" onmouseout="this.bgColor='#ffffff'"onmouseover="this.bgColor='#f2f7ff'">
		<?php 
		//var_dump($avalue);
			if(!empty($value['face']))
		   ...
		   <?php if($user['uid']==$value['tid']){?>
		   	<a href="javascript:" onclick="return degroup('<?=$value["id"]?>','<?=$value["title"]?>')" style="margin-left:0;margin-right:5px;" class="btnshan">删除</a>
		   		<?php if(time()<=($value['cdate'])+$value['ctime']*60){?>
					...
					<?php $ourl = "http://chat.ebh.net/webconf/room.htm?id=".$value['id']."&user=".$uname."&teach=".$uname."&type=t&key=".$key;?>	
