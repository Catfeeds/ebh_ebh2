...
<script type="text/javascript" src="http://static.ebanhui.com/ebh/js/qqFace/js/jquery.qqFace.js?v=201706280001"></script>
...
.lefrig input.lanbtn{backgroun
	d:url("http://static.ebanhui.com/ebh/tpl/2014/images/shoucangico.jpg") no-repeat left center;color: #626262;font-family: "微软雅黑";font-size: 14px;padding-left: 20px;width:65px;}
...
a.play-courseware.playscal:hover {
	...
	<script type="text/javascript">
		var timerpay;
    	//快捷支付的弹窗函数
    	function threefun(img,coursename,coursetxt,money,cwid){
			dialog({
				title:'试听结束，购买完整版',
				content:$("#clickpay")[0],
				onshow:function(){
					$('input[type=radio][name=pay3][value="wx"]').prop("checked","true");
					$(".courname").html(coursename);
					$(".court").html(coursetxt);
					$(".courimgbox img").attr('src',img);
					$(".paymon").html(money);
					...
					$("#settitlename").attr("title",coursename);
					$("#settitletxt").attr("title",coursetxt);
					$('input[type=radio][name=pay3]').change(function(){
						if(this.value === "wx"){
							$(".paywx").addClass("showpay");
							$(".payipay").removeClass("showpay");
							$(".paymw").removeClass("showpay");
		...
		<?php }else{ ?>
			...
			$(function (){
				...
				if(lastsuffix == 'flv'){
					...
					?>
					playmu('<?= $course['m3u8url'] ?>',cwid,'',isfree,num,'562','940',0,'<?= $course['thumb'] ?>',<?= $course['cwsize']?>,null,<?=$autoplay?>,0,-1,<?=$course['looktime']?>);
					<?php
		...
		//试听完成回调
		function lookTimeOverFunc(ctime,ltime,finished,plid){
			<?php if ($course['looktime']){?>
				var logined = <?=empty($logined) ? 'false' : 'true'?>;
		    	if (!logined) {
		    	    $('#nav-login').click();
		    	    return;
		    	}
		    	threefun('<?=$course['logo'];?>','<?=$course['title']?>','<?=!empty($course['summary']) ? str_replace("\r\n","",trim($course['summary'])) : ''?>','<?=$course['cprice']?>','<?=$course['cwid']?>');
			<?php } else { ?>
				var cwid = <?= $course['cwid'] ?>;
				var res = studyfinish(cwid,ctime,ltime,finished,plid);
				if(finished==1){
...
<![endif]-->
    <?php if (!empty($settings)) { ?>
        ...
        <script>
            ...
            var lguser  = <?=!empty($plateUser) ? json_encode($plateUser) : '{}';?>;
...
<?php  if (!empty($head)) { ?>
    <div class="plate-top"><div style="height:<?=$settings['top']?>;position:relative;width:<?=$settings['width']?>;margin:0 auto;"><?=$head?></div>
<?php } else if (!empty($top_components)) { ?>
    <div class="wraps">
        <?php foreach ($top_components as $mid => $top_component) {
            if ($mid == 1) {
                ...
                $this->partial('shop/plate/portfolio-navigation', true, $top_component);
   				 ...
   				 <div id="clickpay" style="display: none;">
			    	<div class="courseinfolog">
						<span class="courimgbox">
							<img src=""/>
						</span>
						<span class="courtext">
							<p class="courname" id="settitlename">课程</p>
							<p class="court" id="settitletxt">拼音：yi</p>
						</span>
					</div>
					...
					<div class="payment">
						<p class="chosepay" style="text-align: left;">选择支付方式</p>
						<div>
							<input type="radio" name="pay3" id="wx_pay" value="wx" checked /> <label for="wx_pay" style="margin-right: 50px;">微信</label>
							<input type="radio" name="pay3" id="ipay_pay" value="ipay" /> <label for="ipay_pay" style="margin-right: 50px;">支付宝</label>
							<input type="radio" name="pay3" id="myw_pay" value="my" /> <label for="myw_pay">我的钱包</label>
						</div>
...
<div style="width:1200px;margin:0 auto 0 auto;font-family:微软雅黑;">
	<div class="cright" style="display: block;margin: 0 auto;width:1200px;margin-bottom:20px; ">
		...
		<div class="lefrig" style="position:relative;background:#fff;min-height:1651px;">
			<!--免费试听新添加右侧模块-->
			<div class="rigvideo">
				<div class="rigvideo_list content_2 content" style="min-height:552px;margin-bottom: 0px;">
				...
				<div class="rigvideo_ter">
					...
					<div class="scole content_2 content" style="width:auto;border:none;height:740px;background-color:#fff;margin-bottom:0px;min-height:740px;">
						<ul>
			...
			<?php if($type == 'flv') { ?>
				<div style=" position: relative;height:562px;z-index:1;float:left;">
				<div id="flvcontrol" style="width:940px;height:562px;"></div>
			...
			<video id="advertising_video" width="100%" height="100%" autoplay="autoplay" src="<?=$intro['sourceurl']?>"></video>
				</div>
				<script type="text/javascript">
...
(function($) {
		...
		$(document).bind('ready', function() {
			...
			<a name="fujian" href="javascript:void(0);"></a>
				...
				<div class="introduce" style="width:940px;height:auto;">
					<div class="intitle" >
						<h2>附件下载</h2>
					</div>
								...
								<thead class="tabhead">
									...
									<?php } ?>
										...
										<td>
											...
											<?php } else { ?>
												<a class="previewBtn" href="javascript:void(0);" source="<?= (empty($source) ?$atta['source']:$source) ?>" title="<?= $atta['title']?>" suffix="<?= $atta['suffix'] ?>" cwid="<?= $course['cwid'] ?>" aid="<?= $atta['attid'] ?>" >播放</a>
											<?php } ?>
										</td>
										<?php } ?>
									  </tr>
		...
		<?php } ?>
			<!--改版又说这个弹窗不需要，所以注释-->
		<!--<div id="overst" style="display: none;">
			<span style="color: #333333;font-size: 18px;font-weight: bold;">购买完整版</span>
			<div class="courseinfolog">
				<span class="courimgbox">
					<img src="<?=$course['logo'];?>"/>
				</span>
				<span class="courtext">
					<p class="courname" title="<?=$course['title']?>"><?=$course['title']?></p>
					<p class="court" title="<?=$course['summary']?>"><?=$course['summary']?></p>
				</span>
			</div>
			<a href="javascript:void(0)" class="buyall">￥<span style="font-weight: bold;font-size: 14px;"><?=$course['cprice']?></span><span style="font-size: 12px;font-weight: normal;">&nbsp;<?php echo $course['cmonth']?$course['cmonth']:'';?>个月</span></a>
		</div>-->
...
<?php } ?>
    <?php if (!empty($foot)) { ?>
        <div style="height:<?=$settings['foot']?>;position:relative;width:<?=$settings['width']?>;margin:0 auto;"><?=$foot?></div></div>
...
function closeWindows() {
	...
	$(window).unload(function(){
		if(typeof theswf == 'undefined'){
			var theswf = document.getElementById('flvcontrol');
		}
		if(theswf.imunload){
			theswf.imunload();
		}
