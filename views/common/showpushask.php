...
<div class="ter_tit" style="width:778px;">
当前位置 > 查看关联问题</div>
<div class="lefrig">
...
<span class="kanwenti">
    ...
    <span class="fenge" style="margin-top:5px;">|</span><!-- Baidu Button BEGIN -->
...
<!--上传音频-->
	<div style="background:#fff;float:left;min-height: 53px;height:230px;">
    	...
    	<!-- <a href="javascript:parent.window.showDialog('tandaandiv');" class="xiugaibtn">解 答</a> -->
...
function addfavorit(qid,flag) {
	...
	if(flag == 1) {
        ...
        var url = '<?= geturl($whichRoom.'/myask/addfavorit') ?>';
	$.ajax({
		url:url,
		type:'post',
...
function addthank(qid) {
	var tips = "感谢";
        var url = '<?= geturl($whichRoom.'/myask/addthank') ?>';
	$.ajax({
		url:url,
		type:'post',
		data:{'qid':qid},
...
function addthankanswer(qid,aid) {
	var tips = "感谢";
        var url = '<?= geturl($whichRoom.'/myask/addthankanswer') ?>';
	$.ajax({
		url:url,
		type:'post',
...
$(function(){
    ...
    function submitanswer(qid,dom) {
        ...
        var url = '<?= geturl($whichRoom.'/myask/addanswer') ?>';
        $.ajax({
            url:url,
            type:'post',
...
function setbest(qid,aid) {
	var tips = "设置最佳答案";
	var url = '<?= geturl($whichRoom.'/myask/setbest') ?>';
	$.ajax({
		url:url,
		type:'post',
		data:{'qid':qid,'aid':aid},
...
function delask(qid,title) {
    var url = '<?= geturl($whichRoom.'/myask/delask') ?>';
    var successurl = '<?= geturl($whichRoom.'/myask/all') ?>';
...
function delanswer(qid,aid) {
    var url = '<?= geturl($whichRoom.'/myask/delanswer') ?>';
