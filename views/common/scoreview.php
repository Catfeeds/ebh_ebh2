...
<div class="detrig">
<h2 class="titxiang">
积分兑换 > 详情 > <a href="#" style="color:#ff4e00;"><?= $value['productname']?></a>
</h2>
<div class="xqtu">
  <img width="219px;" height="120px;" src="<?= getThumb($value['image'],'220_120','http://static.ebanhui.com/ebh/tpl/default/images/nopic.gif')?>"  />
</div>
  <div class="rigxq">
  <h1><?= $value['productname']?></h1>
  <p>原价：<span style="text-decoration: line-through;">￥<?= $value['marketprice']?></span></p>
  <p><span style="float:left;margin-top:12px;">积分：</span><span class="jfsize"><?= $value['credit']?></span>积分</p>
  <!--{block name="member" parameter="cachename/member/memberid/$uid/tpl/data"}-->
  
	  <?php if(empty($user)){ ?>
	  	  <a href="javascript:;" id="duihuan" class="yuebtn"></a>
	  <?php }else{ ?>
	  	 <?php if($value['stockqty']>0){ ?>
			  <?php if($user['credit'] < $value['credit']){ ?>
			  	<a href="javascript:;" onclick="alert('积分不足,兑换(<?= $value['productname']?>)需要<?= $value['credit']?>积分!');" class="yuebtn"></a>
			  <?php }else{ ?>
			  	<a href="javascript:;" onclick="exchange(<?= $value['productid']?>,<?= $value['type']?>);" class="yuebtn"></a>
			  <?php } ?>
		 <?php }else{ ?>
	  ...
	  <?php } ?>
  		...
  		<div id='score' style="display: none;">
				...
				<div class="tswin" id="virtual" style="display: none;width:310px;font-size:14px;">
					<p class="xinxi" style="width:350px;">您确定要兑换<?= $value['productname']?>？</p>
					<table width="375" border="0" cellpadding="0" style="color:#5e5e5e;">
					  <tr>
					    <td width="125" align="right" valign="top">产品名称：</td>
					    <td width="250" align="left"><label><?= $value['productname']?></label></td>
					  </tr>
					  <tr>
					    <td width="125" align="right">平　　台：</td>
					    <td width="150" align="left"><label><?= $room['crname']?></label></td>
					  </tr>
					  <tr>
					    <td width="125" align="right">原　　价：</td>
					    <td width="150" align="left"><label><?= $value['marketprice']?></label></td>
					  </tr>
					  <tr>
					    <td width="125" align="right">积　　分：</td>
					    <td width="150" align="left"><label><?= $value['credit']?></label></td>
					  </tr>
					</table>
				...
				<div id="message" style="display:none;">
					<div class="fenx" style="width:310px">
...
<script type="text/javascript">
  ...
  <div class="detming">
  <span style="margin-right:30px;">已经兑换：<span class="sizcolo"><?= $value['sellqty']?></span>件</span> 库存：<span class="sizcolo"><?= $value['stockqty']?></span>件
...
<!--  </li>-->
  ...
  <div class="xiaxiang">
 <?php if($value['type']==0){?>
<p>商品全称：<?= $value['productname']?></p>
<p>商品产地：杭州</p>
<p>商品毛重：<?= $value['weight']?></p>
<p>1.品牌：<?= $value['brand']?></p>
<p>类型：<?= $value['type']==0?'实物产品':'虚拟产品'?></p>
<p>型号：<?= $value['productno']?></p>
<p>2.规格：<?= $value['specification']?></p>
<p>3.特性：<?= $value['special']?></p>
<?php } ?>
<p><?= $value['message']?></p>
  </div>
  </div>
...
<ul>
<li class="detico4">
<p><a class="colo" href="http://www.ebanhui.com/help/view/1396.html">e板会积分兑换操作步骤介
绍</a></p>
</li>
<li class="detico5">
<p><a class="colo" href="http://www.ebanhui.com/help/view/1397.html">e板会积分实物礼品配送方
式及到货周期</a></p>
</li>
<li class="detico6">
<p><a class="colo" href="http://www.ebanhui.com/help/view/1398.html">e板会积分兑换实物后，可
以退换货么？</a></p>
</li>
</ul>
</div>
...
$(function(){
	$("#scoresubmit").click(function(){
		...
		$("#scode").html(postcode);
		$(".zilan").html('<?= $value['credit']?>');
		$(".zideng").html('<?= $value['marketprice']?>');
		$("#pname").html('<?= $value['productname']?>');
		$("#receiving").hide();
		$("#confirm").show();
	});
	$("#confirmexchange").click(function(){
		...
		$.ajax({
			url:"<?= geturl('member')?>",
			type:'post',
			data:{'pid':'<?= $value['productid']?>','productname':'<?= $value['productname']?>','name':name,'phone':phone,'address':address,'postcode':postcode,'type':'<?= $value['type']?>','op':'submitorders','inajax':1},
			dataType:'json',
			success:function(_json){
				if(_json.result!='success'){
					...
					$("#res").html('你兑换的礼品已保存在我的积分兑换明细中，<a href="<?= geturl('member/score/integral')?>" target="_blank" style="font-weight:bold; color:#000;">点击查看</a>');
					$("#message").show();
				}
			}
		});
	...
	$("#conversion").click(function(){
		$.ajax({
			url:"<?= geturl('member')?>",
			type:'post',
			data:{'pid':'<?= $value['productid']?>','type':'<?= $value['type']?>','productname':'<?= $value['productname']?>','days':'<?= $value['days']?>','op':'submitorders','inajax':1},
			dataType:'json',
			success:function(_json){
				if(_json.result!='success'){
					$("#virtual").hide();
					...
					$("#resmsg").html('你兑换的礼品已保存在我的积分兑换明细中，<a href="<?= geturl('member/score/integral')?>" target="_blank" style="font-weight:bold; color:#000;">点击查看</a>');
					$("#virtualmessage").show();
				}
			}
		});
