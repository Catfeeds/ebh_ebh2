...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('myroom/myask') ?>">互动答疑</a> > 推荐
<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<div class="workol">
	<div class="work_mes">
		<ul>
			...
			<li class="workcurrent"><a href="<?= geturl('myroom/myask/recommend') ?>"><span>推荐</span></a></li>
										...
										<td style="border-top:none;">
											<?php $name=empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>
												<div style="float:left;margin-right:15px;"><a href="http://sns.ebh.net/<?=$avalue['uid']?>/main.html" target="_blank"><img title="<?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>的个人空间" src="<?=$face?>" /></a></div>
													...
													<span class="huirenw" style="width:100px;float:left;"><a href="recommend.html?aq=<?= $name?>"><?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?></a></span>
													...
													<span class="" style="margin-left:294px"><a href="/myroom/myask/recommend.html?cwid=<?=$avalue['cwid']?>"><?=shortstr($avalue['cwname'],40)?></a></span>
													</div>
													<?php }?>
												</div>
...
$(function() {
   ...
   $("#ser").click(function(){
       ...
       if(title == searchtext) 
           title = "";
       var url = '<?= geturl('myroom/myask/recommend') ?>' + '?q='+title;
