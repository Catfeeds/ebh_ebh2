...
#errlist .que p.desc{font-size:12px;height:35px;line-height: 35px;color:#3a3a3a;display: inline-block;border:1px solid #f3f3f3;background: #f3f3f3;width: 747px;}
...
<script type="text/javascript">
	function searchs(strname){
		...
		var sdate = $("#sdate").val();
		var edate = $("#edate").val();
		url = '<?= geturl('myroom/serrorbook')?>?q='+sname+'&sdate='+sdate+'&edate='+edate;
		location.href = url;
	}
...
<div class="lefrig">
	<!-- 搜索功能 -->
	<div class="work_search" style="padding-top:20px;">
		<ul>
	    	
	            <li style="line-height:normal;"><label style="line-height:20px;">试题名称</label><input name="txtname" id="txtname" value="<?= $q ?>" style="float:left;" type="text" /></li>
	            <li style="display:inline-block;margin-left:15px;line-height:normal;">
	            	<label style="line-height:20px;">开始时间</label><input type="text" id="sdate" value="<?= $sdate ?>" name="sdate" onclick="WdatePicker()" />
	            </li>
	            <li style="display:inline-block;margin-left:15px;line-height:normal;">
	            	<label style="line-height:20px;">结束时间</label><input type="text" id="edate" value="<?= $edate ?>" name="edate"  onclick="WdatePicker()"/>
	            </li>
	            <script type="text/javascript">
					$("#edate").focus(function(){
						...
						WdatePicker({dateFmt:"yyyy-MM-dd",minDate:min});
					});
				</script>
	            <li style="line-height:normal;"><input type="button" onclick="searchs('txtname');return false;" class="souhuang" style="float:none;display:inline;*margin-left:0px;" value="搜 索"></li>
	     </ul>
	</div>
	...
	<div class="workol" id="errlist">
		...
		<?php if(!empty($errors)) { ?>
				...
				<?php if($error['quetype']=='A') { ?>
					<div class="que singleContainer singleContainerFocused" qsval="<?= $k ?>" id="que">
						...
						<?php if(!empty($error['ques']['resolve'])) { ?>
			            	<div class="title answerBar">答案解析：<div class="resolve inputBox" style="width:645px;float:right;"><?= $error['ques']['resolve'] ?></div><div class="clearing"></div></div>
						<?php } ?>
				...
				<?php } else if($error['quetype']=='C') { ?>
					<div class="que singleContainer singleContainerFocused" qsval="<?= $k ?>">
			            ...
			            <div class="blankSubject subjectPane">
				            ...
				            <?= str_replace('#input#', '<input type="text" readonly="readonly" maxlength="2147483647" size="20" value="">',trim($error['ques']['subject']))?> 
			...
			<?= $pagestr ?>
		<?php } else { ?>
				 <div style="clear:both;border-top:1px solid #ccc;padding-top:10px;">暂无记录</div>
		<?php } ?>
	</div>
</div>
<script type="text/javascript">
	$(".delerror").click(function(){
		...
		$.confirm("您确认要删除此错题吗？",function(){
			var url = "<?= geturl('myroom/serrorbook/del') ?>";
			$.ajax({
				type: "POST",
				url: url,
				data: {'eid':eid},
