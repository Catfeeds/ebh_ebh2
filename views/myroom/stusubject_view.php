...
<?php foreach($sectionlist as $section) { ?>
	...
	<?php $arr = explode('.',$course['cwurl']);
		...
		if($type != 'flv' && $course['ism3u8'] == 1)
			$type = 'flv';
	?>
	<a class="sa" target="<?= (empty($course['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('myroom/mycourse/'.$course['cwid'].'-0-0-0-'.$from)?>" <?= $color ?>><?= $course['title'] ?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a><em class="yema" <?= $color ?>><?= $i++ ?></em></li>
	<?php } ?>
