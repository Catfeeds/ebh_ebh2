<?php $this->display('myroom/page_header'); ?>
			<div class="ter_tit">
	当前位置 > <a href="<?= geturl('myroom/stusubjectcc') ?>" >高考冲刺</a> > 
			<?php if(!empty($uparr)){
				$upfoldercount = count($uparr);
				foreach($uparr as $k=>$upfolder){
				$index = $upfoldercount-$k-1;
			?>
			<a href="<?=geturl('myroom/stusubjectcc/'.$uparr[$index]['folderid'].'-0-0-0-'.$from)?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>	
			<?= $folder['foldername'] ?>
			 <div class="diles">
	<input name="uname" class="newsou" id="title" value="<?=$q?>" type="text" />
	<input id="room_search_btn" type="button" onclick="_search()" class="soulico" value="" name="courseware_search">
</div>
</div>
<div class="lefrig">
...
$(function() {
   initsearch("title",searchtext);
		$("#room_search_btn").click(function(){
			...
			var url = "<?= geturl('myroom/stusubjectcc/'.$folder['folderid'].'-0-0-0-'.$from)?>?q="+search;
			location.href=url;
		});
	});
</script>
...
<div class="kejian">
	...
	<div class="showimg"><a href="<?=geturl('myroom/stusubjectcc/'.$subfolder['folderid'].'-0-0-0-'.$from)?>" title="<?=$subfolder['foldername']?>"><img src="<?= empty($subfolder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subfolder['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('myroom/stusubjectcc/'.$subfolder['folderid'].'-0-0-0-'.$from) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
...
<?php foreach($sectionlist as $section) { ?>
	...
	<a class="sa" target="<?= (empty($course['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('myroom/stusubjectcc/course/'.$course['cwid'].'-0-0-0')?>" <?= $color ?>><?= $course['title'] ?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a><em class="yema" <?= $color ?>><?= $i++ ?></em></li>
	<?php } ?>
