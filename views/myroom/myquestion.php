...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('myroom/myask') ?>">互动答疑</a> > 我的问题
<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<div class="workol">
	<div class="work_mes">
		<ul>
			...
			<li class="workcurrent"><a href="<?= geturl('myroom/myask/myquestion') ?>"><span>我的问题</span></a></li>
...
initsearch("title",searchtext);
   $("#ser").click(function(){
       ...
       if(title == searchtext) 
           title = "";
       var url = '<?= geturl('myroom/myask/myquestion') ?>' + '?q='+title;
       document.location.href = url;
   });
   });
function delask(qid,title) {
    var url = '<?= geturl('myroom/myask/delask') ?>';
    var successurl = '<?= geturl('myroom/myask/myquestion') ?>';
