    ...
    <div class="buygoodson">
		<div class="work_menu delivery-title" style="position:relative;margin-top:0">
			...
			<div class="buygoodstype">
                <a href="/mall/order/cscore.html" class="first curr">积分商品</a>
                <a href="/mall/order/cgeneral.html" class="second">普通商品</a>
            </div>
		</div>
        <div class="buygoodsearch">
            ...
            <div class="goodsnav">
                <ul>
                        <li class="first <?php if($method=='cscore') echo 'curr'; ?>"><a href="<?= geturl('mall/order/cscore')?>">所有订单</a><span>|</span></li>
                        <li class="<?php if($method=='cswaitsend') echo 'curr'; ?>"><a href="<?= geturl('mall/order/cswaitsend')?>">待发货</a><span>|</span></li>
                        <li class="<?php if($method=='cssended') echo 'curr'; ?>"><a href="<?= geturl('mall/order/cssended') ?>">待收货</a><span>|</span></li>
                        <li class="<?php if($method=='cssucceed') echo 'curr'; ?>"><a href="<?= geturl('mall/order/cssucceed') ?>">交易成功</a></li>
                    </ul>
            </div>
            <div class="clear"></div>
            ...
            <?php foreach ($orders as $order) { ?>
                <table cellpadding="0" cellspacing="0" class="goodstable">
                    <tr>
                        ...
                        <th width="100">实付积分</th>
                        <th width="100">交易状态</th>
                    </tr>
                    <tr>
                        <td class="tdfirst" style="width:100px">
                            <a target="_blank" href="<?= getgoodsurl($order['details'][0]['crid'], $order['details'][0]['gid'])?>" style='color:black'><img src="<?= $order['details'][0]['img'] ?>" class="goodscover" width="80" height="80" /></a>
                        </td>
                        ...
                        <td rowspan="<?= count($order['details']) ?>"><a <?= empty($order['seller']['qq_href']) ? 'onclick="showDialogWithoutQQ()"' :'target="_blank"' ?> href="<?= empty($order['seller']['qq_href']) ? 'javascript:void(0)': $order['seller']['qq_href'] ?>" class="contactseller qqcontent-1 <?= empty($order['seller']['qq_href']) ? 'wsq': '' ?>">联系卖家</a></td>
                        <td rowspan="<?= count($order['details']) ?>"><?php echo $order['score']; ?><br /></td>
                        <td rowspan="<?= count($order['details']) ?>"><?= getOrderStatus($order['type']) ?><br />
                            <?php if($order['type'] == 2){ ?><a href="javascript:void(0)" class="confirmreceipt" data-oid="<?= $order['ordernum'] ?>" onclick="confirm(this)">确认收货</a> <?php } ?></td>
                    </tr>
                    <?php if(count($order['details']) >1 ){ ?>
                        <?php foreach($order['details'] as $goods){ ?>
                            <tr>
                                <td class="tdfirst" style="width:100px">
                                    <a target="_blank" href="<?= getgoodsurl($order['details'][0]['crid'], $goods['gid'])?>" style='color:black'><img src="<?= $goods['img'] ?>" class="goodscover" width="80" height="80" /></a>
                                </td>
                                ...
                                <td class="tdfirst" ><?php echo $goods['score']; ?></td>
                                <td style="border-bottom: 1px solid #ececec;"><?= $goods['quantity']?></td>
                            </tr>
                        <?php } ?>
                    <?php } ?> 
                </table>
...
<script type="text/javascript">
    ...
    function confirm(objCur){
        ...
        top.dialog({
            title: "信息提示",
            content: "<p style='font-weight:bold;font-size:16px'>请确认是否已收到货？</p>",
            okValue: "确认",
            ok: function() {
