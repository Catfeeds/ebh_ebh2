...
<?php $usertype = !empty($user['groupid'])?$user['groupid']:0;?>

<div class="rigtp">
<h3 class="titjl"><?= $room['crname']?></h3>
<div class="zongp">
<p style="margin: 10px 6px 0 40px;_margin-left:20px;" class="pingff">

<span class="barbgs">
<span class="votebars" style="width:<?= round($myroom['score'])?>0%;"></span>
</span>

</p>
<span style="font-size:24px;color:#ffb513;"><?= sprintf("%01.1f", $myroom['score'])?></span> 分(满分10分)
</div>
<p style="float:left;width:110px;margin-left:40px;_margin-left:20px;">已有<span><?= $myroom['viewnum']?></span>人评分</p><span style="float:left;">我的评分：</span>
<p class="pingff" style="width:59px;margin-top:3px;">

<span class="xiaobarbg">

<span class="xiaovotbg" style="width:<?= $myscore ?>0%;"></span>
</span>

</p>
<span style="float:left;margin-left:10px;"><?= $myscore ?>分</span>
</div>
</div>
</div>
<div class="dianpin">
...
<?php if(!empty($user)){ ?>
<input name="" type="button" value="发 布" onclick="submitsubject('subject','good','bad','useful',<?= $usertype?>);" style="*margin-left:-20px;" class="fabubtn" />
<?php }else{ ?>
<a class="fabubtn1" href="javascript:;" style="color:#fff;text-decoration:none;">发 布</a>
<span style="margin-top:20px;float:left;color:#999;">
需要登录才能发布评论,
<a style="color:#3095c6" href="javascript:tologin('/login.html?returnurl=__url__');">马上登录</a>
</span>
<?php } ?>
...
function submitsubject(subject,good,bad,useful,usertype){
	...
	if(subject==''){
		...
		$.ajax({
			url:'<?= geturl('cloudscore/review')?>',
			type:'post',
			data:{'subject':subject,'good':good,'bad':bad,'useful':useful},
			dataType:'json',
			success:function(data){
				$.showmessage({
					...
					title    :      '提交评论',
					callback :    function(){
						if(data != undefined && data.code != undefined && data.code == 'success') {
							location.reload();
						}
					}
				});
