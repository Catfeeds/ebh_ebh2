...
<?php if(empty($mitemlist)){?>
	...
	if($jx) {
		$newsurl = 'http://www.jxzhjy.com/dynews.htm?id='.$value['itemid'];
	}
	?>
	<li class="auli">
	<a href="<?= $newsurl?>" title="<?= $value['subject']?>" target="_blank"><h2 class="cfrgt"><?= $value['subject']?></h2></a>
	<p style="color:#a9b8d6;"><?= date('Y-m-d H:i:s',$value['dateline'])?></p>
	<p class="xiangjie"><?= shortstr($value['note'],350)?></p>
	<a href="<?= $newsurl?>" style="color:#0991bf;" target="_blank">阅读全文>></a>
