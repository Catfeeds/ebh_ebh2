...
if (!empty($setting)) { ?>
    <div class="schoolprofilemain">
        ...
        <?=shortstr($varpool['data']['summary'], 680)?>
    </div>
    <?php return;
} ?>

<!--网校简介-->
<div class="plate-module schoolprofile-3" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;height:<?=$varpool['height']?>px">
