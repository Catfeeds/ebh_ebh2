...
if (!empty($setting)) { ?>
    <div class="schoolprofilemain">
        ...
        <?=shortstr($varpool['data']['summary'], 760)?>
    </div>
    <?php return;
} ?>

<!--网校简介-->
<div class="plate-module schoolprofile-4" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;height:<?=$varpool['height']?>px">
    <div class="rankingtitle"><?=htmlspecialchars($varpool['ititle'], ENT_NOQUOTES)?></div>
    <div class="schoolprofilemain">
        <img src="<?=htmlspecialchars($classroom_logo, ENT_COMPAT)?>" style="width:220px;height:220px" class="fl imgleft" />
        <?=shortstr($varpool['data']['summary'], 798)?>
    </div>
</div>
