            ...
            <?php if (!empty($varpool['data'])) {
                foreach ($varpool['data'] as $item) { ?>
                    <li><a href="javascript:;" class="del-free-item" d="<?=$item['cwid']?>"></a>
                        ...
                        <a href="javascript:;"><div class="coursename" title="<?=htmlspecialchars($item['title'], ENT_COMPAT)?>"><?=htmlspecialchars(shortstr($item['title'], 12), ENT_NOQUOTES)?></div></a>
                    </li>
                <?php }
            } ?>
...
<div class="plate-module freeaudition-1" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;height:<?=$varpool['height']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;">
