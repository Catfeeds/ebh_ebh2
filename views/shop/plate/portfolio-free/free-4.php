...
$show_add_btn = $count == 0 || $count % 12 > 0;
$show_more_btn = $count > 0 && $count % 12 == 0;
$height = 237 + (max(ceil($count / 12.0), 1) - 1) * 320;
if (!empty($setting)) { ?>
    <div class="freeauditionlist" id="free-e" style="height:<?=$height?>px">
        ...
        <div class="exp-more"<?php if (!$show_more_btn){ echo ' style="display:none;"'; } ?>>继续添加>><input type="hidden" value="12" id="freesize" /></div>
    </div>
    <?php return;
} ?>
<div class="plate-module freeaudition-4" style="margin-right:auto;margin-left:auto;<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;height:<?=$varpool['height']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;">
    ...
    <div class="freeauditionlist" style="<?php if(empty($varpool['float'])) { ?>height:<?=$height?>px<?php } else { ?>float:inherit;<?php }?>">
        <ul>
            <?php if (!empty($varpool['data'])) {
                ...
                $findex = 0;
                foreach ($varpool['data'] as $item) { ?>
                    <li<?php if (($findex++ % 6) == 5) { ?> class="marig"<?php } ?>>
                        <div class="imagemin">
