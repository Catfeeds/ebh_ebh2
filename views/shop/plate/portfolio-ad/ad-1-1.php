...
$view_holder = 'http://static.ebanhui.com/ebh/tpl/newschoolindex/images/ad_1.jpg';
...
if (!empty($setting)) {
    ...
    ?>
    <img class="ad" viewholder="<?=htmlspecialchars($view_holder, ENT_COMPAT)?>" src="<?=htmlspecialchars($first_image, ENT_COMPAT)?>" width="285" height="318" />
...
if (empty($varpool['custom_data']['options'])) { ?>
    <div class="plate-module" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;height:<?=$varpool['height']?>px">
        <img src="<?=$view_holder?>" width="285" height="320" />
    </div>
    <?php return; }
?>
...
if (count($varpool['custom_data']['options']) == 1) {
    ...
    if (empty($ad['href'])) { ?>
        <div class="plate-module" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;height:<?=$varpool['height']?>px">
            <img src="<?=$ad['image']?>" width="285" height="320" />
        </div>
    <?php } else { ?>
        <div class="plate-module" style="<?php if (empty($varpool['float'])) { ?>position:absolute;top:<?=$varpool['top']?>px;left:<?=$varpool['left']?>px;<?php } else { ?>margin-top:<?=$varpool['margin_top']?>px;<?php } ?>width:<?=$varpool['width']?>px;height:<?=$varpool['height']?>px">
            <a href="<?=htmlspecialchars($ad['href'], ENT_COMPAT)?>" target="_blank"><img src="<?=$ad['image']?>" width="285" height="320" /></a>
        </div>
    <?php } return; } ?>
