...
<form  method="post" action="/admin/pcategories/edit.html" onsubmit="return checkAll()">
<input type="hidden" name="catid" value="<?=$catinfo['catid']?>">
...
<tr><th>显示顺序</th><td><input type="text" class="w50" id="displayorder" name="displayorder" value="<?=$catinfo['displayorder']?>" onblur="return check_num()"></td></tr>
</tr>
...
<tr><th>打开方式<p>点击该频道时，页面的打开方式。</p></th><td>
  <select id="target" name="target">
    <option value="" <?php if($catinfo['target']!='_blank')echo 'selected=selected';?>>本页打开</option>
    <option value="_blank" <?php if($catinfo['target']=='_blank')echo 'selected=selected';?>>新窗口打开</option>
  </select></td></tr>
