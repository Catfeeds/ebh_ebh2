...
<form method="post" action=<?php echo geturl('admin/evaluate/edit'); ?> onsubmit="return check()">
...
<tbody>
  <tr>
    ...
    <td>
      <input type="text" name="title" class="w300" value="<?=$evaluate['title']?>" />
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      <textarea name="descr" rows="2" style="width:60%;height:120px;" cols="20" ><?=$evaluate['descr']?></textarea>
    </td>
  </tr>
<tr>
  <th>量表导语<span>＊</span><p>描述量表引导语0-200字</p></th>
  <td >
  <textarea name="tutor" rows="2" style="width:60%;height:120px;" cols="20" ><?=$evaluate['tutor']?></textarea>
  </td>
</tr>

<tr><th>量表配图<span>＊</span><p>配图用于封面显示,不要过大,建议尺寸71*83</p></th><td>
<br>
<?php if($evaluate['logo']){?>
<img src="<?=$evaluate['logo']?>"  style="height:120px;width: 120px"/><br /><br />
<?php }?>
<?php
...
<input type="text" class="w100" name="total" value="<?=$evaluate['total']?>">
</td>
</tr>
<tr><th >每道问题选项数<p>不统一可以写0或者不填</p></th><td>
<input type="text" class="w100 nums"  name="nums" value="<?=$evaluate['nums']?>" onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')"  />
<span id="pitem">
<?php 
if(!empty($evaluate['itemstr'])){
	$itemarr = unserialize($evaluate['itemstr']);
	//var_dump($itemarr);
	$html = '';
	foreach($itemarr as $item){
	$html.='<p> 选项 <input type="text" name="item[]" value="'.$item['item'].'" /> 分值 <input type="text" name="score[]" value="'.$item['score'].'" /></p>';
	}
	echo $html;
}?>
...
$(function(){
	 //清空缓存
	 $(".nums").val('<?=$evaluate['nums']?>');
	 //动态添加选项
	 $(".nums").blur(function(){
		 var num = $(this).val();
		 var onum = '<?=$evaluate['nums']?>';
		 if(onum!=num){
			 var i = 0;
			 var pps = [];
			 $("#pitem").empty();
