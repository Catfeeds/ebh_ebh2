...
?>
    ...
    <style>
        ...
        .toptable td{padding-left:0;background:none;}
    </style>
            ...
            <td class="actions">
                <table summary="" cellpadding="0" cellspacing="0" border="0" align="right">
                    <tr>
                        ...
                        <td class="active"><a href="<?=geturl('admin/sporder/btachremove')?>">批量删除</a></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <div class="lefrig" style="background:#fff;float:left;margin-top:15px;width:788px;">
        <div class="annuato" style="line-height:28px;padding-left:20px;position: relative;">
            <a href="http://static.ebanhui.com/ebh/file/dinput.xls" class="mobancun" target="_blank">导入模板下载</a>
        </div>
        <div class="huidis" style="width:905px;">
            <div class="neileft105" style="width:680px;">

                <?php if(empty($inputresult['hasresult'])){?>
                    <form id="inputform" action="<?=geturl('admin/sporder/btachremove')?>" method="post" enctype="multipart/form-data">
                        ...
                        <table cellspacing="0" cellpadding="0" class="toptable">
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input type="text" class="w300" readonly="readonly" value="" id="crname" name="crname" required="required">
                                    <input type="button" id="drop" value="选择" />
                                    <input type="button" id="clear" value="清除" />
                                    <input type="hidden" name="crid" id="mediaid"  value="" required="required" />
                                </td>

                            </tr>
                        </table>
                        <br />
                        <input style="float:left;width:350px;height:30px;cursor:pointer;" type="file" name="inputfile" required="required" />
                    ...
                    <p class="pzhuyi" style="float:left;">注意<span style="color:red;">(非常重要)</span>：<br />
                        ...
                        5.删除课程如有多门,请以英文的逗号(,)隔开。<br />
                        6.删除课程中的科目名称必须和对应网校中的课程名称完全一致，并且已经开设服务项。<br />
                ...
                <?php if(!empty($inputresult) && $inputresult['hasresult']){
                    if(isset($inputresult['result']) && $inputresult['result'] == true){?>
                        <p class="ptishi"><?php if ($inputresult['rowcount'] > 0) { ?>批量删除成功，共处理 <?=$inputresult['rowcount']?> 条学生记录。<?php } else { ?>执行完成<?php } ?></p>
                        <input class="xuetjbtn" type="button" value="继续导入" onclick="window.location.href='<?=geturl('admin/sporder/btachremove')?>'" />
                    ...
                    <?php }else{?>
                        ...
                        <input class="xuetjbtn" type="button" value="重新导入" onclick="window.location.href='<?=geturl('admin/sporder/btachremove')?>'" />
