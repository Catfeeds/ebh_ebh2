...
<a href="http://static.ebanhui.com/ebh/file/<?=$roominfo['property']==3?'classinput_business':'lcyhg_11'?>.xls" class="mobancun" target="_blank">导入模板下载</a>
</div>
<div class="huidis">
<div class="neileft105">

<?php if(empty($inputresult['hasresult'])){?>
<form id="inputform" action="<?=geturl('/aroomv2/classes/input.html?aroomv=3')?>" method="post" enctype="multipart/form-data">
...
3.excel文件中的必须包含部门、姓名、账号三个字段。<br />
<?php }else {?>
...
<input class="xuetjbtn" type="button" value="继续导入" onclick="window.location.href='<?=geturl('/aroomv2/classes/input.html?aroomv=3')?>'" />
	<?php }else{?>
<br />
...
<input class="xuetjbtn" type="button" value="重新导入" onclick="window.location.href='<?=geturl('/aroomv2/classes/input.html?aroomv=3')?>'" />
</p>
<?php }}?>
