...
<?php 
			...
			;
				...
				$sourcepath = 'http://static.ebanhui.com/ebh/tpl/default/images/tx120.png';
	...
	<div class="lefrig" style="border:solid 1px #cdcdcd;background:#fff;float:left;margin-top:15px;width:786px;">
		...
		<div class="tab_box" style="border:none;">
			<div class="ecenter">
		  	<div class="uploadimg">
				<div class="l" style="">
				<form name="uploadfile" id="uploadfile" enctype="multipart/form-data" method="post" action="<?=geturl('troom/setting/avatarold')?>">
					<div class="fileinput" style="width:600px;height:55px;"><input type="text" value="<?=!empty($fname)?$fname:''?>" style="height:22px;line-height:22px;z-index:-1" class="upipt" />
					<input type="button" style="" value="浏览" class="centerBtn3">　<p class="tiship">仅支持JPG，PNG，GIF图片文件，且文件小于1M</p></div>
					<input type="file" style="filter:Alpha(Opacity=0);opacity:0;" onchange="checktype(this.value)" class="upiptfile" size="40" id="UpFile" name="UpFile"/>
					<input type="hidden" value="<?=geturl('teacher/setting/avatarold')?>" name="returnurl" />
					<input type="hidden" value="teacher" name="op" />
				</form>
				</div>
	
			</div>
			<div class="upimgl" style="margin-top:100px;">
				<div style="width:300px;" class="l" id="step2">
				<form name="setavatar" id="setavatar" action="">
					<input type="hidden" value="teacher" name="op" />
					...
					<div id="cut_div" class="Canvas">
						...
						<img src="<?=$sourcepath?>" style="position:relative; top:-230px; left:45px" id="cut_img" />
					</div>
					...
					<input type="button" class="Btn118 pointer" style="background:#f39800;" onclick="saveimg()" value=" 确认裁剪并提交 " />
					<input type="button" style="margin-left: 20px;" class="centerBtn2 pointer" class="rb1" name="cancel" id="cancel" value="取消" onclick="javascript:history.back(1);"  />
					</div>
...
<div class="explanation" style="margin-top:100px;width:350px;">
	<h3>上传说明:</h3>
	<p>图片格式：支持jpg/png/jpeg/gif格式的图片</p>
	<p>图片大小：大小不要超过1M</p>
	<p>操作说明：选择你喜欢的照片，裁剪并提交即可</p>
	<h3>头像照片示例：</h3>
	<div class="Sample" style="width:320px;">
		<div class="tox120">
			<img class="tx120" src="http://static.ebanhui.com/ebh/tpl/default/images/tx120.png" alt="昵称" />
		</div>
		<div class="samp">
			<h3>本人真实头像</h3>
			<p>清晰、正面、免冠</p>
...
<span style="width:270px;height:24px;position: absolute;top:540px;left:270px;">如果无法上传头像,请尝试使用<a style="color:#49a0cb;" href="<?= geturl('teacher/setting/avatar')?>">摄像头上传模式</a></span>	
</div>
<div class="clear"></div>
		  </div>
...
function saveimg(){
	...
	$.ajax({
		url:'<?=geturl('troom/setting/avatarupdate')?>',
		type:'post',
		data:_data,
		success:function(data){
			if(data=="1"){
				alert('上传成功');
				window.location.href = "<?=geturl('teacher/setting/rprofile')?>";
			}else{
