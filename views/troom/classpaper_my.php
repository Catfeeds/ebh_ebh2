...
<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/classpaper') ?>">在线考试</a> > <a href="<?= geturl('troom/classpaper/cor') ?>">批改试卷</a> > <?= $myclass['classname'] ?>
</div>
				...
				<td>
					...
					<div style="float:left;width:238px;">
						...
						<?php if($exam['status'] == 0) { ?>
							...
							<?php }?>
						<?php } else { ?>
						<a class="previewBtn" href="<?= geturl('troom/classpaper/all-0-0-0-'.$myclass['classid'].'-'.$exam['eid']) ?>" >作业批阅</a>
						<?php } ?>
						<?php if($exam['uid'] == $uid) { ?>
...
function delexam(eid,crid) {
	$.confirm("试卷删除后，此试卷下的学生答题记录也会删除，确定要删除吗？",function(){
		var url = '<?= geturl('troom/classpaper/del') ?>';
		$.ajax({
			url:url,
			type:'post',
			data:{'eid':eid},
