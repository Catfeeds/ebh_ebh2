...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('troom/exam') ?>">在线作业</a> > 录入作业
</div>
<div class="lefrig">
<div class="workol">
<div class="work_menuss">
	<ul>
		<li class="workcurrent"><a href="<?= geturl('troom/exam') ?>"><span>录入的作业</span></a></li>
		<li><a href="<?= geturl('troom/exam/all') ?>"><span>批阅作业</span></a></li>
	</ul>
</div>
...
function delexam(eid,title,cwid) {
    var url = "<?= geturl('troom/exam/del') ?>";
	$.confirm("确认删除作业 [" + title + "] 吗？",function(){
		$.ajax({
			url:"<?= geturl('troom/exam/del') ?>",
			type:'post',
			...
			success:function(data){
				if(data=='success'){
					$.showmessage({
						...
						timeoutspeed    :       500,
						callback :    function(){
							location.href='<?= geturl('troom/exam') ?>';
							//location.reload();
						}
					});
				}else{
