...
float:left; padding: 15px 0;
	...
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/subject') ?>">课程列表</a> > <a href="<?= geturl('troom/subject/'.$folder['folderid']) ?>"><?= $folder['foldername'] ?></a> > <?= $course['title']?>
		</div>
	<div class="lefrig">
	...
	<tr>
		<th>课件标题：</th>
		<td>
			<input class="uipt w340" id="title" maxlength="25" type="text" onblur="teacher(this,'课件标题','title','','n','','','请输入课件标题。',25)" name="title" value="<?= $course['title'] ?>" />
	...
	<tr id ="dnone">
		...
		<td>
			<select name="sectionid" id="sectionid" onblur="teacher(this,'所属目录','sectionid','','n','','','所属目录不能为空')" value="<?= $course['sid']?>">
	...
	<td colspan="2">
        ...
        <div style="width:100%;padding-left:30px">
            ...
            <?php
        $editor->xEditor('message','100%','300px',$course['message']);
        ?>
        </div>
	</td>
	</tr>
	...
	<div style="float:left;display:block;min-height:40px;margin-top: 10px;">
		<?php 
                    $upcontrol->upcontrol('up',3,array('upfilename'=>$course['cwname'],'upfilepath'=>$course['cwsource'].','.$course['cwurl'],'upfilesize'=>$course['cwsize']));
        ?>
		</div>
	</td>
	</tr>
	...
	<tr>
		<th>排序编号：</th>
		<td>																				   
			<input class="uipt w100" id="cdisplayorder" name="cdisplayorder" type="text" onblur="teacher(this,'排序编号','cdisplayorder','^[0-9]*$','n',null,'','请输入排序编号,排序号越小越靠前。')" value="<?= $course['cdisplayorder']?>" />
	...
	<tr>
     	<th></th>
    	<td><input class="huangbtn" name="" type="button" value="确认修改" onclick="course_submit()"/><input class="lanbtn marlef" name="" onclick="window.history.back(-1)" value="取 消" type="button" /></td>
    </tr>
	</table>
	</form>
	</div>
                ...
                success: function(data){
                    if(data != null && data != undefined && data.status == 1) {
                         $.showmessage({
                            ...
                            title:'修改课件',
                            callback :function(){
                                 document.location.href = "<?= geturl('troom/subject/'.$folder['folderid']) ?>";
                            }
                        });
                    } else {
                        $.showmessage({
