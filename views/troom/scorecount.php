...
<link type="text/css" href="/static/tpl/default/css/E_ClassRoom.css" rel="stylesheet" />
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/statisticanalysis') ?>">统计分析</a> > 成绩统计
	<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd>
		<div class="category_cont1">
			<div>
				<a <?= empty($classid) ? 'class="curr"':'' ?> href="<?= geturl('troom/statisticanalysis/scorecount') ?>">所有学生</a>
			</div>
			
			<?php foreach($classlist as $myclass) { ?>
			<div>
				<a <?= $classid==$myclass['classid']?'class="curr"':'' ?> href="<?= geturl('troom/statisticanalysis/scorecount-0-0-0-'.$myclass['classid']) ?>"><?= $myclass['classname'] ?></a>
			</div>
			<?php } ?>
		</div>
	</dd>
...
<tbody>
	<?php if(!empty($exams)) { ?>
		...
		<tr>
			...
			<td width="10%">
				<?php if($classid > 0) { ?>
				<a class="previewBtn" title="查看详情" href="<?= geturl('troom/statisticanalysis/scorecount/'.$exam['eid'].'----'.$classid) ?>"><span>查看详情</span></a>
				<?php } else { ?>
				<a class="previewBtn" title="查看详情" href="<?= geturl('troom/statisticanalysis/scorecount/'.$exam['eid']) ?>"><span>查看详情</span></a>
				<?php } ?>
			</td>
		</tr>
		<?php } ?>
...
$(function(){
	...
	$('#searchbutton').click(function(){
		...
		var href = '<?= geturl('troom/statisticanalysis/scorecount-0-0-0-'.$classid)?>?q='+searchvalue;
