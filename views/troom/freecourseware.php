...
<div class="ter_tit">
<a href="#">当前位置</a> > <a href="<?=geturl('troom/subject')?>" >课程列表</a> > 免费课件
</div>
<div class="lefrig">
...
<table width="100%" class="datatab">
			...
			?>
			  ...
			  <!--{/if}-->
    				...
    				<td width="270px"><span style="line-height:1.8;float:left;"><a target="<?= (empty($cl['cwurl']) || $type == 'flv') ? '_blank' : '' ?>"  href="<?= (empty($cl['cwurl']) || $type == 'flv') ? geturl('troom/course/'.$cl['cwid']) : geturl('troom/classcourse/'.$cl['cwid']) ?>" ><?=$cl['title']?></a></span></td>
...
function searchcour(){
	...
	servalue = replaceAll(servalue,'\'','');
	servalue = replaceAll(servalue,'%','');
	var url='<?=geturl('troom/subject/freecourseware-0-0-0-')?>';
...
function isfree(cwid,isfreeobj){
		...
		$.ajax({
			url:"<?=geturl('troom/subject/upisfree')?>?t="+Math.random(),
			type:'post',
