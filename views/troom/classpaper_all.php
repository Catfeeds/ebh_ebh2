...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troom/classpaper') ?>" >在线考试</a> > <a href="<?= geturl('troom/classpaper/my-0-0-0-'.$myclass['classid']) ?>"><?= $myclass['classname'] ?></a> > 试卷批阅
	<a href="/troom/classpaper/examexcel-0-0-0-<?= $myclass['classid'];?>-<?= $myexam['eid'];?>.html" class="addbtnas previewBtn">导 出</a>
...
<div class="lefrig">
		...
		<form action="<?= geturl('troom/classpaper/correct') ?>" method="post" id="form" >
...
$(function(){
	...
	$(".replyBtn").click(function(){
		...
		if(startscore>totalscore||endscore>totalscore){
			alert('分数区间值不能大于试卷总分'+totalscore);
			return false;	
		}
		...
		$.post('<?= geturl('troom/classpaper/correct') ?>',formdata,function(json){
			if(json.code){
				alert('成功设置 '+json.nums+' 份试卷评语');
				location.reload();
			}else{
				alert('批量设置评语失败!\n你可以修改分值区间重新设置!');
				}
	...
	$("#aidarr").attr('value',aidArr.join(','));
		...
		<script type="text/javascript">
			...
			$(".completetime").click(function(){	
				if(order=="1") {
					location.href='<?= geturl('troom/classpaper/all-0-2-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}else{
					location.href='<?= geturl('troom/classpaper/all-0-1-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}
			});
			$(".totalscore").click(function(){
				if(order=="3") {
					location.href='<?= geturl('troom/classpaper/all-0-4-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}else{
					location.href='<?= geturl('troom/classpaper/all-0-3-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}
			});

			function deleteanswer(aid,eid,uid) {
				$.confirm("删除后学生要重新提交试卷,确定要删除吗？",function(){
					var url = '<?= geturl('troom/classpaper/deleteanswer') ?>';
