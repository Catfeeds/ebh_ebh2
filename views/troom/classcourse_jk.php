...
.completetime i{display:inline-block;height: 8px;width: 7px;line-height: 8px;margin-left:5px;background: url(http://static.ebanhui.com/ebh/tpl/default/images/orderbg.png) no-repeat 0 0;}
.totalscore i{display:inline-block;height: 8px;width: 7px;line-height: 8px;margin-left:5px;background: url(http://static.ebanhui.com/ebh/tpl/default/images/orderbg.png) no-repeat 0 0;}
.datatab {border:none;}
</style>

<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troom/classsubject') ?>" >课程管理</a> > <a href="<?= geturl('troom/classsubject/'.$myfolder['folderid']) ?>"><?= $myfolder['foldername'] ?></a> > <?= $course['title'] ?> > 学习监控
	<a href="/troom/classcourse/jkexcel-0-0-0-<?= $course['cwid'];?>.html" class="addbtnas previewBtn" target="_blank">导 出</a>
...
<div class="workol" style="float:left; margin-top:15px;">
	<div class="work_mes">
		<ul>
			<li class="workcurrent"><a href="<?= geturl('troom/classcourse/jk-0-0-0-'.$course['cwid']) ?>"><span>学习跟踪</span></a></li>
			<li><a href="<?= geturl('troom/classcourse/jk-0-0-1-'.$course['cwid']) ?>"><span>未学名单</span></a></li>
		</ul>
	</div>
			...
			<table width="100%" class="datatab">
				...
				<?php if(!empty($myuserlist)) { ?>
					 ...
					 <tr>
						...
						<td><?= date('Y-m-d H:i',$myuser['startdate']) ?></td>
						<td><?= date('Y-m-d H:i',$myuser['lastdate']) ?></td>
						<td><?= secondToStr($myuser['sumltimes']) ?></td>
						<td><?= $myuser['sumtimes'] ?></td>
					 </tr>
					 <?php } ?>
					  <?php } else { ?>
						<tr>
