...
.pbtns {
	...
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/statisticanalysis') ?>">查询统计</a> > 学生笔记统计
		<div class="diles">
				<?php
					$q= empty($q)?'':$q;
					if(!empty($q)){
...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd>
		<div class="category_cont1">
			<div>
				<a <?= empty($classid)?'class="curr"':''?> href="<?= geturl('troom/statisticanalysis/notessum') ?>">所有学生</a>
			</div>

			<?php foreach($classlist as $myclass) { ?>
			<div>
				<a <?= $classid==$myclass['classid']?'class="curr"':'' ?> href="<?= geturl('troom/statisticanalysis/notessum-0-0-0-'.$myclass['classid']) ?>"><?= $myclass['classname'] ?></a>
			</div>
			<?php } ?>
...
$(function(){
	...
	$('#searchbutton').click(function(){
		var href = '<?= geturl('troom/statisticanalysis/notessum-0-0-0-'.$classid)?>';
