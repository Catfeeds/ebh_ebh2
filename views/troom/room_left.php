...
<div class="cleft">
	...
	<div class="menubox">
		...
		<ul class="menulist">
					...
					<li id="li<?= $catitem['code'] ?>" class="li<?= $catitem['code'] ?>">
						<a href="<?= geturl('troom/'.$catitem['code']) ?>" onclick="refresh('<?= $catitem['code']?>')" target="mainFrame"><i class="ui_ico <?= $catitem['code']?>suffix"></i><?= ($room['crid']==10412&&$catitem['code']=='classsubject')?'校本资源库':$catitem['name']?></a>
					</li>
                        <?php } ?>

        <?php $selectcourse = Ebh::app()->getConfig()->load('selectcourse');
		if (in_array($room['crid'], $selectcourse['crids'])) {?>
			<li id="liselectcourse" class="liselectcourse"><a href="<?=geturl('troom/selectcourse/courselist')?>" onclick="refresh('selectcourse')" target="mainFrame"><i class="ui_ico selectcoursesuffix"></i>选课管理</a></li>
		<?php }?>
        <?php $examapply = Ebh::app()->getConfig()->load('examapply');
		if (in_array($room['crid'], $examapply['crids'])) {?>
			<li id="liexamapply" class="liexamapply">
				<a href="<?=geturl('troom/examapply')?>" onclick="refresh('examapply')" target="mainFrame"><i class="ui_ico authement"></i>认证管理</a>
		<?php }?>
			</li>
                <?php foreach($roompers as $per) { ?>
			<li id="li<?= $per['crid']?>" class="lifullcourse">
				<a href="<?= geturl('troom/fullcourse-0-0-0-'.$per['crid'])?>" onclick="refresh('<?= $per['crid']?>')" target="mainFrame"><i class="ui_ico fullcoursesuffix"></i><?= $per['crname']?></a>
			</li>
                <?php } ?>
                <?php if(0 && ($room['isschool'] == 3 || $room['isschool'] == 6 || $room['isschool'] == 7 && $room['domain'] != 'jx')) { ?>
		<li id="li<?= $helpcrid ?>" class="lihelpsuffix">
				<a href="<?= geturl('troom/fullcourse/lists-1-0-0-'.$helpcrid.'-'.$helpcid)?>" onclick="refresh('<?= $helpcrid ?>')" target="mainFrame"><i class="ui_ico helpsuffix"></i>帮助中心</a>
			</li>
                <?php } ?>
   ...
   <!-- <div class="menubox" style="margin-top:10px;">
	...
	<div class="extendbox">
		<ul class="extendlist">
			<li><a target="mainFrame" href="<?= geturl('troom/tplsetting') ?>"><i class="ui_ico tplsettingssuffix"></i>模板设置</a></li>
		</ul>
	</div>
        <?php } ?>
