...
var searchText = "请输入您要搜索的课件名称";
	...
	$(function(){
		...
		$("#room_search_btn").click(function(){
			...
			var url = "<?= geturl('troom/fullcourse/lists-1-0-0-'.$moduleid.'-'.$folder['folderid'])?>?q="+search;
			location.href=url;
		});
	});
</script>
...
<div class="ter_tit">
	当前位置 > <?php if($helpcrid != $moduleid){?><a href="<?= geturl('troom/fullcourse-0-0-0-'.$moduleid) ?>">所有课程</a> > <?php } ?><?= $folder['foldername']?>
	<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<?php foreach($sectionlist as $section) { ?>
	...
	<a class="sa" target="<?= (empty($course['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('troom/fullcourse/'.$course['cwid'].'-1-0-0-'.$moduleid.'-'.$folder['folderid'])?>" <?= $color ?>><?= $course['title'] ?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a><em class="yema" <?= $color ?>><?= $i++ ?></em></li>
	<?php } ?>
