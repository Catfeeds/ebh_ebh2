...
var searchText = "请输入您要搜索的课件名称";
	...
	$(function(){
		...
		$("#searchbutton").click(function(){
			...
			var url = "<?= geturl('troom/fullcourse/lists-1-0-0-'.$moduleid)?>?q="+encodeURIComponent(search);
			location.href=url;
		});
	});
//-->
...
<div class="lefrig" style="margin-top:15px;">
	...
	<li class="danke">
	<div class="showimg"><a href="<?= geturl('troom/fullcourse/lists-1-0-0-'.$moduleid.'-'.$folder['folderid'])?>"><img src="<?= empty($folder['img'])?'http://static.ebanhui.com/ebh/images/nopic.jpg':$folder['img']?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('troom/fullcourse/lists-1-0-0-'.$folder['folderid'].'-'.$moduleid)?>"><?= $folder['foldername']?>(<?= $folder['coursewarenum']?>)</a></span>
	</a>
	</li>
	<?php } ?>
	</ul>
