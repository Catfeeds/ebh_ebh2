...
.control .manage a.CP_a_fuc:hover {
	...
	<div class="ter_tit">
		当前位置 > 
		<a href="<?= geturl('troom/classsubject') ?>">课程管理</a> > 
		<a href="<?= geturl('troom/classsubject/'.$folder['folderid']) ?>"><?= $folder['foldername'] ?></a> > 
		<?=($cwtype=='live')?"发布直播课件":"发布课件"?>
		</div>
	<div class="lefrig" style="background:#fff;float:left;margin-top:15px;width:788px;padding-top:20px;">
...
<?php if($cwtype != 'live') {?>
	...
	<td>
		<br />
		<?php Ebh::app()->lib('Webuploader')->renderHtml('up'); ?>
		<span class="ts2" style="padding-left:0;float:left;">请选择要上传的文件，文件大小不超过500M。</span>
	</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
    ...
    function course_submit() {
        ...
        if(teacher_submit('course',param='cdisplayorder')) {
            ...
            $.ajax({
                url:"<?= geturl('troom/classcourse/add') ?>",
                type: "POST",
...
$(function(){
	parent.window.preparexPhoto("photouploader",callback);
});
function uploadlogo(){
