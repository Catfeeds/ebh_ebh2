...
<div class="ter_tit">
	当前位置 &gt; <a href="<?=geturl('troom/examapply')?>">认证管理</a> &gt; <a href="<?=geturl('troom/examapply')?>">成绩统计</a> &gt; 已发证
        <div class="diles fr">
...
<div class="lefrig">
    <div class="workol">
        <div class="work_mes">
            ...
            <ul class="extendul">
                <li><a href="/troom/examapply/result/<?=$eid?>.html"><span>已通过未发证</span></a></li>
                <li><a href="/troom/examapply/resultfail/<?=$eid?>.html"><span>未通过</span></a></li>
                <li class="workcurrent"><a href="/troom/examapply/resultaward/<?=$eid?>.html"><span>已发证</span></a></li>
            </ul>
        </div>
        <div class="clear"></div>
        <div>
...
function _search(){
	...
	if(searchkey == '请输入学生姓名或登录帐号')
		searchkey = '';
	location.href = '<?=geturl('troom/examapply/resultaward/'.$eid)?>?q='+searchkey;
}
function _examexcel(){
	var startdate = $('#startdate').val();
	var enddate = $('#enddate').val();
	location.href = '/troom/examapply/examexcel/<?=$eid?>.html?type=3&startdate='+startdate+'&enddate='+enddate;
}
</script>
