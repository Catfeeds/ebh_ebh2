...
<div class="ter_tit"> 当前位置 > <a href="<?= geturl('troom/weixin') ?>">微家校通</a> > 发信历史 </div>
<div class="lefrig">
	<form  name="student_send" method="get" action="/troom/weixin/list_msg.html">
	<div class="euwqie" style="background:#fff;padding:10px; margin-top:15px;">
				...
				<?php foreach ($sendList as  $sendInfo) {?>
					<tr class="">
						<td width="18%"><?=date('Y-m-d H:i:s',$sendInfo['dateline'])?></td>
							<td width="82%">
							<span style="float:left;width:580px;"><?=shortstr($sendInfo['weixin_content'],82)?></span><a href="/troom/weixin/showDetail.html?batchid=<?=$sendInfo['batchid']?>&id=<?=$sendInfo['id']?>">详细>></a>
						</td>
					</tr>
				<?php }?>
			<?php }else{?>
