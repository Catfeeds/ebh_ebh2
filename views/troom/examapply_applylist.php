...
<div class="ter_tit">
	当前位置 &gt; <a href="<?=geturl('troom/examapply')?>">认证管理</a> &gt; 待审核
        <div class="diles fr">
...
<div class="lefrig" style="min-height:1000px;">
    <div class="workol">
        <div class="work_mes">
            <ul class="extendul">
                <li class="workcurrent"><a href="<?=geturl('troom/examapply/applylist')?>"><span>待审核</span></a></li>
                <li><a href="<?=geturl('troom/examapply/passapply')?>"><span>已通过</span></a></li>
                <li><a href="<?=geturl('troom/examapply/unpassapply')?>"><span>未通过</span></a></li>
            </ul>
        </div>
        <div class="clear"></div>
		<div style="float:left;margin-top:0px;" class="workdata">
			<table class="datatab" cellpadding="0" cellspacing="0">
            	<tr class="first">
                    <td width="251"><span style="padding-left:20px;">账号</span></td>
                	<td width="210">姓名</td>
                    <td width="161">申请时间</td>
                    <td width="116">操作</td>
                </tr>
<?php foreach($applylist as $apply){?>
                <tr >
                    ...
                    <td><a href="javascript:;" onclick="applydetail(<?=$apply['applyid']?>)" class="annius">查看详情</a></td>
                </tr>
<?php }?>
			</table>
		</div>
        ...
        <div class="bottoms">
            ...
            <div class="fl ml20"><a id="dopass" href="javascript:;" class="result">审核通过</a> <a id="donopass" href="javascript:;" class="result ml15">审核不通过</a></div>
        </div>
        <div class="clear"></div>
<?=$pagestr?>
	</div>
...
function _search(){
	...
	if(searchkey == '请输入学生姓名或登录帐号')
		searchkey = '';
	location.href = '<?=geturl('troom/examapply/applylist')?>?q='+searchkey;
}
function applydetail(applyid){
	...
	$.ajax({
		type: 'post',
		url: '<?=geturl('troom/examapply/applydetail')?>',
		dataType: 'json',
		...
		success: function(data){
			if(data!=undefined && data!=null && data.code==1){
				...
				$("#major").html(major);
				$("#stuid").html(stuid);
				$("input[name='status'][value='"+data.apply['status']+"']").attr("checked",true);
...
function savecheck(){
	...
	var nexttype = 0;//下一个待审核
	$.ajax({
		type: 'post',
		url: '<?=geturl('troom/examapply/check')?>',
		dataType: 'json',
	...
	$("#dopass").click(function(){
		...
		button.add({
			...
			callback:function(){
				...
				$.post('/troom/examapply/batchcheck.html',{ids:idarr.join(","),status:1},function(data){
	...
	$("#donopass").click(function(){
		...
		button.add({
			...
			callback:function(){
				...
				$.post('/troom/examapply/batchcheck.html',{ids:idarr.join(","),status:2},function(data){
					if(data != undefined && data != null && data.code ==1){//审核成功
						$.showmessage({
							img : 'success',
							message:data.msg,
							title:'消息通知',
							callback:function(){
								H.get('dialogdonopass').exec('close');
								window.location.reload();
							}
						});
					} else {
		...
		if(!H.get('dialogdonopass')){
			H.create(new P({
				...
				width:455,
				padding:5,
				content:$('#dialogdonopass')[0],
				button:button
			}),'common');
		}
		H.get('dialogdonopass').exec('show');
	});
