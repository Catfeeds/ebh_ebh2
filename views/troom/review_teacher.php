...
<?php if(!empty($reviews)){ ?>
	...
	?>
			...
			&nbsp;课程名称：<a target="<?= ($type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('myroom/mycourse/'.$review['toid'])?>" style="font-weight:bold;"><?= $review['title']?></a> 主讲：<?= !empty($review['realname'])?$review['realname']:$review['nickname'] ?> <span style="float:right; margin-right: 10px;_margin-top:-25px;"><?= date('Y-m-d H:i:s',$review['dateline'])?></span>
			</div>
			<div class="grades">
			总体评分:
			...
			<?php if(!empty($review['rsubject'])){ ?>
				<div class="restore">
					...
					<h1>老师回复：</h1>
					<?= $review['rsubject'] ?><a target="<?= ($type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('myroom/mycourse/'.$review['toid'])?>?q=#rev" style="color:#2696f0;margin-left:10px;">继续评论</a>
					</div>
				</div>
			<?php } ?>
			</li>
