...
?>
	...
	<?php }else{?>
		当前位置 > <a href="/troom/statisticanalysis.html">查询统计</a> > <a href="/troom/statisticanalysis/teach.html"> 教师统计 </a> > 评论查看
	<?php }?>
	</div>
...
function shielddiv(cwid,logid){
	...
	function shield(cwid,logid){
		...
		$.ajax({
			...
			type:'post',
			url:'<?= geturl('troom/review/shield')?>',
			dataType:'json',
...
function canceldiv(cwid,logid){
	...
	function cancel(cwid,logid){
		...
		$.ajax({
			...
			type:'post',
			url:'<?= geturl('troom/review/cancleshield')?>',
			dataType:'json',
			data:{'cwid':cwid,'logid':logid},
			success:function(data){
				if(data != undefined && data.status != undefined && data.status == 1) {
