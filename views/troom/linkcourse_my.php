...
<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/classexam') ?>">班级作业</a> > 关联课件 > <?= $myclass['classname'] ?>
</div>
...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd style="width:681px;">
		<div class="category_cont1">
			<div>
				<a <?php if($classid==0){echo 'class="curr"';}?> href="/troom/linkcourse/my.html">所有班级</a>
			</div>
			<?foreach ($classDb as $class) {?>
				<div>
					<a <?php if($classid==$class['classid']){echo 'class="curr"';}?> href="/troom/linkcourse/my-0-0-0-<?=$class['classid']?>.html"><?=$class['classname']?></a>
				</div>
			<?php }?>
        </div>
	</dd>
		 ...
		 <?php if(!empty($exams)) { ?>
			...
			<?php 
				if($exam['grade'] == 0){
            		...
            		$classname = array_key_exists('c_'.$exam['classid'], $classDb)?'<a href="/troom/linkcourse/my-0-0-0-'.$exam['classid'].'.html">'.$classDb['c_'.$exam['classid']]['classname'].'</a>':$exam['classid'];
				}else{
					$relationName = '班级';
					$isclass = 0;
