...
<div class="biaowaim" style="margin-top:20px;">
  ...
  <div class="txtxdaru">
    ...
    <?php $editor->xEditor('message','750px','500px',$ask['message']); ?>
  ...
  <?php $upcontrol->upcontrol('image',1,array('upfilename'=>$ask['imagename'],'upfilepath'=>$ask['imagesrc']),'askimage'); ?>
  </div>

<div style="background:#fff;float:left">
	 <div style="float:left;margin-left:19px;width:70px;margin-top:14px;font-weight:bold;color:#777;">上传音频：</div>
	 <div style="float:left;margin-left:0px;width:600px;margin-top:10px;display:<?=empty($ask['audiosrc'])?"inline":"none"?> " id="audio_float" >
	...
	<div style="float:left;width:630px;height:50px;_margin-top:20px;display:<?=empty($ask['audiosrc'])?"none":"inline"?>" id="audio_show">
		...
		<span class="spanUpfilename" id="audio_name"><?=$ask['audioname']?></span>
	...
	<input type="hidden" value="<?=$ask['audiosrc']?>" name="audio" id="audio" />
 </div> 
 	
	<div class="fontfen" id="doheight" >
	  	<input class="tijibtn" type="button" value="提交问题" style="margin-left:320px;width:190px;margin-right:20px;float:left;"/>
	</div>
</div>
</form>
...
<div id="coursedialogdiv" style="display:none">
	...
	<?php if($otherfolders){ ?>
		<div class="ewtlt" style="width:550px;background:white;border-bottom:none;">
...
function editquestion() {
    $.ajax({
                url:"<?= geturl('troom/myask/editquestion/'.$qid) ?>",
                type: "POST",
