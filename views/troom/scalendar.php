...
.pbtns {
	...
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troom/tastulog') ?>">学生监察</a> > <?= $typename ?>
		<div class="diles">
			<?php
				$q= empty($q)?'':$q;
				if(!empty($q)){
...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd>
		<div class="category_cont1">
			<div>
				<a <?= empty($classid) ? 'class="curr"':''?> href="<?= geturl('troom/scalendar-0-0-0-0-'.$type) ?>">所有学生</a>
			</div>
	
                        <?php foreach ($classlist as $myclass) { ?>
			<div>
				<a <?= ($myclass['classid'] == $classid)? 'class="curr"':''?> href="<?= geturl('troom/scalendar-0-0-0-'.$myclass['classid'].'-'.$type)?>"><?= $myclass['classname'] ?></a>
			</div>
                        <?php } ?>
...
<tbody>
		...
		<tr>
			...
			<td width="7%"><a class="workBtn" style="color:#fff;text-decoration: none;" href="<?= geturl('troom/scalendar/'.$student['uid'].'-0-0-0-'.$type) ?>"/>查看</a></td>
		</tr>
                <?php } ?>
...
$(function(){
	...
	$('#searchbutton').click(function(){
		var href = '<?= geturl('troom/scalendar-0-0-0-'.$classid.'-'.$type) ?>';
