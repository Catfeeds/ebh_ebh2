...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('troom/online')?>" >在线直播</a> > 我的直播
<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
	...
	<?php foreach ($courseList as $value) {?>
		<tr onmouseout="this.bgColor='#ffffff'"	onmouseover="this.bgColor='#f2f7ff'" style="border-bottom:1px solid #efefef;">
	      <td style=" height: 35px;"><a  href="/troom/online/edit.html?op=view&id=<?=$value['id']?>" style="padding:5px 10px 5px;width:52%;color:blue; cursor: pointer;font-size:12px;" title="$value['title']"><?=$value['title']?></a></td> 
		   ...
		   <?php if(time()<=($value['cdate'])+$value['ctime']*60){?>
		   		<a href="/troom/online/edit.html?pos=my&id=<?=$value['id']?>" class="workBtn" style="margin:0;">修改</a>
		   		<?php 
...
function _search($classorgrade,$tag){
	...
	location.href='/troom/online/my.html?q='+q+'&'+$tag+'='+$classorgrade;
}
function _dosearch(){
