...
<div class="wangind">
	<div style="background:#fff;float:left;width: 788px;border-bottom:none;">
        <div class="cmain_top_r fr">
            <div class="titles fl">
                <span>Hello！<?=$user['realname']?>&nbsp;老师，欢迎使用<?= $room['crname'] ?>。</span>
            </div>
			<?php if(!empty($signed)){
		...
		<div class="esukang">
			...
			<a href="<?=geturl('troom/classsubject/courses')?>" class="addcourse" id="course"></a>
			<a href="<?=geturl('troom/myask')?>" class="addanswer" id="ask">
			<?php if($count>0 && $count<=99 ){ ?><span style="top:15px;" title="未回答的课程问题数量"><?= $count?></span>
			<?php }elseif($count>99){ ?><span style="top:15px;">99+</span>
			<?php } ?>
			</a>
			<a href="<?=geturl('troom/notice/send')?>" class="addnotice"></a>
			<a href="<?=geturl('troom/review')?>" class="review">
...
<?php if($room['isschool'] == 3 || $room['isschool'] == 6 || $room['isschool'] == 7) { ?>
    ...
    <div class="liattop">
        <h2 class="kusts">
            <span>通知</span>
            <div class="aenbh">
            	<a class="luist" href="<?= geturl('troom/notice/receive') ?>">更多 >></a>
            </div>
        </h2>
	...
	<?php foreach($notices as $k=>$notice) {?>
        <div class="brftg">
            <span class="lanyuan" style="width:345px;">
            	<a target="_blank" href="<?= geturl('troom/notice/'.$notice['noticeid'])?>"><?= shortstr($notice['title'],44)?></a>
            </span>
...
<?php if($room['isschool'] == 3 || $room['isschool'] == 6 || $room['isschool'] == 7) { ?>
	...
	<div class="liattop">
        <h2 class="kusts">
            <span>课件</span>
            <div class="aenbh">
            	<a class="luist" href="<?= geturl('troom/classsubject/courses') ?>">更多 >></a>
            </div>
        </h2>
    <?php 
    ...
    ?>
        <div class="kustke">
            <div class="ndtete">
                <a target="<?= (empty($cw['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= (empty($cw['cwurl']) || $type == 'flv') ? geturl('troom/course/'.$cw['cwid']) : geturl('troom/classcourse/'.$cw['cwid']) ?>" class="stgeke">
            ...
            <div class="ristke">
                <h3 class="stfton"><a target="<?= (empty($cw['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= (empty($cw['cwurl']) || $type == 'flv') ? geturl('troom/course/'.$cw['cwid']) : geturl('troom/classcourse/'.$cw['cwid']) ?>"><?=$cw['title']?></a></h3>
...
<?php if($room['isschool'] == 3 || $room['isschool'] == 6 || $room['isschool'] == 7) { ?>
	...
	<div class="liattop">
        <h2 class="kusts">
            <span>作业</span>
            <div class="aenbh">
            	<a class="luist" href="<?= geturl('troom/classexam/cor') ?>">更多 >></a>
            </div>
        </h2>
        <table width="100%" class="datable">
            ...
            ?>
                <tr class="">
                    ...
                    <td width="1%" style="">
                    <a href="<?= geturl('troom/classexam/cor')?>" class="jinxuexi">作业批阅</a>
                    </td>
                </tr>
            <?php }?>
            </tbody>
...
<?php if($room['isschool'] == 3 || $room['isschool'] == 6 || $room['isschool'] == 7) { ?>
	...
	<div class="liattop">
        <h2 class="kusts">
            <span>问题</span>
            <div class="aenbh">
            	<a class="luist" href="<?= geturl('troom/myask/askme') ?>">更多 >></a>
            </div>
        </h2>
                ...
                <tr class="">
                    <td width="45%" style="text-align:left;">
                        <div class="<?= $noticeclass ?>" style="float:left;margin-right:15px;">
                            <a href="/troom/myask/askme.html?aq=<?= $name?>">
                        ...
                        <div class="kneit">
                            <p class="tsiter">
								...
								<?php }?>
								<a  href="<?= geturl('troom/myask/'.$ask['qid']) ?>" style="color:
...
<!--
	...
	function getcreditstat(dp){
		...
		$.getJSON('/troom/mysetting/creditStat.html?dayfrom='+dayfrom+'&dayto='+dayto+'&rnd='+Math.random(), function (csv) {
