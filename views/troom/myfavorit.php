...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('troom/myask') ?>">师生答疑</a> > 我的关注
<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<div class="workol" >
	<div class="work_mes">
		<ul class="extendul">
			...
			<li class="workcurrent"><a href="<?= geturl('troom/myask/myfavorit') ?>"><span>我的关注</span></a></li>
			<!-- 新增 -->
...
$(function() {
   ...
   $("#ser").click(function(){
       ...
       if(title == searchtext) 
           title = "";
       var url = '<?= geturl('troom/myask/myfavorit') ?>' + '?q='+title;
       document.location.href = url;
   });
});
function delfavorit(aid) {
    var url = "<?= geturl('troom/myask/delfavorit') ?>";
    $.ajax({
	url:url,
	type:'post',
	data:{'aid':aid},
