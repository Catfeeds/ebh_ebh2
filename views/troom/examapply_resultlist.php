...
<div class="ter_tit">
	当前位置 &gt; <a href="<?=geturl('troom/examapply')?>">认证管理</a> &gt; 成绩统计
        <div class="diles fr">
...
<?php if(!empty($exams)) {
	foreach($exams as $exam) {?>
                    <tr >
                        ...
                        <td><a href="/troom/examapply/result/<?=$exam['eid']?>.html" class="annius">查看详情</a></td>
                    </tr>
<?php }
} else { ?>
...
function _search(){
	...
	if(searchkey == '请输入认证名称')
		searchkey = '';
	location.href = '<?=geturl('troom/examapply/resultlist')?>?q='+searchkey;
}
</script>
