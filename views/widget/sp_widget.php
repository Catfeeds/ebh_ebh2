...
<script type="text/javascript" src="http://static.ebanhui.com/ebh/js/artDialog/artDialog.js?skin=opera"></script>
<style type="text/css">
...
#header { position: relative; height: 60px; margin-top: 15px; border-bottom: 5px solid #ff9900; background-color: #ffffe0; }
...
#menu li.active { padding-top: 0; border: solid #ff9900; border-width: 2px 1px 0; background: #FFF; }
...
.title { margin-bottom: 10px; padding-bottom: 0.1em; border-bottom: 1px solid #ff9900;}
...
.footactions { margin: 0 0; padding: 0.5em; background: #FFF; border: 2px solid #ff9900; border-top-width: 0px; }
...
/*细线边框区域*/
/*-------------*/
.bgrcontent { width: 580px; background-color: #ffffe0; padding: 10px; margin: 0 auto;}
.bdrcontent { padding: 1em; border: 2px solid #ff9900; background: #FFF; zoom: 1; border-bottom-width: 0; }
    ...
    <div id="content">
      <div class="bgrcontent">
        <div class="bdrcontent">
            ...
            <p>
			<?=empty($data['message'])?'添加成功':$data['message']?>
			
            </p>
        </div>
        <div class="footactions">&nbsp; 
		<?php 
		foreach($data['note'] as $k=>$name){
		if($name=='确定并关闭'){?>
<a href="javascript:void(0)" onclick="closethis('<?=$data['returnurl'][$k]?>')"><?=$name?></a> &nbsp; 
<?php }else{?>
<a href="<?=$data['returnurl'][$k]?>"><?=$name?></a> &nbsp; 
<?php }}?>
</div>
      </div>
    </div>
...
function closethis(url){
	var newurl = "http://<?=$this->uri->uri_domain().'.'.$this->uri->curdomain?>"+url;
