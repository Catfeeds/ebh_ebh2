...
<div class="diles">
	...
	<div class="lefrig">
<div class="annuato" style="line-height:28px;padding-left:20px;position: relative;background:#fff;">
...
<div id="icategory" style="background:#fff;margin-bottom:10px;" class="clearfix">
	<dt>所属班级：</dt>
	<dd style="width:680px;">
		<div class="category_cont1">
			<div>
				<a <?php if(empty($classid)){?>class="curr"<?php }?> href="<?=geturl('aroom/astuexam-0-0-0-0')?>">所有学生</a>
			</div>
			<?php foreach($classlist as $cl){?>
			<div>
				<a <?php if($classid==$cl['classid']){?>class="curr"<?php }?>href="<?=geturl('aroom/astuexam-0-0-0-'.$cl['classid'])?>"><?=$cl['classname']?></a>
			</div>
			<?php }?>
		</div>
	</dd>
...
<tbody>
		...
		<tr>
			...
			<td style="width:300px;">
				<a title="班级作业" class="previewBtn" href="<?=geturl('aroom/astuexam/astuexamlist-0-0-0-'.$v['uid'])?>">查看作业</a>
			</td>
		</tr>
		<?php }}else{?>
...
$(function(){
	$('#searchbutton').click(function(){
		...
		var url = '<?=geturl('aroom/astuexam-0-0-0-'.$classid)?>';
