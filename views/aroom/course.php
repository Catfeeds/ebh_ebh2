...
<div class="ter_tit">
	当前位置 > <a href="<?=geturl('aroom/course')?>"><?=($room['crid']==10412)?'校本资源库管理':'课程管理'?></a> > 所有课程
	</div>
	...
	<div class="annuato" style="line-height:28px;padding-left:20px;position: relative;">
		<div class="tiezitool"><a class="hongbtn jiabgbtn" onclick="location.href='<?=geturl('aroom/course/add')?>'">开设课程</a></div>
	</div>
	...
	<?php if(!empty($courselist)){
			foreach($courselist as $fval){
				...
				if(!empty($fval['hassub'])){
					$atagbegin = '<a href="'.geturl('aroom/course/sub/'.$fval['folderid']).'">';
					$atagend = '</a>';
				}
	?>
		<tr>
		...
		<td width="20%">
		<a class="workBtn" href="<?=geturl('aroom/course/edit/'.$fval['folderid'])?>">编辑</a>
		<a class="workBtn" href="javascript:delcourse(<?=$fval['folderid']?>,<?=$fval['coursewarenum']?>);">删除</a>
		<?php if(!empty($fval['hassub'])){?>
			<a class="workBtn" style="margin-right:0px;" href="<?=geturl('aroom/course/sub/'.$fval['folderid'])?>">子目录</a>
		<?php }?>
		</td>
		</tr>
	<?php }}else{?>
