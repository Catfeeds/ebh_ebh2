...
<div class="ter_tit">
	当前位置 > <a href="<?=geturl('aroom/clist')?>">课程列表</a>
	</div>
	<div class="lefrig" style="margin-top:10px;">
	<div class="annuato" style="line-height:28px;padding-left:20px;position: relative;">此页为课程列表页面。
	</div>
...
<tbody>
	...
	?>
		<tr>
		<td width="150px" title="<?=$fval['foldername']?>"><?=shortstr($fval['foldername'],30,'')?></td>
		<td width="500px" id="teachername_<?=$fval['folderid']?>" title="<?=!empty($fval['teachers'])?$fval['teachers']:''?>"><?=!empty($fval['teachers'])?shortstr($fval['teachers'],30,''):''?></td>
		</tr>
	<?php }}else{?>
