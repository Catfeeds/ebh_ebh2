...
.jqTransformRadioWrapper{margin-top:4px;}*/
	...
	<div class="lefrig">
		...
		<div id="icategory" class="clearfix" style="background:#fff;">
			...
			<dd>
				<div class="category_cont1">
					<div>
						<a <?php if(empty($classid)){?>class="curr"<?php }?> href="<?=geturl('aroom/astuerrorbook-0-0-0-0')?>">所有学生</a>
					</div>
					<?php foreach($classlist as $cl){?>
					<div>
						<a <?php if($classid==$cl['classid']){?>class="curr"<?php }?>href="<?=geturl('aroom/astuerrorbook-0-0-0-'.$cl['classid'])?>"><?=$cl['classname']?></a>
					</div>
					<?php }?>
				</div>
			</dd>
		...
		<div class="workol" id="errlist" style="border-top:none;">
			<?php if(!empty($errorlist)){
				...
				<?php }}echo show_page($errorcount);}else{?>
					 ...
					 <div style="clear:both;padding-top:10px;padding-left:20px;">暂无记录</div>
				<?php }?>
...
$(function(){
	$('#searchbuttons').click(function(){
		...
		var href = '<?=geturl('aroom/astuerrorbook-0-0-0-'.$classid)?>';
