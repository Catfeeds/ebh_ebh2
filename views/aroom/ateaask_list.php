...
<a href="<?=geturl('aroom/ateaask')?>" style="width:90px;" class="tfanhui">返回上一页</a>
</div>
...
<?php if(empty($answerlist)){?>
				...
				?>
				 		<tr>
				 			<td width="300px;"><?=$al['title']?></td>
					  		<td><?=$al['foldername']?></td>
					    	<td><?=Date('Y-m-d H:i:s',$al['dateline'])?></td>
					    	<td width="8%"><?=$al['answercount']?></td>
							<td width="12%"><a class="previewBtn" target="_blank" href="<?=geturl('question/'.$al['qid'])?>">查看详情</a></td>
				    	</tr>
