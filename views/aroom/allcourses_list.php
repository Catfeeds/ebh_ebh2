...
<script type="text/javascript">
	$(function(){
		$("#room_search_btn").click(function(){
			...
			var url = '<?=geturl('aroom/allcourses/'.$folderdetail['folderid'].'-0-0-0')?>';
...
<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroom/allcourses')?>">学校所有课程</a> > 
			<?php if(!empty($uparr)){
				$upfoldercount = count($uparr);
				foreach($uparr as $k=>$upfolder){
				$index = $upfoldercount-$k-1;
			?>
			<a href="<?=geturl('aroom/allcourses/'.$uparr[$index]['folderid'])?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>
			<?= $folder['foldername'] ?>
	<?php 
...
<div class="kejian">
	...
	<div class="showimg"><a href="<?=geturl('aroom/allcourses/'.$subfolder['folderid'])?>" title="<?=$subfolder['foldername']?>"><img src="<?= empty($subfolder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subfolder['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('aroom/allcourses/'.$subfolder['folderid']) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
...
<div class="cqmain">
	...
	<?php $arr = explode('.',$course['cwurl']);
		...
		if($type == 'mp3')
			$type = 'flv';
		?>
	<a class="sa"  target="<?= (empty($course['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= (empty($course['cwurl']) || $type == 'flv') ? geturl('aroom/course/'.$course['cwid']) : geturl('aroom/allcourses/course/'.$course['cwid']) ?>" $color><?=$course['title']?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a><em class="yema" $color><?=$i++?></em></li>
<?php }}?>
	</ul>
</div>
</div>
