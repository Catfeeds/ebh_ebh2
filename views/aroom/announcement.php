...
<div class="flopadt">
<a class="hongbtn jiabgbtn" href="<?= geturl('aroom/announcement/add') ?>" >添加公告</a></div>
<table width="100%" class="datatab">
	<thead class="tabhead">
	 ...
	 <?php foreach($announcementlist as $send) { ?>
			...
			<a class="workBtn" href="<?= geturl('aroom/announcement/edit-0-0-0-'.$send['infoid'])?>" >修改</a>
...
<!--
	...
	$.confirm("您确定要删除这条公告吗？",function(){
		...
		$.ajax({
			url:"<?= geturl('aroom/announcement/delsend')?>",
			type:'post',
			data:{'infoid':infoid},
			dataType:'text',
