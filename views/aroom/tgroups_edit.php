...
.terlie .mylabelhover a.labeldel img {
	...
	<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroom/tgroups')?>">分组管理</a> > 编辑分组
		</div>
...
<form id="groupvalue" action="<?=geturl('aroom/tgroups/addGroup')?>" method="post">
<input type="hidden" name="groupid" value="<?=$group['groupid']?>" />
<div class="cqshangc">
	<h2 class="sckezi">编辑分组</h2>
	<div class="cqleftsc">
	<span class="pxxuanx">分组名称：</span>
	  <input name="groupname" id="groupname" type="text" onblur="checkgroupname()" class="inpxuanx" style="width: 333px;background: url(http://static.ebanhui.com/ebh/tpl/2012/images/inp_bg01.png) no-repeat;" value="<?=$group['groupname']?>" maxlength="20"/>
	  <p id="groupnamespan" style="margin:5px 0px 10px 80px; color:#999;">请输入分组名称如：语文组</p>
	  <span class="pxxuanx">分组排序：</span>
	  <input type="text" class="inpxuanx" onblur="checkdisplayorder()" style="width: 122px;background: url(http://static.ebanhui.com/ebh/tpl/2012/images/inp_bg02.png) no-repeat;" maxlength="10" id="displayorder" name="displayorder" value="<?=$group['displayorder']?>" />
	  	  <p id="displayorderspan" style="margin:5px 0px 10px 80px; color:#999;">序列号越小越靠前</p>

	<span class="pxxuanx">分组介绍：</span>
	  <textarea name="summary" style="min-height: 100px;height:100px;float:left;" onblur="checksummary()" id="summary" class="w388 txt" ><?=$group['summary']?></textarea>
		...
		$(".btnlogo").click(function(){
			if(checkgroupname() && checkdisplayorder() && checksummary()){
				$.ajax({
				   type: "POST",
				   dataType: "json",
				   url: "<?=geturl('aroom/tgroups/editGroupAjax')?>",
