...
<div class="ter_tit">
		...
		<div class="searchbtnx">
			...
			<a class="btnx float_left" href="javascript:void(0)" onclick="doexport($(this).attr('requesturl'))" requesturl = "/aroom/report/tcexcel.html?classid=<?=$classid?>&starttime=<?=$starttime_str?>&endtime=<?=$endtime_str?>"> 导出excel</a>
		</div>
	</div>
	<div class="lefrig">
...
<div id="icategory" class="clearfix" style="background:#fff;margin-bottom:10px;">
	...
	<dd>
		<div class="category_cont1">
			<div>
				<a <?php if(empty($classid)){?>class="curr"<?php }?> href="<?=geturl('aroom/report/tcreport-0-0-0-0')?>?starttime=<?=$starttime_str?>&endtime=<?=$endtime_str?>">所有班级</a>
			</div>
			<?php foreach($classlist as $cl){?>
			<div>
				<a <?php if($classid==$cl['classid']){?>class="curr"<?php }?>href="<?=geturl('aroom/report/tcreport-0-0-0-'.$cl['classid'])?>?starttime=<?=$starttime_str?>&endtime=<?=$endtime_str?>"><?=$cl['classname']?></a>
			</div>
			<?php }?>
		</div>
	</dd>
...
<tbody>
	...
	?>
		...
		<tr>
		<td width="100px" style="border-bottom:none;"><?=$sl['name']?></td>
		<td width="100px" style="border-bottom:none;"><?=$sl['foldername']?></td>
		<td width="200px" style="border-bottom:none;"><?=$sl['title']?></td>
...
<script>
	function dosearch(){
		...
		var url = "/aroom/report/tcreport-1-0-0-<?=!empty($classid)?$classid:0?>.html?starttime="+starttime+"&endtime="+endtime;
		location.href = url;
	}
