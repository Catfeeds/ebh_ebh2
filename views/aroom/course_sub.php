...
//-->
	...
	<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroom/course')?>">课程管理</a> >
		<?php if(!empty($uparr)){
				$upfoldercount = count($uparr);
				foreach($uparr as $k=>$upfolder){
				$index = $upfoldercount-$k-1;
			?>
			<a href="<?=geturl('aroom/course/sub/'.$uparr[$index]['folderid'])?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>
			<?= $folder['foldername'] ?>
	...
	<div class="annuato tiezi_search" style="line-height:28px;padding-left:20px;position: relative;">
		<div class="tiezitool">
			...
			<a class="mulubgbtn hongbtn" style="margin-right:10px;" href="<?=geturl('aroom/folder/add/'.$folder['folderid'])?>">添加目录</a>
			<?php }?>
...
<div class="kejian">
	...
	<?php if(empty($subject['hassub']) && $subject['coursewarenum']>0){?>
		...
		<span class="spne gray" title="目录下已有课件,不能操作"><?= shortstr($subject['foldername'],16,'') ?>(<?= $subject['coursewarenum'] ?>)<br/><a href="<?=geturl('aroom/folder/edit/'.$subject['folderid'])?>">[编辑]</a></span>
	<?php }else{?>
	<div class="showimg"><a href="<?= geturl('aroom/course/sub/'.$subject['folderid']) ?>" title="<?= $subject['foldername'] ?>"><img src="<?= empty($subject['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subject['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('aroom/course/sub/'.$subject['folderid']) ?>" style="text-decoration: none;" title="<?= $subject['foldername'] ?>"><?= shortstr($subject['foldername'],16,'') ?>(<?= $subject['coursewarenum'] ?>)</a><br/><a href="<?=geturl('aroom/folder/edit/'.$subject['folderid'])?>">[编辑]</a><a href="javascript:delcourse(<?=$subject['folderid']?>,<?=$subject['coursewarenum']?>);">[删除]</a></span>
