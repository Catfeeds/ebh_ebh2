...
.terlie .mylabelhover a.labeldel img {
	...
	<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroom/course')?>">课程管理</a> > 开设课程
		</div>
...
<div class="cqshangc">
	...
	<span class="pxxuanx">课程名称：</span>
	  <input name="foldername" id="foldername" type="text" onblur="checkfoldername()" class="inpxuanx" style="width: 333px;background: url(http://static.ebanhui.com/ebh/tpl/2012/images/inp_bg01.png) no-repeat;" value="" maxlength="20"/>
	...
	<?php
		if($roominfo['grade']==1){
			...
			for($i=1;$i<7;$i++){
				echo '<label><input style="margin-top:5px;float:left;width:26px;margin-left:10px" type="radio" value="'.$i.'" name="grade"/><span style="float:left">'.$gradearr[$i-1].'</span></label>';
			}
		}elseif($roominfo['grade']==7){
			$gradearr = array('初一','初二','初三');
			for($i=7;$i<10;$i++){
				echo '<label><input style="margin-top:5px;float:left;width:26px;margin-left:10px" type="radio" value="'.$i.'" name="grade"/><span style="float:left">'.$gradearr[$i-7].'</span></label>';
			}
		}elseif($roominfo['grade']==10){
			$gradearr = array('高一','高二','高三');
			for($i=10;$i<13;$i++){
				echo '<label><input style="margin-top:5px;float:left;width:26px;margin-left:10px" type="radio" value="'.$i.'" name="grade"/><span style="float:left">'.$gradearr[$i-10].'</span></label>';
			}
		}
...
<?php }?>
	...
	<input name="speaker" type="text" class="inpxuanx" style="width:310px;" value="主讲老师"/>
	</div>
	</div>
	</div>
