...
<p style="color:#817f7f;">定期修改密码，有助于保护您的账号安全及个人隐私</p>
</div>
<div class="etatb">
...
<a href="javascript:void(0)" onclick="chkform()" class="borhuangbtn" style="margin:40px 0 10px 120px;">确认</a>
</div>
...
function chkoldpwd(oldpassword,nochecktrue){
	...
	if(nochecktrue)
		return;
	$.ajax({
		url:"<?=geturl('home/profile/checkoldpassword')?>",
		type:'post',
...
function pwdchagne(oldpassword,password){
	$.post("<?=geturl('home/profile/updatepassAjax')?>",{oldpassword:oldpassword,password:password},function(result){
		if(result>0){
			var d = dialog({
				title: '提示信息',
				content:'密码修改成功！',
