...
<div class="divcontent">
	...
	<div class="eertrew" id="myform" style="height:490px">
	<div class="khrtye">
	...
	<div class="ksert">
      <span>当前支付密码：</span>
      <input id="oldpaypwd" class="hterys xinput" type="password"  value="" name="oldpaypwd" x_hit="请输入旧的支付密码" x_func="chkoldpwd" />
   	</div>
	...
	<div class="ksert">
     <a href="/homev2/safety/paypass.html?type=paypass&op=forget" class="etksret">忘记旧密码？</a>
    </div>
	<div class="ksert">
...
<script type="text/javascript">
	...
	function chkoldpwd(obj){
		obj.res = -1;
		var oldpaypwd = _xform.getdata().oldpaypwd;
		if (oldpaypwd == '') {
			obj.msg = '请输入旧的支付密码！';
		} else if (oldpaypwd.length < 6 || oldpaypwd.length > 16 ) {
			obj.msg = '密码位数不正确。';
		}else {
			//验证是否与登录密码相同
			$.ajax({
				...
				data:{op:'checkoldpaypwd',oldpaypwd:oldpaypwd},
				dataType:'json',
				type:'GET',
	...
	function docheck(){
		...
		$.ajax({
			url:'/homev2/safety/paypass.html',
			data:{op:'save_edit',password:password,oldpaypwd:oldpaypwd,level:level},
			dataType:'json',
			type:'GET',
			success:function(json){
				if(json.code){
