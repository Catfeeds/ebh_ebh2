...
<div class="conentlft">
	<div class="lefrig"style="margin-top:10px; background:#fff;">
		<div class="work_menu" style="width:786px; position:relative;margin-top:0px;">
			<ul>
				 <li class="workcurrent"><a href="<?=geturl('homev2/score/lottery/exchange')?>" style="font-size:16px;"><span>兑换礼品</span></a></li>
				 <li><a href="<?=geturl('homev2/score/lottery/exchange_des')?>" style="font-size:16px;"><span>兑换说明</span></a></li>
			</ul>
		</div>
		<?php $i=0;
		...
		$.ajax({
			...
			success:function(_json){
				if(_json.result!='success'){
					...
					$("#res").html('你兑换的礼品已保存在我的积分兑换明细中，<a href="<?= geturl('homev2/score/record')?>" target="_blank" style="font-weight:bold; color:#000;">点击查看</a>');
					$("#message").show();
				}
			}
		});
		...
		$.ajax({
			...
			success:function(_json){
				if(_json.result!='success'){
					...
					$("#resmsg").html('你兑换的礼品已保存在我的积分兑换明细中，<a href="<?= geturl('homev2/score/record')?>" target="_blank" style="font-weight:bold; color:#000;">点击查看</a>');
					$("#virtualmessage").show();
				}
			}
		});
