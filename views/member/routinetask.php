...
<div class="cright_cher">
	...
	当前位置 > <a href="<?=geturl('member/score/routinetask')?>">我的积分</a> > 常规任务
	</div>
...
foreach($tasklist as $tl){
	...
	if($tl['type']==1){
		...
		if(substr($tl['url'],0,8)=='[domain]')
			$tl['url'] = 'http://'.(!empty($room)?$room['domain']:'www').'.ebanhui.com/'.'/'.substr($tl['url'],8).'.html';
		else 
			$tl['url'] = 'http://'.$_SERVER['SERVER_NAME'].'/'.$tl['url'].'.html';
		if(!$tl['isactive'])
