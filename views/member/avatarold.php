...
<link href="http://static.ebanhui.com/ebh/tpl/default/css/E_ClassRoom.css" rel="stylesheet" type="text/css" />
		...
		<div class="center" style="float:left;width:750px;">
			<div class="uploadimg">
			<form name="uploadfile" id="uploadfile" enctype="multipart/form-data" method="post" action="<?=geturl('member/setting/avatarold')?>">
				...
				<input type="hidden" value="<?=geturl('member/setting/avatarold')?>" name="returnurl" />
		...
		<div class="explanation">
			...
			<div class="Sample">
				...
				<span style="margin-left:-50px;margin-top:200px;float:left;">如果无法上传头像,请尝试使用<a style="color:#49a0cb;" href="<?=geturl('member/setting/avatar')?>">摄像头上传模式</a></span>
		</div>
		</div>
...
function saveimg(){
	...
	$.ajax({
		url:'<?=geturl('member/setting/avatarupdate')?>',
		type:'post',
		data:_data,
		success:function(data){
			if(data=="1"){
				alert('上传成功');
				window.location.href = "<?=geturl('member/setting/profile')?>";
			}else{
