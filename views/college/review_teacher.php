...
<!--
	...
	$(function(){
		...
		$("#searchbutton").click(function(){
			...
			var url = '/college/review/teacher.html?q='+encodeURIComponent(search);
			location.href=url;
		});
	});
//-->
	...
	?>
			...
			<?php if(!empty($review['replysubject'])){ ?>
				<div class="restore">
					...
					<div class="restore_cont" style="width:825px;">
					<h1>老师回复：</h1>
					<?= $review['replysubject'] ?><a target="<?= (empty($review['cwurl']) || $type == 'flv') ? '_blank' : '_blank' ?>" href="<?= geturl('myroom/mycourse/'.$review['toid'])?>?q=#rev" style="color:#2696f0;margin-left:10px;">继续评论</a>
					</div>
				</div>
			<?php } ?>
			</li>
