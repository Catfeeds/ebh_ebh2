...
<div class="workol">
	<div class="work_mes" style="width:998px; position:relative;">
		<ul style="float:left; display:inline;">
			...
			<li class="workcurrent"><a href="<?= geturl('college/myask/hot') ?>"><span>热门</span></a></li>
			...
			<li><a href="<?= geturl('college/myask/myfavorit') ?>"><span>我的关注</span></a></li>
		</ul>
			...
			<div class="workdata" style="margin-top:0px;float:left; width:1000px;">
				<table  width="100%" class="datatab" style="border:none;">
			  	...
			  	<tbody>
						<?php if(empty($asks)) { ?>
			  			<tr>
							<td colspan="6" align="center" class="nonejunrs" style="border-top:none;"></td>
						</tr>
                         <?php } else { ?>
										...
										<td style="border-top:none;">
											<?php $name=empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>
												<div style="float:left;margin-right:15px;"><a href="<?php if ($user['uid'] != $avalue['uid']) { ?>/sns/feeds/<?=$avalue['uid']?>.html<?php } else { ?>/sns/feeds/my.html<?php } ?>"><img class="radiust" title="<?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>的个人空间" src="<?=$face?>" /></a></div>
													...
													<span class="cwsp" style="margin-left:294px"><a href="/college/myask/hot.html?cwid=<?=$avalue['cwid']?>"><?=shortstr($avalue['cwname'],40)?></a></span>
													</div>
													<?php }?>
												</div>
...
$(function() {
   ...
   $("#ser").click(function(){
       ...
       if(title == searchtext) 
           title = "";
       var url = '<?= geturl('college/myask/hot') ?>' + '?q='+title;
	   <?php if(!empty($folder)){
