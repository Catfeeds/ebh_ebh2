...
<link rel="stylesheet" type="text/css" href="http://exam.ebanhui.com/static/css/tikutop.css?v=20160412" />
...
<script type="text/javascript" src="http://static.ebanhui.com/ebh/js/xquestion_for_multic.js?v=20151123001?>"></script>
<style type="text/css">
...
#courseselecttit,#middleselecttitle,#mysecondselecttitle{height: 25px;width:120px;display:block;overflow:hidden}
.work_menu ul li a{
	font-family:微软雅黑;
	font-size:16px;
	color:#666;
...
<!-- 主要内容 -->
<div class="lefrig" id="lefrig" style="background:#fff;float:left;margin-top:0px;width:998px;">
	<?php 
	if(!empty($folder)){
	...
	<div class="workol" id="errlist" style="margin-top:0;">
		<div class="work_menu" style="position:relative;margin-top:0;">
    <ul>
    	<li ><a href="<?= geturl('college/myexam/all') ?>"><span>做作业</span></a></li>
		<li><a href="<?= geturl('college/myexam/my') ?>"><span>我做过的</span></a></li>
		<!--<li><a href="<?= geturl('college/myexam/box') ?>"><span>草稿箱</span></a></li>-->
		<li class="workcurrent"><a href="<?= geturl('college/myerrorbook') ?>"><span>错题本</span></a></li>
    </ul>
<?php if(empty($folder)){?>
	<div class="diles">
	<?php
...
<div id="schoollayerWrap" style="z-index:999;width: 980px;height: 115px;margin:0 auto;">
	<?php $this->display('college/chapter_fragment')?>
</div>
...
<script type="text/javascript">
	$(function(){
		...
		<?php if(!empty($folder)){?>
			$.each($('.work_menu li a'),function(k,v){
				$(this).attr('href',$(this).attr('href')+'?folderid=<?=$folder['folderid']?>');
			});
		<?php }?>
	});
	...
	function searchs(strname){
		...
		location.href='<?= geturl('college/myerrorbook')?>?q='+sname;
	}
	...
	*/
	function page_load(pagetxt,param){
		...
		var url = "/college/myerrorbook/getajaxpage.html?q="+encodeURIComponent(param.keyword);
		$("#lodding").show();
		...
		/**ajax后台读取json数据*/
		$.post(url,{'page':page,folderid:param.folderid,chapterid:param.chapterid,quetype:param.quetype,topchapterid:param.topchapterid,secchapterid:param.secchapterid},function(data){
    ...
    $('.singleContainerFocused:last').css('border-bottom','none');
		},'json');
	}
	$(function(){
		Xques.doshow("<?=$crid?>");
