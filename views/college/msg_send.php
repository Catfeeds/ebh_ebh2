...
<div class="lefrig" style="background:#fff;float:left;width:1000px;">
	<div class="work_mes" style="float:none; margin-bottom:15px;width:1000px;padding-top:10px;">
		<ul>
			<li><a href="<?= geturl('college/msg/sendlist') ?>"><span>私信列表</span></a></li>
			<li class="workcurrent"><a href="<?= geturl('college/msg/send') ?>"><span>写私信</span></a></li>
		</ul>
	</div>

<div class="waiyry clearfix" >
<div class="chouad" style="height:auto;width:890px;">
...
<textarea class="txttiantl" name="summary" style="position:relative; *left:-45px;width:890px;"></textarea>
<div class="wtkkr" style="height:45px;width:890px;">
内容不超过500字
		...
		$teacher['showname'] = empty($teacher['realname']) ? $teacher['username'] : $teacher['realname'];
			?>
				<li class="etklys">
					...
					<a class="atfwt" tid="<?=$teacher['uid']?>" tname="<?=$teacher['showname']?>"><?=shortstr($teacher['showname'],6,'')?></a>
				</li>
			<?php }?>
			</ul>
		</div>
		...
		$.ajax({
		    ...
		    success:function(res){
		        if(res=="1"){
					...
					window.location.href = '<?=geturl('college/msg/sendlist')?>';
				}else{
...
function getStudents(cid){
	...
	$.ajax({
		type: "POST",
		url: "<?=geturl('college/msg/getStudents')?>",
		data: {"classid":cid},
		dataType:"json",
