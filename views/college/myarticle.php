					...
					<li class="wenlist">
                        ...
                        <div class="flstrey">
								...
								<div class="problemdetail">
									<a href="/college/myarticle/articleDetail/<?=$value['itemid']?>.html" target="_blank"><?=$value['message']?></a>
								</div>
                            
								 <div class="questinformation-1">
									 <span class="questiontime-1"><?=timetostr($value['dateline'])?></span>
									<img class="questionerhead" src="<?=empty($value['face'])?getavater($value):$value['face']?>" title="<?=$value['username']?>">
                            ...
                            <div class="answernumber">
                                ...
                                <?php if ($value['uid'] == $uid) {?>
								    <a href="javascript:;" class="dershan" onclick="del_comment(<?=$value['itemid']?>,this);" >删除</a>
                                <?php }?>
                            </div>
                    </li>
                    <?php }}?>
...
<script type="text/javascript">
    ...
    function del_comment(itemid){
        var d = dialog({
            title: '删除评论',
            content: '您确定要删除吗？删除后不可查看!',
            okValue: '确定',
            ok: function () {
                var url = "<?= geturl('college/myarticle/updateArticle')?>";
                $.ajax({
                    url:url,
                    type:'post',
                    ...
                    success:function(result){
                        if(result.code == '1'){
                            ...
                            cancel: false,
                                okValue: '确定',
                                ok: function () {
                                    var url = '<?= geturl("college/myarticle/{$method}") ?>';
                                    document.location.href = url;
                                }
                            }).showModal();
                            setTimeout(function(){
