...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd style="width:681px;">
		<div class="category_cont1">
			<div>
				<a <?= empty($cid) ? 'class="curr"' : ''?> href="<?= geturl('troomv2/classstudent') ?>">所有学生</a>
			</div>
			
                        <?php foreach ($classlist as $myclass) { ?>
			<div>
				<a <?= $cid == $myclass['classid'] ? 'class="curr"':'' ?> href="<?= geturl('troomv2/classstudent-0-0-0-'.$myclass['classid']) ?>"><?= $myclass['classname'] ?></a>
			</div>
                        <?php } ?>
...
$(function(){
	...
	$('#searchbutton').click(function(){
		...
		var href = '<?= geturl('troomv2/classstudent-0-0-0-'.$cid)?>';
                href += '?q=' + searchvalue;
		location.href = href;
	});
	...
	function savepass()
	{
		...
		$.ajax({
			type:'post',
			url:'<?=geturl('troomv2/classstudent/editpass')?>',
			dataType:'text',
