...
<div class="ter_tit">
	当前位置 &gt; <a href="<?=geturl('troomv2/examapply')?>">认证管理</a> &gt; <a href="<?=geturl('troomv2/examapply')?>">成绩统计</a> &gt; 已发证
        <div class="diles fr">
    ...
    <div class="workol">
        <div class="work_mes">
            ...
            <ul class="extendul">
                ...
                <li><a href="/troomv2/examapply/resultfail/<?=$eid?>.html"><span>未通过</span></a></li>
                <li class="workcurrent"><a href="/troomv2/examapply/resultaward/<?=$eid?>.html"><span>已发证</span></a></li>
            </ul>
        </div>
        <div class="clear"></div>
        <div>
...
<?php if(!empty($answers)) {
	foreach($answers as $answer) {?>
                    <tr >
                        ...
                        <td><a href="http://exam.ebanhui.com/etmark/<?=$answer['aid']?>.html" class="caoz" target="_blank">查看试卷</a>&nbsp;&nbsp;</td>
                    </tr>
<?php }
} else { ?>
...
function _search(){
	...
	if(searchkey == '请输入学生姓名或登录帐号')
		searchkey = '';
	location.href = '<?=geturl('troomv2/examapply/resultaward/'.$eid)?>?q='+searchkey;
}
function _examexcel(){
	var startdate = $('#startdate').val();
	var enddate = $('#enddate').val();
	location.href = '/troomv2/examapply/examexcel/<?=$eid?>.html?type=3&startdate='+startdate+'&enddate='+enddate;
}
</script>
