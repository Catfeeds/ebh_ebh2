...
<div class="ter_tit" style="width:778px;display:none;">当前位置 >
			...
			?>
			<a href="<?=geturl('troomv2/linkcourse/'.$uparr[$index]['folderid'].'-0-0-0-'.$from)?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>	
			<?= $folder['foldername'] ?>
...
$(function() {
   initsearch("title",searchtext);
		$("#room_search_btn").click(function(){
			...
			var url = "<?= geturl('troomv2/linkcourse/'.$folder['folderid'].'-0-0-0-'.$from)?>?q="+search;
			location.href=url;
		});
	});
</script>
...
<div class="kejian">
	...
	<div class="showimg"><a href="<?=geturl('troomv2/linkcourse/'.$subfolder['folderid'].'-0-0-0-'.$from)?>" title="<?=$subfolder['foldername']?>"><img src="<?= empty($subfolder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subfolder['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('troomv2/linkcourse/'.$subfolder['folderid'].'-0-0-0-'.$from) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
...
<?php foreach($sectionlist as $section) { ?>
	...
	<a class="sa" href="javascript:void(0)" onclick="doLink(<?=$course['cwid']?>)" <?= $color ?>><?= $course['title'] ?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a><em class="yema" <?= $color ?>><?= $i++ ?></em></li>
	<?php } ?>
...
<?= $pagestr ?>
<script>
	function doLink(cwid){
		var link_eid = top.link_eid;
		var url = '/troomv2/linkcourse/dolink.html';
		$.ajax({
		    type:'POST',
		    url: url ,
		    data: {cwid:cwid,eid:link_eid},
		    success: function(result){
		    	// $.showmessage({
						// message:result.info,
						// callback :function(){
                           top.closeLinkDialog(result.info);
                        // }});
			},
		    dataType: 'json'
		});
