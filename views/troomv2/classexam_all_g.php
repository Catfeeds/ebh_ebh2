...
<div class="lefrig">
    ...
    <!--    </div>-->
    <p class="kishre" style="padding-left: 300px"><span class="ksrdgae">布置时间：<?php echo date('Y-m-d H:i',$myexam['dateline'])?></span><span class="ksrdgae">总分：<?php echo $myexam['score']?></span><span class="ksrdgae">计时：<?php echo $myexam['limitedtime']?>分钟</span></p>
    <div class="kishre" style="padding-left: 300px">
        ...
        <div class="fl" style="width:605px;">
            <a href="<?= geturl('troomv2/classexam/all_g-0-0-0-0-'.$myexam['eid']) ?>" class="bjchose <?php if(empty($classid)) echo 'cuur'?>"><?php echo $myexam['gradename']?>年级</a>
            <?php foreach($classes as $class){?>
            <a href="<?= geturl('troomv2/classexam/all_g-0-0-0-'.$class['classid'].'-'.$myexam['eid']) ?>" class="bjchose <?php if($class['classid']==$classid) echo 'cuur'?>"><?php echo $class['classname']?></a>
            <?php }?>
        </div>
    </div>
    <!--    <div class="dsiters">-->
    ...
    <div class="lishnrt">
        <a href="/troomv2/classexam/all_g-0-0-0-<?php echo $myclass['classid']?>-<?php echo $myexam['eid']?>.html" class="hietse <?php echo empty($status)?'xhusre':''?>">1111全部</a>
        <a href='/troomv2/classexam/all_g-0-0-0-<?php echo $myclass['classid']?>-<?php echo $myexam['eid']?>.html?status=1'class="hietse <?php echo $status == 1?'xhusre':''?>">已交作业</a>
        <a href='/troomv2/classexam/all_g-0-0-0-<?php echo $myclass['classid']?>-<?php echo $myexam['eid']?>.html?status=2' class="hietse <?php echo $status==2?'xhusre':''?>">未交作业</a>
        <a class="jaddre" href="/troomv2/statisticanalysis/scexcel.html?eid=<?=$myexam['eid']?>" style="right:40px;top:-38px;"> 导出excel</a>
<!--        <a class="jaddre" href="/troomv2/statisticanalysis/scexcel.html?eid=--><?//=$myexam['eid']?><!----><?php //if(!empty($myclass['classid'])){ echo '&classid='.$myclass['classid'];}elseif(!empty($myexam['grade'])&&empty($myclass['classid'])){ echo '&grade='.$myexam['grade'];}?><!--" style="right:40px;top:-38px;"> 导出excel</a>-->
    </div>
    ...
    <div class="workdata" style="float:left;margin-top:0;">
        <table class="datatab" width="100%" cellspacing="0" cellpadding="0" style="border:none;">
            ...
            <?php if(!empty($answers)){ foreach($answers as $v){?>
                <tr class="">
                    <td style="word-break: break-all; word-wrap:break-word;">
                        ...
                        ?>
                        <a title="<?=$v['realname']?>" href="javascript:;" style="float:left;">
                        ...
                        <p class="ghjut" style="width:160px;color:#999;"><?php echo $v['username']?></p>
                    </td>
                    <td>
                        <p><?= (empty($v['dateline'])|| $v['status']==0)?'--':(date('Y-m-d H:i',$v['dateline'])) ?></p>
                        <p style="color:#999;" title="<?=$v['remark']?>"><?= shortstr($v['remark'],36) ?></p>
                    </td>
                    <td style="text-align: center;"><?= (!empty($v['aid']) && $v['status']!=0)?ceil($v['completetime']/60).' 分钟':'--'?></td>
                    <td style="text-align: center;"><?php echo $v['totalscore']?$v['totalscore'].'分':'--'?></td>
                    <td>
                        <?php if(!empty($v['status'])&&!empty($v['aid'])){?>
                            <a class="waskes" href="http://exam.ebanhui.com/etmark/<?=$v['aid']?>.html" target="_blank">批阅</a>
...
<script type="text/javascript">
    ...
    $(".completetime").click(function(){
        if(order=="1") {
            location.href='<?= geturl('troomv2/classexam/all_g-0-2-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>?status=<?=$status?>';
        }else{
            location.href='<?= geturl('troomv2/classexam/all_g-0-1-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>?status=<?=$status?>';
        }
    });
    $(".totalscore").click(function(){
        if(order=="3") {
            location.href='<?= geturl('troomv2/classexam/all_g-0-4-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>?status=<?=$status?>';
        }else{
            location.href='<?= geturl('troomv2/classexam/all_g-0-3-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>?status=<?=$status?>';
        }
    });
    <?php }?>
