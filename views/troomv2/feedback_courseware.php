 <?php $this->display('troomv2/page_header'); ?>
			<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/feedback/courses') ?>" >听课反馈</a> > 
			...
			?>
			<a href="<?=geturl('troomv2/feedback/'.$uparr[$index]['folderid'])?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>	
			<?= $folder['foldername'] ?>
			<div class="diles">
	<?php
...
$(function() {
   initsearch("title",searchtext);
		$("#room_search_btn").click(function(){
			...
			var url = "<?= geturl('troomv2/feedback/'.$folder['folderid'].'-0-0-0-')?>?q="+search;
			location.href=url;
		});
	});
</script>
...
<div class="kejian">
	...
	<li class="danke" >
	<div class="showimg"><a href="<?=geturl('troomv2/feedback/'.$subfolder['folderid'])?>" title="<?=$subfolder['foldername']?>"><img src="<?= empty($subfolder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subfolder['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('troomv2/feedback/'.$subfolder['folderid']) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
...
<?php foreach($sectionlist as $section) { ?>
	...
	<a class="sa" target="<?= ($type == 'flv') ? '_blank' : '' ?>" href="<?= geturl('troomv2/feedback/courses/'.$course['cwid'])?>" <?= $color ?>><?= $course['title'] ?><span>…………………………………………………………………………………………………………………………………………………………………………</span></a>
	<?php }else{?>
