...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/student') ?>">学员管理</a> > 添加学员
</div>
<div class="lefrig">
...
function submit_add() {
    if(add_student('add')) {    //验证
        var url = "<?= geturl('troomv2/student/add') ?>";
	$.ajax({
            type:"post",
            url:url,
            dataType:'json',
            ...
            success:function(data){
                if(data != null && data != undefined && data.status == 1) {
                         $.showmessage({
                            ...
                            title:'添加学员',
                            callback :function(){
                                 document.location.href = "<?= geturl('troomv2/student') ?>";
                            }
                        });
                    } else {
                        $.showmessage({
...
function checksname(_this){
	...
	if(_value == ''){
		...
		_bsname = false;
            ...
            var url = "<?= geturl('troomv2/student/checkname') ?>";
		$.ajax({
			type:"post",
			url:url,
			dataType:'json',
