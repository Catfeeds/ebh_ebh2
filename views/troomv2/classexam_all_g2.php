...
<div class="lefrig">
    ...
    <div class="kishre" style="padding-left: 300px">
        ...
        <div class="fl" style="width:605px;">
            ...
            <?php foreach($classes as $class){?>
                <a href="<?= geturl('troomv2/classexam/all_g-0-0-0-'.$class['classid'].'-'.$myexam['eid']) ?>" class="bjchose <?php if(!empty($classid)){if($class['classid']==$classid) echo 'cuur';}?>"><?php echo $class['classname']?></a>
            <?php }?>
        </div>
    </div>
    <!--    <div class="dsiters">-->
    ...
    <div class="lishnrt">
        <a href="/troomv2/classexam/all_g2-0-0-0-0-<?php echo $myexam['eid']?>.html" class="hietse <?php echo empty($status)?'xhusre':''?>">全部</a>
        <a href='/troomv2/classexam/all_g2-0-0-0-0-<?php echo $myexam['eid']?>.html?status=1'class="hietse <?php echo $status == 1?'xhusre':''?>">已交作业</a>
        <a href='/troomv2/classexam/all_g2-0-0-0-0-<?php echo $myexam['eid']?>.html?status=2' class="hietse <?php echo $status==2?'xhusre':''?>">未交作业</a>
        ...
        <table class="datatab" width="100%" cellspacing="0" cellpadding="0" style="border:none;">
            ...
            <?php if(!empty($list)){ foreach($list as $v){?>
                <tr class="">
                    ...
                    <td style="text-align: center;"><?php echo !empty($v['totalscore'])?$v['totalscore'].'分':'--'?></td>
                    <td>
...
<script type="text/javascript">
    ...
    $(".completetime").click(function(){
        if(order=="1") {
            location.href='<?= geturl('troomv2/classexam/all_g2-0-2-0-0-'.$myexam['eid']) ?>?status=<?=$status?>';
        }else{
            location.href='<?= geturl('troomv2/classexam/all_g2-0-1-0-0-'.$myexam['eid']) ?>?status=<?=$status?>';
        }
    });
    $(".totalscore").click(function(){
        if(order=="3") {
            location.href='<?= geturl('troomv2/classexam/all_g2-0-4-0-0-'.$myexam['eid']) ?>?status=<?=$status?>';
        }else{
            location.href='<?= geturl('troomv2/classexam/all_g2-0-3-0-0-'.$myexam['eid']) ?>?status=<?=$status?>';
        }
    });
    <?php }?>
