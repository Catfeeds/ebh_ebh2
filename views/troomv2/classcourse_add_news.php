<?php $this->display('troomv2/page_header'); ?>
	...
	<div class="lefrig" style="min-height:1300px;">
		<?php $this->assign("folderid",$folder['folderid']);?>
		<?php $this->display('troomv2/course_menu'); ?>
		<?php if($iszjdlr){?>
		<div class="zjdlr-workmes">
			<ul>
				<a href="/troomv2/classcourse/add-0-0-0-<?php echo $folderid;?>-<?php echo $cwtype;?>.html?toid=0">普通课件</a>
				<a href="/troomv2/classcourse/add-0-0-0-<?php echo $folderid;?>-<?php echo $cwtype;?>.html?toid=1" class="zjdlrcurr">图文课件</a>
			</ul>
		</div>
		<?php }else{?>
					...
					<tr>
						<td colspan="2">
							<div style="width:100%;padding-left:30px;margin-top:13px;">
								<?php $editor->xEditor('message','725px','300px'); ?>
							</div>
						</td>
					</tr>
				<?php } ?>
...
<?php if($cwtype == 'live') { ?>
	<tr>
		...
		<td valign="middle" style="height:40px;line-height:40px;"><select type="text"  maxlength="100" name="cwlength" value="" >
	...
	<tr>
		...
		<td><ul><li class="lantewu" id="assistant" style="background-color:#fff;"><span class="languan" style="display:none;">&nbsp;</span><span class="la">添加</span><input type="hidden" name="assistantid" /></li></ul></td>
	</tr>
<?php }} ?>
...
<tr>
	...
	<td>
		<label style=" "><input type="radio" name="open_chatroom" id="noopen_chatroom" value="0">不开启</label> <label style=" "><input type="radio" id="open_chatroom" name="open_chatroom" value="1"  checked="checked">开启</label><em style="color: #ADADAD"></em>
	</td>
</tr>
<?php } ?>
<?php if($iszjdlr){?>
...
<?php }?>
	...
	<script type="text/javascript">
		...
		function course_submit2(cwtype) {
				...
				<?php }?>
				$.ajax({
					url:'/troomv2/classcourse/add.html?toid=1',
					type: "POST",
					...
					success: function(data){
						if(data != null && data != undefined && data.status == 1) {
							...
							dialog({
							skin:"ui-dialog2-tip",
							content:"<div class='TPic'></div><p>发布失败，请稍后再试或联系管理员。</p>",
							width:350,
							onshow:function () {
								var that=this;
								setTimeout(function() {
		...
		var updatesection = function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					objhtml+='<td>'
					objhtml+='<span class="SG_txtc" style="margin-left:5px;width:290px;display:block;color:#999; text-align:left;">请用中文,英文或数字.1-50个字符!</span>'
					objhtml+='</td>'
					objhtml+='</tr>'
					objhtml+='</tbody>'
					objhtml+='</table>'
