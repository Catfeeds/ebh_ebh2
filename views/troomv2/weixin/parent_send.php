...
<form id="replyform" acion="<?= geturl('troomv2/weixin/do_reply') ?>" method="post" name="student_send">
<div class="lefrig">
<div class="work_mes">
	<ul>
		<li>
...
<div class="euwqie" style="float:left;">
...
function getvalue(ob)
{
	...
	location.href='<?= geturl('troomv2/weixin/parent_send') ?>?classid='+opval;
}
$(function () {
...
function submitreply() {
	...
	$("#content").val(content);
	var url = "<?= geturl('troomv2/weixin/do_reply') ?>";
	$.ajax({
		url:url,
		type: "POST",
...
function submitdelreply() {
	var url = "<?= geturl('troomv2/weixin/do_delreply') ?>";
	var ridlist = [];
	$(".usercheck").each(function(){
...
function submitclearreply() {
	var url = "<?= geturl('troomv2/weixin/do_clearreply') ?>";
	$.ajax({
		url:url,
		type: "POST",
		data:$("#replyform").serialize(),
