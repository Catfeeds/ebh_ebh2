...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/classpaper') ?>" >在线考试</a> > <a href="<?= geturl('troomv2/classpaper/my-0-0-0-'.$myclass['classid']) ?>"><?= $myclass['classname'] ?></a> > 试卷批阅
	<a href="/troomv2/classpaper/examexcel-0-0-0-<?= $myclass['classid'];?>-<?= $myexam['eid'];?>.html" class="addbtnas previewBtn">导 出</a>
...
<div class="lefrig">
		...
		<form action="<?= geturl('troomv2/classpaper/correct') ?>" method="post" id="form" >
...
$(function(){
	...
	$(".replyBtn").click(function(){
		...
		$.post('<?= geturl('troomv2/classpaper/correct') ?>',formdata,function(json){
			if(json.code){
	...
	$("#aidarr").attr('value',aidArr.join(','));
		...
		<script type="text/javascript">
			...
			$(".completetime").click(function(){	
				if(order=="1") {
					location.href='<?= geturl('troomv2/classpaper/all-0-2-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}else{
					location.href='<?= geturl('troomv2/classpaper/all-0-1-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}
			});
			$(".totalscore").click(function(){
				if(order=="3") {
					location.href='<?= geturl('troomv2/classpaper/all-0-4-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}else{
					location.href='<?= geturl('troomv2/classpaper/all-0-3-0-'.$myclass['classid'].'-'.$myexam['eid']) ?>';
				}
			});
