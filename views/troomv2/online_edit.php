...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/online')  ?>">在线直播</a> > 编辑直播
</div>
<div class="lefrig" style="background:#fff;float:left;margin-top:15px;width:788px;">
<div class="work_mes">
	<ul>
		<li <?php if($pos=='my'){echo 'class=workcurrent';}?>><a href="<?= geturl('troomv2/online/my')  ?>"><span>我的直播</span></a></li>
		<li <?php if($pos=='all'){echo 'class=workcurrent';}?>><a href="<?= geturl('troomv2/online/allcourse') ?>"><span>所有直播</span></a></li>
...
<form id="formdata">
	...
	<table class="addteacher_tab" width="100%" border="0" cellspacing="0" cellpadding="0" style="border:none;width:786px;">
	  <tr>
	  ...
	  <tr>
		...
		<td style="padding-top:14px">
            	<div id="xuanban" class="xuanban" style="background:#fff;">
					<h2 class="rbxht">
						<span style="float:left" id="gradetip" class="gradetip">请选择直播的<?=$flag==0?'班':'年'?>级</span>
						<?php if(!empty($grades)){?>
							<a href="javascript:" class="bluebtn gradebtn" style="float:left;margin-left:200px;">按<?=$flag==1?'班':'年'?>级直播</a> 
						<?php }?> 
					</h2>
...
<script>
	...
	function beforesubmit(){
		...
		$.post("/troomv2/online/edit.html",{param:$("#formdata").serialize()},function(message){
				if(message=="1"){
					$.showmessage({
						message:'编辑成功！',
						callback :function(){
