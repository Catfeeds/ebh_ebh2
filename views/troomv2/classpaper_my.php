...
<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troomv2/classpaper') ?>">在线考试</a> > <a href="<?= geturl('troomv2/classpaper/cor') ?>">批改试卷</a> > <?= $myclass['classname'] ?>
</div>
			   ...
			   <tr>
				<td>
					...
					<div style="float:left;width:520px;">
						...
						<span style="width:100px;float:left;"><?= $exam['score'] ?>分</span>
					...
					<div style="float:left;width:238px;">
						...
						<?php if($exam['status'] == 0) { ?>
							...
							<?php }?>
						<?php } else { ?>
						<a class="previewBtn" href="<?= geturl('troomv2/classpaper/all-0-0-0-'.$myclass['classid'].'-'.$exam['eid']) ?>" >作业批阅</a>
						<?php } ?>
						<?php if($exam['uid'] == $uid) { ?>
							<a class="previewBtn" href="javascript:void(0)" onclick="changetype(<?= $exam['eid'] ?>)">转成作业</a>
						<?php }?>
					</div>
				</td>	
			  </tr>
...
function delexam(eid,crid) {
	$.confirm("试卷删除后，此试卷下的学生答题记录也会删除，确定要删除吗？",function(){
		var url = '<?= geturl('troomv2/classpaper/del') ?>';
		$.ajax({
			url:url,
			type:'post',
			data:{'eid':eid},
...
function changetype(eid){
	$.ajax({
			url:"<?=geturl('troomv2/classexam/changeType')?>",
			type:'post',
			data:{'eid':eid},
			dataType:'json',
