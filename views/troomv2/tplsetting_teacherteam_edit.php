...
<div class="ter_tit">
当前位置 > <a href="/troomv2/tplsetting/teacherteam_add.html">师资团队</a> > 添加教师</div>
<div class="lefrig">
...
<div id='atsrc' style="display: none;"></div>
				...
				<br/>      
		<form id="upform">
			<input type="hidden" name="itemid" value="<?=$teacher['itemid']?>" />
			...
			<table class="addteacher_tab" width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				...
				<td colspan="3">
					<input style="padding-left: 5px;" class="uipt w295" maxlength="25" name="subject" id="subject" type="text" value="<?=$teacher['subject']?>"  maxlength="30" onblur="subjs(this.value)">
...
<script type="text/javascript" src="http://static.ebanhui.com/ebh/js/jquery/jquery-lightbox-0.5/jquery.lightbox-0.5.min.js"></script>
					<a class='sn' id='a' style="display:block" href='<?=$teacher['thumb']?>'>
						<img id="imageid" class="imagewidth" style="height:134px;width:94px;" src="<?=$teacher['thumb']?>" >
					</a><br/>
					<script>$('#a').lightBox();</script>
					<?php }?>
					<?php
					  if(empty($teacher['thumb'])){
					    $Upcontrol->upcontrol('thumb',1,null,'thteam');
					  }else{
					    $Upcontrol->upcontrol('thumb',1,array('upfilepath'=>$teacher['thumb']),'thteam');
					  }
					?>
			  ...
			  <tr>
				...
				<td colspan="3">
					<textarea style="padding-left: 5px;" name="note" id="note" cols="80" rows="5" value="" onblur="nots(this.value)"><?=$teacher['note']?></textarea>
...
$("#savebtn").click(function(){
      if(submit_check()) {
        var url="<?= geturl('troomv2/tplsetting/teacherteam/edit')?>";
        $.ajax({
          url:url,
          type: "POST",
          ...
          success:function(data){
            if(data == 'success') {
              $.showmessage({
                ...
                title:'编辑师资团队',
                callback :function(){
                   document.location.href = "<?= geturl('troomv2/tplsetting/teacherteam') ?>";
                }
              });
            } else {
              $.showmessage({
                img : 'error',
                message:'编辑师资团队失败，请稍后再试或联系管理员<br />'+data,
                title:'编辑师资团队'
              });
            }
          }
