...
<div class="ter_tit">
当前位置 > <a href="<?=geturl('troomv2/teachingplan')?>">电子教案</a> > 教案管理
	<div class="diles">
...
<div class="lefrig" style="background:#fff;float:left;margin-top:15px;width:786px;">
	...
	<ul>
		<?php if(empty($tplanlist)){
			if(empty($search)){?>
			您还没有教案，去创建吧。<a style="color:red" href="<?=geturl('troomv2/teachingplan/add')?>">点击这里</a>
			<?php }else{?>
			...
			<li class="quito">
			<span style="float:left;width:424px;word-wrap: break-word;"><a href="<?=geturl('troomv2/teachingplan/'.$tl['tpid'])?>"><?=$tl['title']?></a></span><span style="float:left;width:185px;"><?=Date('Y-m-d H:i:s',$tl['dateline'])?></span><a href="<?=geturl('troomv2/teachingplan/edit/'.$tl['tpid'])?>" class="workBtn">编辑</a><a href="javascript:tplandelete(<?=$tl['tpid']?>)" class="workBtn">删除</a><a href="javascript:show_div(<?=$tl['tpid']?>)" class="previewBtn">上传附件</a>
			
			<?php if(!empty($tl['attlist'])){
				foreach($tl['attlist'] as $al){
				$url = geturl('troomv2/teachingplan/downloadatt').'?attid='.$al['attid'].'&tpid='.$tl['tpid'];
				?>
...
function tplandelete(tpid){
	...
	$.confirm("您确定要删除您的教案？这将同时删除您的附件。",function(){
		...
		$.ajax({
			url:"<?=geturl('troomv2/teachingplan/deletetplan')?>",
			type:"post",
			data:{"tpid":tpid},
			dataType:"text",
...
function search(){
	...
	if(key == $.trim("请输教案名称"))
	key="";
	var url = '<?=geturl('troomv2/teachingplan/manage-0-0-0')?>';
	url = url+'?q='+key;
...
{
    var objhtml="";
    objhtml = " <iframe id='iframeid' scrolling='no' width='400' height='168' frameborder=no src='<?=geturl('troomv2/teachingplan/uploadtplanattach')?>'></iframe>";
	$("#dialog").html(objhtml);
	H.create(new P({
		id:"dialog",
		title:'上传文件',
...
function attachdelete(tpid,attid)
{
	$.confirm("您确定要删除这个附件？",function(){
		...
		$.ajax({
			url:"<?=geturl('troomv2/teachingplan/deleteattach')?>",
			type:"post",
...
function attachupload(returns)
{
	...
	$.ajax({
		url:"<?=geturl('troomv2/teachingplan/uploadtplanattach')?>",
		type:"post",
...
{
	$.ajax({
		url:"<?=geturl('troomv2/teachingplan')?>",
		type:"post",
