...
$v = getv();?>
	...
	<link rel="stylesheet" type="text/css" href="http://static.ebanhui.com/ebh/js/webuploader/webuploader.css<?=$v?>" />
	...
	<div class="lefrig">
		<?php $this->assign("folderid",$folder['folderid']);?>
		<?php $this->display('troomv2/course_menu'); ?>
		<?php if($iszjdlr){?>
		<div class="zjdlr-workmes">
			<ul>
				<a href="/troomv2/classcourse/add-0-0-0-<?php echo $folderid;?>-<?php echo $cwtype;?>.html?toid=0" class="zjdlrcurr">普通课件</a>
				<a href="/troomv2/classcourse/add-0-0-0-<?php echo $folderid;?>-<?php echo $cwtype;?>.html?toid=1">图文课件</a>
			</ul>
		</div>
		<?php }else{?>
		<h2 class="mkysetit"><?=($cwtype=='live')?"发布直播课":"发布视频课"?></h2>
		<?php }?>
		...
		<form id="courseform" name="form">
			...
			<table class="user_config_tab" width="100%">
				<tr>
					<th>&nbsp;</th>
					<td>
						<input class="uipt w340 bgerg" id="title" maxlength="40" type="text" onblur="teacher(this,'课件标题','title','','n','','','控制在1-40个字之间',40)" name="title" value="" />
				...
				<tr id ="dnone">
					<th>&nbsp;</th>
					<td>
						<select style="width:470px;" name="sectionid" id="sectionid" onblur="teacher(this,'所属目录','sectionid','','n','','','所属目录不能为空')">
				...
				<tr>
					<th>&nbsp;</th>
					<td>
						<input class="uipt w340 bgerg" id="tag" type="text" name="tag" value="" onblur="teacher(this,'所属标签','tag','','n','','','多个标签之间请用逗号隔开')" />
					...
					<tr>
						<td colspan="2">
							<div style="width:100%;padding-left:30px;margin-top:13px;">
								<?php $editor->xEditor('message','940px','600px'); ?>
							</div>
						</td>
					</tr>
				<?php } ?>
			...
			<table class="user_config_tab" width="100%" style="margin-top:10px;">
				<?php if($cwtype != 'live') {?>
					<tr>
						...
						<td>
							...
							Ebh::app()->lib('Webuploader')->renderHtml('up', false, array(), array('fileSizeLimit' => $limitB)); ?>
							<span class="ts2" style="padding-left:0;float:left;">请选择要上传的文件，文件大小不超过<?=$limitSize?>G。</span>
						</td>
					</tr>
				<?php } ?>
                <?php if (!empty($isnewzjdlr) && $cwtype!='live') { ?>
                    <tr>
                        ...
                        <td>
                            <label style="margin-right:10px;"><input name="delaytime" id="noopen_chatroom" value="<?=isset($delaytime) ? $delaytime : '0'?>" style="text-align: center; width: 35px; margin-right: 6px; height: 20px;" type="text">秒</label>
                        </td>
                    </tr>
                <?php } ?>
				<!-- 上传封面逻辑开始 -->
				<?php if(!empty($folder['coursewarelogo']) || !empty($roominfo['iscollege']) ){?>
					<tr style="display:none">
						...
						<td>
							...
							<input  id="logo"  type="hidden" value="<?=!empty($course['logo'])?$course['logo']:""?>" name="logo" />
						</td>
					</tr>
					<!-- 上传封面逻辑结束 -->
					<!-- 上传课程封面 -->
					<tr>
						...
						<td>
							<a title="点我上传课件封面" style="display:block;" href="javascript:void(0)"  onclick="uploadlogo()" class="jnlihrey" <?=$style?>><img style="float:left;" id="showclog" width=178px height=103px src="http://static.ebanhui.com/ebh/tpl/default/images/lstyjast.jpg"/></a>
				...
				<tr>
					...
					<td>
						<div style="margin-top:11px;">
							<div style="float:left; display:inline;"><input type="text" id="submitat" name="submitat" class="readonly" readonly="readonly" style="text-indent:15px;height:22px;line-height:22px;border:1px solid #B3DDF4;margin:5px;display:inline;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-%d %H:%m:%s'});"/>&nbsp;&nbsp;&nbsp;&nbsp;</div>
							<?php if($cwtype != 'live') { ?>
							<div style="float:left; display:inline;"><span style="display:block;height:33px; line-height:33px; float:left;">截止时间：</span><input type="text" id="endat" name="endat" class="readonly" readonly="readonly" style="text-indent:15px;height:22px;line-height:22px;border:1px solid #B3DDF4;margin:5px;display:inline;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'submitat\',{H:1})||\'%y-%M-%d {%H+1}:%m:%s\'}'});"/></div>
						</div>
						<?php } ?>
	...
	<?php if($cwtype == 'live') { ?>
					<tr>
						...
						<td class="address">
							<label style="margin-right:10px;"><input type="radio" name="live_type" id="software_live" value="1" checked="checked" style="margin-right:3px;">软件直播</label>
							<label style="margin-right:10px;"><input type="radio" id="phone_live" name="live_type" value="2" style="margin-right:3px;">手机直播</label>
							<label style="margin-right:10px;"><input type="radio" id="equipment_live" name="live_type" value="3" style="margin-right:3px;">设备直播</label>
							<label><input type="radio" id="pseudo_live" name="live_type" value="4" style="margin-right:3px;">伪直播</label>

							<div class="live_address" style="overflow:hidden;margin-top: 20px;display:none;">
								<div class="address_show">
									<span class="address_label">推流地址：</span>
									<input type="text" class="address_url" style="width:440px;height:30px;border:1px solid #ccc;padding:0 10px;" readonly="readonly"/>
								</div>
								<p style="margin-left:100px;color:#999;margin-top: 10px;">请将此推流地址复制到硬件设备中</p>
								<div class="address_bottom" style="padding-left:63px;">
									<div class="create_btn address_btn" disable="true">生成地址</div>
					...
					<input type="hidden" name="liveInfo[liveid]" value="">
					<input type="hidden" name="liveInfo[httpPullUrl]" value="">
					<input type="hidden" name="liveInfo[hlsPullUrl]" value="">
					<input type="hidden" name="liveInfo[rtmpPullUrl]" value="">
...
<?php if($cwtype == 'live') { ?>
	...
	<tr class="continuous">
		...
		<td valign="middle" style="height:40px;line-height:40px;">
			<select id="continued" type="text"  maxlength="100" name="cwlength" value="" >
				<option value="10">10分钟</option>
				<option value="20">20分钟</option>
				<option value="30">30分钟</option>
				<option value="40" selected="selected">40分钟</option>
	...
	<tr class="playback">
		<th valign="middle" style="width:90px;padding:0 5px 15px 0;">允许回看：</th>
		 <td class="address" style="height:50px;padding-top:7px;">
		 	<div>
	        	<label style="margin:4px 40px 0 0;"><input type="radio" value="1" id="is_playback" name="review" style="margin-right:3px;" /> 是</label>
	       		<label><input type="radio" value="0" id="no_playback" name="review" style="margin-right:3px;" checked="checked" />  否</label>
	       	</div>
	        <div style="clear: both;color:#999;font-size:12px;">学生在直播结束之后是否能回看直播课</div>
	    </td>
	</tr>
	<tr class="playback_time" style="display:none;">
		<th valign="top" style="width:90px;padding:21px 5px 15px 0;">回看时间：</th>
		<td>
			<div style="margin-top:11px;">
				<div style="float:left; display:inline;">
					<input type="text" id="review_start" name="review_start" class="readonly" readonly="readonly" placeholder="请选择开始时间" style="text-indent:15px;height:22px;line-height:22px;border:1px solid #ccc;margin:5px;display:inline;margin-left: 0;" onclick="var review_end=$dp.$('review_end');WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-%d %H:%m:%s'});"/>
				</div>
				<!--
					WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',onpicked:function(){review_end.click();},minDate:'#F{$dp.$D(\'submitat\',{m:1});}'});
				-->
				<div style="float:left; display:inline;">
					<input type="text" id="review_end" name="review_end" class="readonly" readonly="readonly" placeholder="请选择结束时间" style="text-indent:15px;height:22px;line-height:22px;border:1px solid #ccc;margin:5px;display:inline;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'review_start\',{m:1})}'});"/>
				</div>
			</div>
		</td>	
	</tr>	
...
<tr>
	...
	<td>
		<label style="margin-right:10px;"><input type="radio" name="open_chatroom" id="noopen_chatroom" value="0" style="margin-right:3px;">不开启</label>
		<label style="margin-right:10px;"><input type="radio" id="open_chatroom" name="open_chatroom" value="1"  checked="checked" style="margin-right:3px;">开启</label>
		<label style=" "><input type="radio" id="open_chatroom" name="open_chatroom" value="2" style="margin-right:3px;">仅开启私聊</label>
...
<?php if($cwtype == 'live'){
//由于之前直播没有该选项 数据库中之前的直播默认配置为0 所以直播改为0为开启1为关闭	
?>
<tr>
	<th valign="top">开启群聊：</th>
	<td>
		<label style="margin-right:10px;"><input type="radio" name="open_chatroom" id="noopen_chatroom" value="0" checked="checked" style="margin-right:3px;">开启</label>
		<label style="margin-right:10px;"><input type="radio" id="open_chatroom" name="open_chatroom" value="1"   style="margin-right:3px;">不开启</label>
	</td>
</tr>
<?php } ?>
...
<tr>
	...
	<td>
		<div class="eeret" id="ask-for-students" style="background:url(http://static.ebanhui.com/ebh/images/xuanter.jpg) no-repeat;width:130px;padding-left:8px;height:30px;line-height:30px;">
			<a class="ekiyt" href="javascript:void(0)"><span class="show_studentname" title="<?=empty($student['realname']) ? $student['username'] : $student['realname'];?>"><?=getusername($student,16)?></span></a>
...
<tr>
    ...
    <td valign="top">
        <div class="set-view-power"><input type="radio" value="0" checked="checked" id="view-all" name="view_power" style="margin-right:3px;" /><label for="view-all" style="margin-right:10px;">全部</label>
        <input type="radio" value="1" id="view-grade" name="view_power" style="margin-right:3px;" /><label for="view-grade">指定班级</label></div>
		<div class="grades-box" id="grades-box"><div class="classes-panel">
			<div class="grades" id="choose-grades"></div>
...
<td>
	<div class="ewartre" id="" style="">
		<div class="botmer">
			...
			<p class="huisrt"><span class="huisprt" style="color:#444;">课件价格：</span><input class="husrrt intinput" name="cprice" type="text" id="priceinput" value="" maxlength="5" irequired="true"/>元</p>
			<p class="huisrt">(请输入非负整数)</p>
			<p class="huisrt">
				<label>
				<span class="huisprt" style="color:#444;">课件有效期：<input class="bywhich" style="vertical-align:middle;margin-right:5px;" type="radio" name="bywhich" value="0" checked="checked"/>按月：</span><input class="husrrt intinput" name="cmonth" type="text" id="bym_input" value="" maxlength="3" irequired="true"/><span class="huisprt">个月</span>
				</label>
				<label>
				<span class="huisprt"><input class="bywhich" style="vertical-align:middle;margin-right:5px;margin-left:20px;" type="radio" name="bywhich" value="1" />按日：</span><input class="husrrt intinput" name="cday" type="text" id="byd_input" value="" maxlength="3" readonly="readonly" style="background:#CCC"/>日
				</label>
			</p>
		</div>
	</div>
...
?>
	<script type="text/javascript">
		...
		function course_submit() {
			$(".fakstbttn").attr('disabled',true);

			if(teacher_submit('course',param='cdisplayorder') && checkirequired()) {
		...
		function course_submit2(cwtype) {
				...
				<?php }?>
				$.ajax({
					url:'/troomv2/classcourse/add.html?toid=0',
					type: "POST",
					data:$("#courseform").serialize(),
					dataType:"json",
					success: function(data){
						if(data.msg == "请先生成推流地址"){
							dialog({
							skin:"ui-dialog2-tip",
						...
						if(data != null && data != undefined && data.status ==0 && data.showerrmsg==1){
							dialog({
							skin:"ui-dialog2-tip",
							content:"<div class='FPic'></div><p>"+data.errmsg+"</p>",
							width:350,
							onshow:function () {
								var that=this;
								setTimeout(function() {
							...
							dialog({
							skin:"ui-dialog2-tip",
							content:"<div class='TPic'></div><p>发布课件成功</p>",
							width:350,
							onshow:function () {
								var that=this;
								setTimeout(function() {
							...
							dialog({
							skin:"ui-dialog2-tip",
							content:"<div class='FPic'></div><p>"+(data.msg || "发布失败，请稍后再试或联系管理员。")+"</p>",
							width:350,
							onshow:function () {
								var that=this;
								setTimeout(function() {
					...
					error:function(){
						dialog({
							skin:"ui-dialog2-tip",
							content:"<div class='FPic'></div><p>登录信息可能已失效，请重新登录。</p>",
							width:350,
							onshow:function () {
								var that=this;
								setTimeout(function() {
...
(function($) {
	...
	$('.bywhich').click(function(){
		if($(this).val() == 0){
			$('#byd_input').attr('readonly','readonly').attr('irequired',false).val('').css('background','#CCC');
			$('#bym_input').attr('irequired',true).removeAttr('readonly').css('background','#fff');
		}else{
			$('#bym_input').attr('readonly','readonly').attr('irequired',false).val('').css('background','#CCC');
