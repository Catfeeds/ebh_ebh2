...
<div class="ter_tit"> 当前位置 > <a href="<?= geturl('troomv2/weixin') ?>">微家校通</a> > 班级发信 </div>
<div class="lefrig">
...
<?php };?>
<form id="student_send" acion="<?= geturl('troomv2/weixin/do_send') ?>" method="post" name="student_send">
...
//var replacepop = 0;
	function getvalue(ob)
	{
		...
		location.href='<?= geturl('troomv2/weixin/student_send_msg') ?>?classid='+opval;
    }
	$(function() {
		...
		$("#class_send_form").click(function () {
			...
			var url = "<?= geturl('troomv2/weixin/do_send') ?>";
			$.ajax({
				url:url,
				type: "POST",
