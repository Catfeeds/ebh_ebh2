...
$(function(){
	...
	$(".locking").click(function(){
		...
		var _type = $(this).attr("stype");
                var url = '<?= geturl('troomv2/teacher/upinfo') ?>';
		$.ajax({
			type:'post',
			url:url,
			dataType:'json',
	...
	$(function(){
		$('#searchbutton').click(function(){
			var href = '<?= geturl('troomv2/teacher') ?>';
...
<div class="annuato">
		...
		<input class="souhuang" id="searchbutton" type="button" name="searchbutton" value="搜索" />
				<div class="tiezitool">
					<a class="hongbtn jiabgbtn" onclick="location.href='<?= geturl('troomv2/teacher/add') ?>'">添加教师</a>
				</div>
		</div>
         ...
         <?php if(!empty($teachers)) { ?>
                <?php foreach($teachers as $teacher) { 
				  ...
				  <tr>
					...
					<td width="6%" <?= $style ?>><div class="avatar"><a href="<?= geturl('troomv2/teacher/'.$teacher['uid']) ?>">
					<img width="28px" height="28px" src="<?= $facethumb ?> " <?= $style ?> border="0" /></a></div></td>
					<td width="15%"><a href="<?= geturl('troomv2/teacher/'.$teacher['uid']) ?>" <?= $style ?>><?= $teacher['username'] ?></a></td>
					...
					<td width="20%"><div class="teac_manage">
						...
						<a class="workBtn" style="margin:0;" onclick="location.href='<?=geturl('troomv2/teacher/edit/'.$teacher['uid'])?>'">修改</a>
