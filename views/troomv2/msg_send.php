...
<div class="lefrig" style="background:#fff;float:left;width:1000px;">
	<div class="work_mes" style="float:none; margin-bottom:15px;">
		<ul>
			<li><a href="<?= geturl('troomv2/msg/sendlist') ?>"><span>私信列表</span></a></li>
			<li class="workcurrent"><a href="<?= geturl('troomv2/msg/send') ?>"><span>写私信</span></a></li>
		</ul>
	</div>
		...
		$.ajax({
		    ...
		    success:function(res){
		        if(res=="1"){
					...
					window.location.href = '<?=geturl('troomv2/msg/sendlist')?>';
				}else{
...
function getStudents(cid){
	...
	$.ajax({
		type: "POST",
		url: "<?=geturl('troomv2/msg/getStudents')?>",
		data: {"classid":cid},
		dataType:"json",
