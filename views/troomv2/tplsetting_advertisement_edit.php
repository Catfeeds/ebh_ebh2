...
<div class="ter_tit">
当前位置 > <a href="/troomv2/tplsetting/advertisement.html">广告管理</a> > 添加广告</div>
<div class="lefrig">
<?php $this->display('troomv2/tplsetting_menu'); ?>
<div id='atsrc' style="display: none;"></div>
<div class="annotate" style="color:#888888; margin-left: 20px;">在此页面中，您可以填写广告标题、上传广告图片、广告分类。<span style="color:red;">您所上传的广告图片会在30分钟后更新。</span></div>
<br/>      
<form id="upform">
...
<td >
<input style="padding-left: 5px;float:left;" class="uipt w295" name="itemurl" id="itemurl" type="text" value="<?=$ad['itemurl']?>"  maxlength="30" >
...
//-->
$(function(){
	$("div.tab_menu ul li:eq(3)").attr('class','workcurrent');
});

$("#savebtn").click(function(){
      if(submit_check()) {
        var url="<?= geturl('troomv2/tplsetting/advertisement/edit')?>";
        $.ajax({
          url:url,
          type: "POST",
          ...
          success:function(data){
            if(data == 'success') {
              $.showmessage({
                ...
                title:'修改广告',
                callback :function(){
                   document.location.href = "<?= geturl('troomv2/tplsetting/advertisement') ?>";
                }
              });
            } else {
              $.showmessage({
