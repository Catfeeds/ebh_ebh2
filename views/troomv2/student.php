...
<script type="text/javascript" src="http://static.ebanhui.com/js/jquery-migrate-1.2.1.min.js"></script>
					<div class="ter_tit">
						当前位置 > <a href="<?= geturl('troomv2/student') ?>">学员管理</a> > 所有学员
					</div>
					<div class="lefrig">
		...
		<div class="work_menuss">
			<ul>
				<li class="workcurrent" style="margin-left:45px;"><a href="<?= geturl('troomv2/student') ?>"><span>所有学员</span></a></li>
				<li ><a href="<?= geturl('troomv2/student/check') ?>"><span>学员审核</span></a></li>
			...
			<table width="100%" class="datatab">
			  ...
			  <?php
                          ...
                          if($student['cstatus'] == 1 && ( $student['enddate']>SYSTIME || empty($student['enddate'])))
                              $style = '';
                          ?>
                            <td width=""><a href="<?= geturl('troomv2/student/'.$student['uid'])?>" <?= $style ?> ><?= $student['username'] ?></a></td>
...
function delcfm(name,memberid) {
    $.confirm("确认要删除此教室下的该学员("+name+")吗？",function(){
        var url="<?= geturl('troomv2/student/upinfo') ?>"
    	$.ajax({
		type:"post",
		url:url,
...
{
	var _url = "<?= geturl('troomv2/student/upinfo')?>";
	if(status == 0){
		status = 1;
	}else{
		status = 0;
...
$(function(){
	...
	$("#newpwd").blur(function(){
		...
		var url = '<?= geturl('troomv2/student')?>';
