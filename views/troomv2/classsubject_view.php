...
<link type="text/css" href="http://static.ebanhui.com/ebh/tpl/default/css/E_ClassRoom.css" rel="stylesheet" />
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troomv2/classsubject') ?>" ?>课程管理</a> > <a href="<?= geturl('troomv2/classsubject/courses') ?>">班级课程</a> > 
			<?php if(!empty($uparr)){
				$upfoldercount = count($uparr);
				foreach($uparr as $k=>$upfolder){
				$index = $upfoldercount-$k-1;
			?>
			<a href="<?=geturl('troomv2/classsubject/'.$uparr[$index]['folderid'])?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>
			<?= $folder['foldername'] ?>
			<div class="diles">
			<?php
...
<div class="tiezitool">
	...
	<a class="mulubgbtn hongbtn" style="margin-right:10px;" href="<?=geturl('troomv2/folder/add/'.$folder['folderid'])?>">添加目录</a>
	<?php }?>
	<?php if(empty($subfolderlist)){?>
	<a class="chuanbgbtn hongbtn marrig" href="<?= geturl('troomv2/classcourse/add-0-0-0-'.$folder['folderid'])?>">上传课件</a>
...
<div class="kejian">
	...
	<span class="spne"><a href="<?= geturl('troomv2/classsubject/'.$subfolder['folderid']) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
...
<?php }?>
				  ...
				  <tr>
					...
					?>
					    		...
					    		<img alt="此课件包含附件"  src="http://static.ebanhui.com/ebh/tpl/default/images/fujian.gif"/>(<?= $course['attachmentnum'] ?>)
                                        <?php } ?>
				    	<a target="<?= (empty($course['cwurl']) || in_array($type,array('flv','mp4','avi','mp3','mpeg','mpg','rmvb','rm','mov'))) ? '_blank' : '' ?>" href="<?= ($type == 'flv') ? geturl('troomv2/course/'.$course['cwid']) : geturl('troomv2/classcourse/'.$course['cwid']) ?>"><?= $course['title'] ?></a>
				    </td>
				    ...
				    <td>
                                                ...
                                                <?php if($course['uid'] == $uid) { ?>
                                                        ...
                                                        <?php if(!empty($course['attachmentnum'])) { ?>
					 			<a class="previewBtn" href="<?= geturl('troomv2/classattachlist-0-0-0-'.$course['cwid'])?>">附件管理</a>
                                                        <?php } else { ?>
					 			<a class="previewBtn" href="<?= geturl('troomv2/classcourse/upattach-0-0-0-'.$course['cwid']) ?>">上传附件</a>
					 		<?php } ?>
	<input type="button" class="previewBtn" style="cursor:pointer;vertical-align: middle;font-weight:100;" onclick="window.open('http://exam.ebanhui.com/enew/<?=$roominfo['crid']?>/0/<?=$folderid?>/<?=$course['cwid']?>.html','_blank')" value="布置作业" />
					    		<input type="button" class="replyBtn" style="cursor:pointer;vertical-align: middle;font-weight:100;" onclick="location.href='<?= geturl('troomv2/classcourse/edit-0-0-0-'.$course['cwid'])?>'" value="修改" />
