...
<div class="lefrig">
    <div class="annotate"> 在此页面中,您可以维护教室的课程信息,包括添加、修改、删除课程.
        <div class="tiezitoolss">
            <a class="hongbtn jiabgbtn marrig" href="<?= geturl('troomv2/subject/add') ?>" >开设课程</a>
            <a class="hongbtn mianbgbtn" href="<?= geturl('troomv2/subject/freecourseware') ?>">免费课件</a>
        </div>
    </div>
    <style type="text/css">
        .kejian {
    ...
    <script type="text/javascript">
        ...
        function delfolder(folderid, fname) {
            $.confirm("确认要删除课程 [" + fname + "] 吗？", function() {
                var url = '<?= geturl('troomv2/subject/delfolder') ?>';
                $.ajax({
                    url: url,
                    type: "post",
                    dataType: "text",
        ...
        function move(folderid, flag) {
            var url = "<?= geturl('troomv2/subject/move') ?>";
            $.ajax({
                url: url,
                type: "post",
                dataType: "text",
            ...
            <ul class="liss">
                ...
                <?php foreach ($folders as $folder) { ?>
                    <li class="danke" style="margin-left:4px; _margin-left:2px;list-style: none;">
                        <div class="showimg"><a href="<?= geturl('troomv2/subject/' . $folder['folderid']) ?>" title="<?= $folder['foldername'] ?>"><img src="<?= empty($folder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $folder['img'] ?>" width="114" height="159" border="0" /></a></div>
                        <span class="spne">
                            ...
                            <a href="<?= geturl('troomv2/subject/edit/' . $folder['folderid']) ?>" style="float:left;text-decoration:none;">[编辑]</a>
                        ...
                        <span class="spne"><a href="<?= geturl('troomv2/subject/' . $folder['folderid']) ?>" style="text-decoration: none;" title="<?= $folder['foldername'] ?>"><?= ssubstrch($folder['foldername'], 0, 20) ?>(<?= $folder['coursewarenum'] ?>)</a></span>
                    </li>
