...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('troomv2/subject') ?>">课程列表</a> > <a href="<?= geturl('troomv2/subject/'.$course['folderid'])?>"><?= $course['foldername']?></a> > 修改附件
</div>
<div class="lefrig">
...
//		}
		...
		function submitform() {
			var url="<?= geturl('troomv2/course/editattach') ?>";
			$.ajax({
				url:url,
				type:"post",
				dataType:"json",
				...
				success:function(data){
					if(data != undefined && data.status == 1) {
						$.showmessage({
							...
							title    :      '附件修改',
							callback :    function(){
								var backurl = "<?= geturl('troomv2/subject/'.$course['folderid'])?>";
