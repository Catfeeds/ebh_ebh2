...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/tools')  ?>">应用工具</a> > <a href="<?= geturl('troomv2/slock')  ?>">学生锁屏</a> > 修改锁屏
</div>
<div class="lefrig" style="background:#fff;float:left;margin-top:15px;">
<div class="work_mes">
	<ul>
		<li><a href="<?= geturl('troomv2/slock')  ?>"><span>我的锁屏</span></a></li>
		<li><a href="<?= geturl('troomv2/slock/add')?>"><span>添加锁屏</span></a></li>
	...
	<table class="addteacher_tab" width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	   ...
	   <tr>
		...
		<td colspan="3">
			<input class="uipt w175" name="cdate" id="cdate" type="text" value="<?=date('Y-m-d H:i',$slock['startdate'])?>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-%d %H:%m:%s'});" onblur="teacher(this,'锁屏开始时间','cdate','','n','','','请输入锁屏开始时间，不能早于当前时间。');checkdate();"/>
		...
		<td colspan="3">
			<?php
			 	$defdurationArr = array(0,10,20,30,40,45,50,60,80,90,100,120,150);
			 	$curduration = ($slock['enddate'] - $slock['startdate'])/60;
			 	$curduration = intval($curduration);
			 	if(!in_array($curduration, $defdurationArr)){
			 		$settmpoption = true;
			 	}
			?>
			<select id="duration" class="uipt w175" type="text"  maxlength="100" name="ctime" value="" >
				<option value="0">请选择</option>
				<?php foreach ($defdurationArr as $duration) {?>
					<option value="<?=$duration?>" <?=($duration==$curduration)?'selected=selected':''?>><?=$duration?>分钟</option>
				<?php }?>

				<?php if(!empty($settmpoption)){?>
					<option id="tmpoption" selected=selected value="<?=$curduration?>"><?=$curduration?>分钟</option>
				<?php }?>
				</select>
	   ...
	   <tr>
		...
		<td colspan="3">
			<input class="uipt w175" name="enddate" id="enddate" type="text" value="<?=date('Y-m-d H:i',$slock['enddate'])?>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'cdate\',{m:5})||\'%y-%M-%d {%H}:{%m+5}:%s\'}'});" onblur="teacher(this,'锁屏结束时间','enddate','','n','','','请输入锁屏开始时间，不能早于当前时间。');checkdate();"/>
            	...
            	<div id="xuanban" class="xuanban" style="background:#fff;">
					...
					<ul class="ulclass" style="<?=($flag==1)?'display:none;':''?>" >
						<?php foreach ($classes as $class) {?>
							<li><input class="goxua"  type="checkbox" <?=!empty($class['checked'])?'checked=checked':''?> name="classes[]" title="<?=$class['classname']?>" value="<?=$class['classid']?>:<?=$class['grade']?>:<?=$class['district']?>"><span  style="cursor:pointer;"><?=$class['classname']?></span></li>
						<?php }?>
					</ul>
					<?php if(!empty($grades)){?>
					<ul class="ulgrade" style="<?=($flag==0)?'display:none;':''?>">
						<?php foreach ($grades as $grade) {?>
							<li><input class="mygrade" type="checkbox" <?=!empty($grade['checked'])?'checked=checked':''?> name="grades[]" title="<?=$grade['gradename']?>" value="<?=$grade['grade']?>:<?=$grade['district']?>"><span  style="cursor:pointer;"><?=$grade['gradename']?></span></li>
						<?php }?>
					</ul>
					<?php }?>
...
<script>
	...
	function beforesubmit(){
		...
		$.post("/troomv2/slock/edit.html",{param:$("#formdata").serialize()},function(res){
				if(res.status == 0){
					$.showmessage({
						message:res.msg,
						callback :function(){
                            document.location.href = "/troomv2/slock.html";
                        }});
				}else{
