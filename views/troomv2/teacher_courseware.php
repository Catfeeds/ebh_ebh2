...
?>
	<?php 
	if($rurl == 'troomv2/teacher/teacherlist'){?>
		当前位置 > <a href="<?=geturl('troomv2/report')?>">统计分析</a> > <a href="<?=geturl('troomv2/teacher/viewnav')?>"> 教师查看 </a> > <a href="<?=geturl('troomv2/teacher/teacherlist')?>"> 教师列表 </a> > 课件查看
	<?php }else{?>
		当前位置 > <a href="/troomv2/statisticanalysis.html">查询统计</a> > <a href="/troomv2/statisticanalysis/teach.html"> 教师统计 </a> > 课件查看
		<?php }?>
	</div>
<div class="coursewareview">
	<table cellpadding="0" cellspacing="0" class="tables">
    	<tr class="second">
        	<td colspan="6">
				...
				<ul style="padding-top:5px;">
					...
					?>
                    <li class="fl"><a href="<?=geturl('troomv2/teacher/courseware/'.$tuser[0]['uid'])?>?rurl=<?=$rurl?>&rrurl=<?=$rrurl?>" class="<?=empty($selfolderid)?'select':''?>">不限</a></li>
					<?php
					foreach($folderlist as $folder){
					?>
                    <li class="fl"><a href="<?=geturl('troomv2/teacher/courseware/'.$tuser[0]['uid'])?>?rurl=<?=$rurl?>&selfolderid=<?=$folder['folderid']?>&rrurl=<?=$rrurl?>" class="<?=$selfolderid==$folder['folderid']?'select':''?>"><?=$folder['foldername']?></a></li>
                    <?php }?>
                </ul>
                <div class="clear"></div>
            </td> 
        ...
        <tr>
            ...
            <td width="139"><a target="<?=$target?>" href="<?=geturl('troomv2/teacher/'.$coursetype.'/view-1-0-0-'.$cw['cwid'])?>">查看</a>&nbsp;&nbsp;&nbsp;<a href="<?=geturl('troomv2/teacher/cwstudylog/'.$cw['cwid']).'?rurl='.$this->uri->path.'&rrurl='.$rurl?>">学习监控</a></td>
        </tr>
		<?php }
		}else{?>
