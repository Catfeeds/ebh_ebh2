						...
						<tbody class="tabcont">
										...
										<?php $arr = explode('.',$avalue['cwurl']);
												$type = $arr[count($arr)-1]; ?>
                                                                <?php if($akey == 0) { ?>
									<td style="width:190px;" rowspan="<?= count($att)?>"><a target="<?= (empty($avalue['cwurl']) || $type == 'flv') ? '_blank' : '' ?>" href="<?= (empty($avalue['cwurl']) || $type == 'flv') ? geturl('troomv2/course/'.$avalue['cwid']) : geturl('troomv2/'.$coursepath.'/'.$avalue['cwid']) ?>"><?= $avalue['ctitle'] ?></a></td>
...
<a class="atfalsh" href="javascript:void(0);" source="<?= (empty($source) ?$avalue['source']:$source) ?>" title="<?= $avalue['title']?>" suffix="<?= $avalue['suffix'] ?>" cwid="<?= $avalue['cwid'] ?>" aid="<?= $avalue['attid'] ?>" ><?= $avalue['title'] ?></a>
								...
								<input type="button" class="workBtn" onclick="location.href='<?= geturl('troomv2/course/editattach/'.$avalue['attid'])?>'"  value="修改" />
