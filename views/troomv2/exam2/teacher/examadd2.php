   		...
   		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="infoPane">
			...
			<tr>
				<td colspan="3" class="marg10">
					<span>布置时间：<?=date('Y-m-d H:i',time())?></span>
					<span>限时：</span><input type="text" id="limittime" onkeyup="this.value=this.value.replace(/\D/g,'')" onblur="examobj.changetime(this.value)"    onafterpaste="this.value=this.value.replace(/\D/g,'')" maxlength="5" name="limittime"  class="timeLimit inputTxt" value="120" /><span>分钟</span>
...
<div class="rykhje">
	<div class="rtykwr" style="margin: 0 auto;">
		...
		<div class="center"><!--examobj.addExam(0);-->
			 <a href="javascript:void(0);"  class="rtyiok" onclick="if($.trim($('#viewcontent').html())==''){alertnoZero();return;};examobj.uploadData(0);">保存草稿</a>
			 ...
			 <a href="javascript:void(0);"  class="rtyiok active" onclick="if($.trim($('#viewcontent').html())==''){alertnoZero();return;};examobj.uploadData(1);">布置作业</a>
		</div>
		<div class="righ">
...
$(function(){
	...
	examobj.init({crid:<?php echo $crid?>,classid:'0',folderid:"<?php echo empty($folderid)?'':$folderid;?>",cwid:"<?php echo empty($cwid)?'':$cwid;?>",cwtitle:"<?php echo empty($cwtitle)?'':$cwtitle;?>"});
	new xChapter({'callback':function(data){
    },'checkbox':true,'showitem':true,'bindthird':1});
    hearttoolbar();
    Myestype.prototype.addJobtype();  //作业类型管理
	//上传图片
	H.create(new P({
		id:'upImgDialog',
		title:'上传图片',
...
<link href="http://exam.ebanhui.com/static/css/play.css<?=$v?>" rel="stylesheet" type="text/css" />
...
$(function(){
	...
	$("#btnsearch").on("click",function(){
		if($(".tikes").attr("source") == "my") {
			var key = $.trim($("#skey").val()) == "请输入你要搜索的关键词" ? "":$.trim($("#skey").val());
			myque.getpubquestion(1);
		} else if($(".tikes").attr("source") == "fav") {
			var key = $.trim($("#skey").val()) == "请输入你要搜索的关键词" ? "":$.trim($("#skey").val());
			favque.getpubquestion(1);
		}else if($(".tikes").attr("source") == "school") {
			var key = $.trim($("#skey").val()) == "请输入你要搜索的关键词" ? "":$.trim($("#skey").val());
			schoolque.getpubquestion(1);
		} else {
			$(".divgrade").hide();
			$(".divsubject").hide();
			var key = $.trim($("#skey").val()) == "请输入你要搜索的关键词" ? "":$.trim($("#skey").val());
			pub.getpubquestion(1);
		}
	});

	$('body').on('mouseover',".datatab tr",function(){
		$(this).addClass("over3");
	}).on('mouseout',".datatab tr",function(){
		$(this).removeClass("over3");
	}) //移除该行的class 
	$('body').on('click','#closequestion',function(){
...
<div style="display:hidden;background:#fff;" id="cwDialog"></div>
