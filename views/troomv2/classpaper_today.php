...
<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troomv2/classexam') ?>">班级作业</a> > <a href="<?= geturl('troomv2/classexam/cor') ?>">批改作业</a> > <?= $myclass['classname'] ?> > <?=$d?>
</div>
...
<div id="icategory" class="clearfix" style="border:none;">
	...
	<dd style="width:681px;">
		<div class="category_cont1">
			<div>
				<a <?php if($curclassid==0){echo 'class="curr"';}?> href="/troomv2/classexam/todayexams.html?d=<?=$d?>">所有班级</a>
			</div>
			<?foreach ($classDb as $class) {?>
				<div>
					<a <?php if($curclassid==$class['classid']){echo 'class="curr"';}?> href="/troomv2/classexam/todayexams-0-0-0-<?=$class['classid']?>.html?d=<?=$d?>"><?=$class['classname']?></a>
				</div>
			<?php }?>
        </div>
	</dd>
		 ...
		 <?php if(!empty($exams)) { ?>
			...
			<?php 
				if($exam['grade'] == 0){
            		...
            		// $classname = array_key_exists('c_'.$exam['classid'], $classDb)?'<a href="/troomv2/classexam/todayexams-0-0-0-'.$exam['classid'].'.html?d='.$d.'">'.$classDb['c_'.$exam['classid']]['classname'].'</a>':$exam['classid'];
            ...
            ?>
	              ...
	              <?php 
	              		...
	              		$classname = array_key_exists('c_'.$classid, $classDb)?'<a href="/troomv2/classexam/todayexams-0-0-0-'.$classid.'.html?d='.$d.'">'.$classDb['c_'.$classid]['classname'].'</a>':$classid;
				  ...
				  <tr>
				  	<td colspan=5 width="100%" name="<?= $exam['eid'] ?>">
						...
						<span style="width:100px;float:left;text-align:center;"><?= $stunum.'/'.$exam['answercount'] ?></span>
							...
							<?php } else { ?>
							<a class="previewBtn" href="<?= geturl('troomv2/classexam/all-0-0-0-'.$classid.'-'.$exam['eid']) ?>" >作业批阅</a>
							<?php } ?>
						</div>
					<!-- ========= -->
