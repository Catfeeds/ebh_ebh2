...
float:left; padding: 15px 0;
	...
	<div class="ter_tit">
		当前位置 > <a href="<?= geturl('troomv2/subject') ?>">课程列表</a> > <a href="<?= geturl('troomv2/subject/'.$folder['folderid']) ?>"><?= $folder['foldername'] ?></a> > <?= $course['title']?>
		</div>
	<div class="lefrig">
...
<script type="text/javascript">
    function course_submit() {
        if(teacher_submit('course',param='cdisplayorder')) {
            $.ajax({
                url:"<?= geturl('troomv2/classcourse/edit') ?>",
                type: "POST",
