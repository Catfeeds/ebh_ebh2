...
<div class="ter_tit">
	当前位置 > <a href="<?= geturl('troomv2/online')?>" >在线直播</a> > 添加直播
</div>
...
<form id="formdata">
	...
	<table class="addteacher_tab" width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td colspan="3">
			<input style="padding-left: 5px;" class="uipt w295" maxlength="25" name="title" id="title" type="text" value="" onblur="teacher(this,'课程名称','title','','n','','','请输入课程名称。')">
	   ...
	   <tr>
		...
		<td colspan="3">
			...
			<!-- <input class="uipt w175" name="cdate" id="cdate" onfocus="" type="text" /> -->
		...
		<td colspan="3">
			<select class="uipt w175" type="text"  maxlength="100" name="ctime" value="" >
	  ...
	  <tr>
		...
		<td colspan="3">
			<input style="padding-left: 5px;" class="uipt w175" maxlength="15" name="tname" id="tname" type="text" value="<?=empty($user['realname'])?$user['username']:$user['realname']?>" onblur="teacher(this,'主讲老师','tname','','n','','','请输入课程主讲老师，默认为当前登录的老师。')" >
	  ...
	  <tr>
		...
		<td colspan="3">
			<input style="padding-left: 5px;" class="uipt w175" name="maxnum" id="maxnum" type="text" value="100" onblur="teacher(this,'最大人数','maxnum','^[0-9]*$','n',null,'','请输入直播课程允许容纳的最大学生人数。')" >
    ...
    <tr>
		...
		<td colspan="3">
			<textarea style="padding-left: 5px;border:solid 1px #9EB7CB;resize:none;" name="summary" id="summary" cols="80" rows="5" value="" onblur="teacher(this,'课程简介','summary','','n',10,'','请输入课程简介信息，字数控制在10-250个字符之间。',250)"></textarea>
	  ...
	  <tr>
		...
		<td style="padding-top:14px">
            	<div id="xuanban" class="xuanban" style="background:#fff;">
					<h2 class="rbxht">
						<span style="float:left" id="gradetip" class="gradetip">请选择直播的班级</span>
						<?php if(!empty($grades)){?>
							<a href="javascript:" class="bluebtn gradebtn" style="float:left;margin-left:200px;">按年级直播</a> 
						<?php }?>
					</h2>
					<ul class="ulclass">
					...
					<ul class="ulgrade" style="display:none;">
						<?php foreach ($grades as $grade) {?>
							<li><span><input class="mygrade" type="checkbox" name="grades[]" title="<?=$grade['gradename']?>" value="<?=$grade['grade']?>:<?=$grade['district']?>"><?=$grade['gradename']?></span></li>
						<?php }?>
					</ul>
					<?php }?>
...
<script>
	$('.rbxht a').click(function(){
		...
		if($('#flag').val()==1){
			$('#flag').val(0);
			$('#gradetip').html('请选择直播的班级');
			$(this).html("按年级直播");
		}else{
			$('#flag').val(1);
			$('#gradetip').html('请选择直播的年级');
			$(this).html("按班级直播");
		}
	});
	...
	function beforesubmit(){
		...
		$.post("/troomv2/online/add.html",{param:$("#formdata").serialize()},function(message){
				if(message=="1"){
					$.showmessage({
						message:'添加成功！',
						callback :function(){
                            document.location.href = "/troomv2/online/allcourse.html";
                        }});
				}
				else
					$.showmessage({message:'添加失败！'});
		});
	}
	$(function(){
