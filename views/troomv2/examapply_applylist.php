...
<div class="ter_tit">
	当前位置 &gt; <a href="<?=geturl('troomv2/examapply')?>">认证管理</a> &gt; 待审核
        <div class="diles fr">
...
<div class="lefrig" style="min-height:1000px;">
    <div class="workol">
        <div class="work_mes">
            <ul class="extendul">
                <li class="workcurrent"><a href="<?=geturl('troomv2/examapply/applylist')?>"><span>待审核</span></a></li>
                <li><a href="<?=geturl('troomv2/examapply/passapply')?>"><span>已通过</span></a></li>
                <li><a href="<?=geturl('troomv2/examapply/unpassapply')?>"><span>未通过</span></a></li>
            </ul>
        </div>
        <div class="clear"></div>
...
function _search(){
	...
	if(searchkey == '请输入学生姓名或登录帐号')
		searchkey = '';
	location.href = '<?=geturl('troomv2/examapply/applylist')?>?q='+searchkey;
}
	...
	$("#donopass").click(function(){
		...
		button.add({
			...
			callback:function(){
				...
				$.post('/troomv2/examapply/batchcheck.html',{ids:idarr.join(","),status:2},function(data){
					if(data != undefined && data != null && data.code ==1){//审核成功
						$.showmessage({
							img : 'success',
							message:data.msg,
