...
<div class="ter_tit">
    当前位置 > <a href="<?= geturl('troomv2/classsubject/courses') ?>" >课程管理</a> > <a href="<?= geturl('troomv2/classsubject/' . $course['folderid']) ?>"><?= $course['foldername'] ?></a> >  <?= $course['title'] ?>
</div>
<div class="lefrig">
        ...
        <div class="classboxmore">
            ...
            <p style="padding:10px;margin-left:-10px;">
				...
				<?php if($course['uid'] == $user['uid']){ ?>
                    <a class="huangbtn" href="<?= geturl('troomv2/classcourse/upattach-0-0-0-'.$course['cwid'])?>">上传附件</a>
				<?php } ?>

				<?php if(preg_match('/.*(\.ebh|\.ebhp)$/',$course['cwurl'])) {?>
						<?php if(empty($hasnobtn) || $hasnobtn != TRUE) { ?>
						<a href="<?= geturl('troomv2/course/'.$course['cwid']) ?>" class="huangbtn marrig" target="_blank" style="float:right;margin-right:50px;">网页播放</a>
						<?php } ?>
					<?php } ?>
                </p>
            </div>
