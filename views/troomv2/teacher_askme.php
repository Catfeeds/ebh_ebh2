...
?>
	<?php 
	if($rurl == 'troomv2/ask'){?>
		当前位置 > <a href="<?=geturl('troomv2/report')?>">统计分析</a> > <a href="<?=geturl('troomv2/myask')?>"> 答疑查看 </a> > 指定提问
	<?php }else{?>
		当前位置 > <a href="/troomv2/statisticanalysis.html">查询统计</a> > <a href="/troomv2/statisticanalysis/teach.html"> 教师统计 </a> > 指定提问
	<?php }?>
	</div>
<div class="toview">
	<table cellpadding="0" cellspacing="0" class="tables">
    	<tr class="second">
        	<td width="786"  colspan="7">
           		<div>
					...
					?>
                    <li class="fl"><a href="<?=geturl('troomv2/teacher/askme/'.$tuser[0]['uid'])?>?rurl=<?=$rurl?>" class="<?=empty($selfolderid)?'select':''?>">不限</a></li>
					<?php
					foreach($folderlist as $folder){
					?>
                    <li class="fl"><a href="<?=geturl('troomv2/teacher/askme/'.$tuser[0]['uid'])?>?rurl=<?=$rurl?>&selfolderid=<?=$folder['folderid']?>" class="<?=$selfolderid==$folder['folderid']?'select':''?>"><?=$folder['foldername']?></a></li>
                    <?php }?>
                    </ul>
                </div>
                <div class="clear"></div>
        ...
        <tr>
            ...
            <td width="114"><?php if(empty($ask['shield'])){?>
					<a href="<?=geturl('troomv2/myask/'.$ask['qid']).'?rurl='.$this->uri->path.'&rrurl='.$rurl?>">
					查看详情</a>
...
function qshield(qid,shield){
	$.ajax({
		url:'/troomv2/myask/qshield.html',
		type:'post',
