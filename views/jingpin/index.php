    ...
    <div class="tisrwe">
        <div class="ltsire">
            ...
            if(!($this->input->get('bsid'))){
                $display = 'none';
                ?>
                <li class="fklisr" ><a class="wursk dusre" style="border-left:0px; " href="/ke.html">热门课程</a></li>
            <?php }else{ 
                $display = '';
                ?>
               ...
               <?php
                ...
                ?>
                <li class="fklisr"><a class="wursk" href="/ke/myclass.html">我的课程</a></li>
            </ul>
        </div>
        <div class="xaiostie" style="display: <?php echo $display?>">
        <?php if(!empty($nextsorts)){
                foreach ($nextsorts[2] as $key => $value) { ?>
                <div class="nisnfe" style="border-bottom: 0px;">
                <a href="/ke.html?bsid=<?php echo $bsid?>&msid=<?php echo $value['sid']?>"><span class="nstrdss"><?php echo $value['sname']?>：</span></a>
                <?php $msid = $this->input->get('msid');$ssid=$this->input->get('ssid');if(($msid == $value['sid']) && empty($ssid)){?>
                <a href="/ke.html?bsid=<?php echo $bsid?>&msid=<?php echo $value['sid']?>" class="khrusre dianlan">全部</a>
                <?php }else{?>
                <a href="/ke.html?bsid=<?php echo $bsid?>&msid=<?php echo $value['sid']?>" class="khrusre">全部</a>
                <?php }if(!empty($value['child'])){
                        foreach ($value['child'] as $key1 => $value1) { ?>
                        <?php $ssid=$this->input->get('ssid');if($ssid == $value1['sid']){?>
                             <a href="/ke.html?bsid=<?php echo $bsid?>&msid=<?php echo $value['sid']?>&ssid=<?php echo $value1['sid']?>" class="khrusre dianlan"><?php echo $value1['sname']?></a>
                        <?php }else{?>
                            <a href="/ke.html?bsid=<?php echo $bsid?>&msid=<?php echo $value['sid']?>&ssid=<?php echo $value1['sid']?>" class="khrusre"><?php echo $value1['sname']?></a>
                       <?php }
                   }
                ?>
        ...
        <?php if(!empty($label)){?>
        <div class="lisrtde lisrtde1">
            <span class="sherdfs">课程：</span>
            <div class="kertsd">
            <?php foreach($label as $key=>$value){?> 
                <a href="javascript:void(0);" class="kluehrt kluehrt1"><?php echo $value['label']?></a>
            <?php }?>                
            </div>
            <a href="javascript:void(0);" class="fewtfd"></a>
        </div>
        <?php }?>
        <?php if(!empty($label)){?>
        <div class="lisrtde" id="morechoose" style="display: none">
            <span class="sherdfs">课程：</span>
            <div class="kertsd">
            <?php foreach($label as $key=>$value){?>
                <a href="javascript:void(0);" class="kluehrt kluehrt2"><label><input class="leutwr" type="checkbox" name="checkbox2" id="checkbox2" value="<?php echo $value['label']?>"/><?php echo $value['label']?></label></a>
            <?php }?>
            </div>
            <div class="kertsd">
                <a href="javascript:void(0);" class="jisrfe">确定</a>
                <a href="javascript:void(0);" class="queses">取消</a>
            </div>
        </div>
        <?php }?>
        <div class="kjutes">
        <?php $order = $this->input->get('order');?>
            <a class="<?php if($order == null){echo 'lisrhe';}else{echo 'luiets';}?>" href="javascript:void(0);" onclick="orderby()">综合排序</a>
            <a class="<?php if($order == 'time'){echo 'lisrhe';}else{echo 'luiets';}?>" href="javascript:void(0);" onclick="orderby('time')">最新</a>
            <a class="<?php if($order == 'viewnum'){echo 'lisrhe';}else{echo 'luiets';}?>" href="javascript:void(0);" onclick="orderby('viewnum')">人气</a>
            <a class="<?php if($order == 'price_high'||$order == 'price_low'){echo 'lisrhe';}else{echo 'luiets';}?>" id="price" href="javascript:void(0);">价格 <img src="http://static.ebanhui.com/ebh/tpl/courses/images/lisrne.png" /></a>
            <div class="lirtys" style="display: none">
                <ul>
                    <li><a class="luetsa" href="javascript:void(0);" onclick="orderby('price_high')">从高到低</a></li>
                    <li><a class="luetsa" href="javascript:void(0);" onclick="orderby('price_low')">从低到高</a></li>
                </ul>
            </div>
            <span>
            <?php $price = $this->input->get('price');?>
                <label><input class="kuiets" type="checkbox" name="checkbox" id="checkbox" <?php if($price){echo 'checked="checked"';}?>/>只看免费公开课</label>
            </span>
        </div>
        <div class="ketds">
        <?php if(!empty($bestitems)){$i=0;?>
            ...
            <?php foreach($bestitems as $key=>$value){
                ...
                <li class="iuhni" style="margin-right:0px;">
                    ...
                    <h2 class="klejts"><a  title="<?php echo $value['iname']?>" href="/ke/<?php echo $value['itemid']?>.html"><?php if(mb_strlen( $value['iname'], 'utf-8') > 30){$value['iname'] = mb_substr($value['iname'],0,29,'utf-8').'...';}?><?php echo $value['iname']?></a></h2>
...
<script type="text/javascript">
    ...
    $(function(){
        $('.nisnfe:last').css('border-bottom','0px');
    })
</script>
