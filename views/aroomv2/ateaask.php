...
input.workBtns-1{
	...
	<div class="ter_tit">
		当前位置 > <a href="/aroomv2/report.html">统计分析</a> > <a href="/aroomv2/teacher/viewnav.html">教师查看</a> > 教师答疑统计
		</div>
	...
	<a class="workBtns workBtns-1" onclick="selecttpye()"> 导出excel</a>
	</div>
	</div>
...
<input type="text" id="startdate" readonly="readonly" style="border:1px solid  #dbdbdb;float:left;text-indent:10px;width:100px;height:28px;line-height:28px;margin:5px;display:inline;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" value="<?=$startdate?>"/>
<span style="float:left;margin-top:10px;margin-left:3px">
-
</span>
<input type="text" id="enddate" readonly="readonly" style="border:1px solid  #dbdbdb;float:left;text-indent:10px;width:100px;height:28px;line-height:28px;margin:5px;display:inline;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" value="<?=$enddate?>"/>


</div>
<input class="workBtns workBtns-1" type="button" id="" onclick="searchbydate()" value="搜 索"/>

<span style="float:left;height:28px;line-height:28px;margin-left:20px;">查看教师：</span>
<input type="text" class="kipt" style="float:left;color:black;width:120px;height:28px;line-height:28px;" oninput="hlteacher()" onpropertychange="hlteacher()" id="tsearch"/>
<input class="workBtns workBtns-1" style="margin-left:16px;" type="button" id="searchbutton" onclick="clearsearch()" value="清 除">
...
<tbody>
	...
	?>
		...
		<td width="100px"><a href="<?=geturl('aroomv2/ateaask/ateaasklist-0-0-0-'.$tl['uid'])?>" class="seedayi" title="查看答疑">查看答疑</a></td>
		</tr>
	<?php }?>
	<tr>
...
function searchbydate(){
	var sdate = $('#startdate').val();
	var edate = $('#enddate').val();
	var href='/aroomv2/ateaask.html?sdate='+sdate+'&edate='+edate;
	location.href = href;
}
