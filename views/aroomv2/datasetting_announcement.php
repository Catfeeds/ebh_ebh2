...
<div class="flopadt">
<a class="hongbtn jiabgbtn" href="<?= geturl('aroomv2/datasetting/announcement/add') ?>" >添加公告</a></div>
<table width="100%" class="datatab" style="border-left:none;border-right:none;">
	<thead class="tabhead">
	 ...
	 <?php foreach($sendlist as $send) { ?>
			...
			<a class="workBtn" href="<?= geturl('aroomv2/datasetting/announcement/edit-0-0-0-'.$send['infoid'])?>" >修改</a>
...
<!--
	...
	$.confirm("您确定要删除这条公告吗？",function(){
		...
		$.ajax({
			url:"<?= geturl('aroomv2/datasetting/delsend')?>",
			type:'post',
			data:{'infoid':infoid},
			dataType:'text',
