...
<body>
<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroomv2/classstudent')?>">班级学生</a> > <a href="<?=geturl('aroomv2/lcyhstu')?>">学生管理</a> > 批量导入
		</div>
...
<a href="http://static.ebanhui.com/ebh/file/lcyhg_22.xls" class="mobancun" target="_blank">导入模板下载</a>
</div>
<div class="huidis">
<div class="neileft105">

<?php if(empty($inputresult['hasresult'])){?>
<form id="inputform" action="<?=geturl('aroomv2/lcyhstu/input')?>" method="post" enctype="multipart/form-data">
  <input style="float:left;width:350px;height:30px;cursor:pointer;" type="file" name="inputfile" />
  <input name="flag" type="hidden"/>
  <a style="float:right" href="<?=geturl('aroomv2/lcyhstu')?>" class="flhui">返回</a>
...
<input class="xuetjbtn" type="button" value="继续导入" onclick="window.location.href='<?=geturl('aroomv2/lcyhstu/input')?>'" />
...
<input class="xuetjbtn" type="button" value="重新导入" onclick="window.location.href='<?=geturl('aroomv2/lcyhstu/input')?>'" />
<a href="<?=geturl('aroomv2/lcyhstu')?>" class="flhui" style="color:#065ed7;float:left;font-size:14px;margin:5px 0 0 10px;">返回</a>
</p>
<?php }}?>
