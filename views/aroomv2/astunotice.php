...
<div class="cright" style="margin-top:0px;">
	<div class="ter_tit"> 当前位置 > <a href="<?=geturl('aroomv2/astunotice')?>">全校师生通知</a> > 通知列表 </div>
	<div class="lefrig" style="border:solid 1px #cdcdcd;background:#fff;float:left;margin-top:15px;width:786px;">
		<div class="worknu">
			<ul>
				<li class="workcurren"><a href="<?=geturl('aroomv2/astunotice')?>"><span>通知列表</span></a></li>
				<?php if($haspower == 1) { ?><li><a href="<?=geturl('aroomv2/astunotice/send')?>"><span>发送通知</span></a></li><?php } ?>
			</ul>
		</div>
			...
			<tbody>
				...
				$noticetype = array(1=>'全校师生',2=>'全校教师',3=>'全校学生',4=>'班级学生',5=>'部分年级学生');
					...
					<tr>
						...
						<td width="18%">
							<a href="<?=geturl('aroomv2/astunotice/edit/'.$nl['noticeid'])?>" class="liuibtn">修改</a>
...
<script type="text/javascript">
	$(".delnotice").click(function(){
		...
		$.confirm("您确定删除【"+title+"】这个通知消息吗？",function(){
			$.ajax({
				type: "POST",
				url: "<?=geturl('aroomv2/astunotice/del')?>",
				data: {"nid":nid},
				dataType:"json",
