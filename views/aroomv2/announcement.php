...
<div class="flopadt">
<a class="hongbtn jiabgbtn" href="<?= geturl('aroomv2/announcement/add') ?>" >添加公告</a></div>
<table width="100%" class="datatab">
	<thead class="tabhead">
	 ...
	 <?php foreach($announcementlist as $send) { ?>
			 <tr>
			 <td width="74%"><?= $send['message'] ?></td>
			<td width="10%"><?= date('Y-m-d',$send['dateline']) ?></td>
			 <td width="16%"><div class="teac_manage">
			<a class="workBtn" href="<?= geturl('aroomv2/announcement/edit-0-0-0-'.$send['infoid'])?>" >修改</a>
...
<!--
	...
	$.confirm("您确定要删除这条公告吗？",function(){
		...
		$.ajax({
			url:"<?= geturl('aroomv2/announcement/delsend')?>",
			type:'post',
			data:{'infoid':infoid},
			dataType:'text',
