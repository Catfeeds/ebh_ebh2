...
<div>
	<div class="ter_tit">
        当前位置 > <a href="<?=geturl('aroomv2/classes/student')?>">班级学生</a> > <a href="<?=geturl('aroomv2/student')?>">学生管理</a> > 学生列表
    </div>
    <div class="xueshengguanli2 banjiguanli1s">
    <?php if(!empty($classlist)){ ?>
		<div class="selectclass">
			<div class="kstdg" style="min-width:140px;">
				<span class="xchapterverselected"></span>
			</div>
			<div class="xchaptersver" id="chaptersver" style="display: none;width:175px;">
			<?php if(!empty($classlist)){ ?>
				<?php foreach ($classlist as $clist) { ?>
				<a class="chver_option" style="width:173px;" href="/aroomv2/student/list_view.html?classid=<?php echo $clist['classid'];?>&rurl=aroomv2/student" cid="<?php echo $clist['classid'];?>"><?php echo $clist['classname'];?></a>
			<?php } ?>
			<?php } ?>
			</div>
		</div>
        ...
        <div class="xueshengguanli2_bottom">
        	<table cellpadding="0" cellspacing="0" class="tables">
				...
				foreach($roomuserlist as $user){
					...
					?>
                <tr>
					<td width="215" style="text-align:left;"><div class="fl"><img style="width:50px;height:50px;border-radius:25px;" src="<?=$face?>" /></div><div class="p2" style="line-height:18px;">
					<p style="width:155px;height:18px; overflow:hidden;">
						<b title="<?=$user['realname']?>" style="float:left;"><?=shortstr(empty($user['realname'])?$user['username']:$user['realname'],12,'')?></b>
						<!--性别修改-->
						<span class="<?=(empty($user['sex']) ? 'xbwomen' : 'xbman')?>"></span>
						<!--性别修改-->
					</p>
					<div style="clear:both;"></div>
					<p style="width:95px;height:18px; overflow:hidden;" class="p2sxg"><?=$user['username']?></p><p class="jifenico" style="color:#9e9ea0 !important;"><?=$user['credit']?></span></p></div></td>
                	<td width="110"><?=$user['classname']?></td>
                    <td width="155"><?=empty($user['email'])?'':$user['email']?></td>
                    <td width="115"><?=empty($user['mobile'])?'':$user['mobile']?></td>
                    <td width="140">
						<div style="position:relative; vertical-align:middle;">
							...
							<div class="moreoperate" style="display:none;">
								...
								<span  class="adel moreoperatea">删除</span>
								<span  class="aresetpass moreoperatea">重置密码</span>
    ...
    <div class="button5 fr"><a href="/<?=$rurl?>.html?rurl=<?=$rrurl?>" class="xgbutton5">返 回</a></div>
	<?=$pagestr?>
</div>
...
<!--
    ...
    <div class=" ml55 edus">
		...
		<input type="hidden" id="user_edit_oldclassid"/>
		<select class="selects" name="classid" id="user_edit_classid">
		<?php foreach($classlist as $cl){?>
	    	<option value="<?=$cl['classid']?>"><?=$cl['classname']?></option>
		<?php }?>
		</select>
		<!--
        <input id="user_edit_classname" readonly="readonly" class="text input readonly" type="text" value=""/>
		
        <input type="button" value="选择班级" class="text1" />
		-->
    </div>
</div>
</div>
...
function saveedit(){
	...
	var classid = $('#user_edit_classid').val();
	var birthdate = $('#user_edit_birthdate').val();
	var oldclassid = $('#user_edit_oldclassid').val();
	// return;
	$.ajax({
		type:'post',
		url:'<?=geturl('aroomv2/student/edit')?>',
		dataType:'text',
		data:{'uid':uid,'realname':realname,'sex':sex,'email':email,'mobile':mobile,'birthdate':birthdate,'classid':classid,'oldclassid':oldclassid},
		success:function(data){
			dialogtip();
			if(data==1){
...
function savedel(){
	...
	$.ajax({
		type:'post',
		url:'<?=geturl('aroomv2/student/del')?>',
		dataType:'json',
		data:{'uid':uid},
...
function checksname(){
	...
	if(username == ''){
		...
		$.ajax({
			...
			url:"<?=geturl('aroomv2/student/add')?>",
			dataType:'json',
			data:{'username':username,'checkonly':1},
			async:false,
			success:function(_json){
				if(_json.code == 0){
					$("#username_msg").attr('class','p3 emacuo');
...
$(function(){
	...
	$("#chaptersver").children('a').each(function(){
		if($(this).attr('cid') == cid){
			var classname = $(this).html();
			$(".xchapterverselected").html(classname);
		}
	});
	$(".kstdg").on('click',function(){
		$('.xchaptersver').toggle();
	})
})
</script>
