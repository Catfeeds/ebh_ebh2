...
<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroomv2/classes/student')?>">班级学生</a> > <a href="<?=$domain == 'lcyhg' ? geturl('aroomv2/lcyhstu') : geturl('aroomv2/classes')?>">班级管理</a> > 批量导入
		</div>
...
<a href="http://static.ebanhui.com/ebh/file/lcyhg_11.xls" class="mobancun" target="_blank">导入模板下载</a>
</div>
<div class="huidis">
<div class="neileft105">

<?php if(empty($inputresult['hasresult'])){?>
<form id="inputform" action="<?=geturl('aroomv2/classes/input')?>" method="post" enctype="multipart/form-data">
  <input style="float:left;width:350px;height:30px;cursor:pointer;" type="file" name="inputfile" />
  <input name="flag" type="hidden"/>
  <a style="float:right" href="<?=$domain == 'lcyhg' ? geturl('aroomv2/lcyhstu') : geturl('aroomv2/student')?>" class="flhui">返回</a>
...
3.excel文件中的必须包含教学班名称、教师、账号三个字段。<br />
...
<p class="ptishi">导入成功，共导入 <?=$inputresult['rowcount']?> 条班级记录。</p>
<input class="xuetjbtn" type="button" value="继续导入" onclick="window.location.href='<?=geturl('aroomv2/classes/input')?>'" />
<a href="<?=$domain == 'lcyhg' ? geturl('aroomv2/lcyhstu') : geturl('aroomv2/student')?>" class="flhui">返回</a>
	<?php }else{?>
<br />
...
<input class="xuetjbtn" type="button" value="重新导入" onclick="window.location.href='<?=geturl('aroomv2/classes/input')?>'" />
<a href="<?=$domain == 'lcyhg' ? geturl('aroomv2/lcyhstu') : geturl('aroomv2/student')?>" class="flhui" style="color:#065ed7;float:left;font-size:14px;margin:5px 0 0 10px;">返回</a>
</p>
<?php }}?>
