...
此页为导入教师页面。<a href="http://static.ebanhui.com/ebh/file/tinput.xls" class="mobancun" target="_blank">导入模板下载</a>
</div>
<div class="huidis">
<div class="neileft105">
<?php if(empty($inputresult['hasresult'])){?>
<form id="inputform" action="<?=geturl('aroomv2/tclassconnect')?>" method="post" enctype="multipart/form-data">
  <input style="float:left;width:350px;height:30px;cursor:pointer;" type="file" name="inputfile" />
  <input name="flag" type="hidden"/>
  <a style="float:right" href="<?=geturl('aroomv2/classes')?>" class="flhui">返回</a>
...
<input class="xuetjbtn" type="button" value="继续导入" onclick="window.location.href='<?=geturl('aroomv2/teacher/input')?>'" />
<a href="<?=geturl('aroomv2/teacher')?>" class="flhui">返回</a>
		<?php }else{?>
...
<input class="xuetjbtn" type="button" value="重新导入" onclick="window.location.href='<?=geturl('aroomv2/teacher/input')?>'" />
<a href="<?=geturl('aroomv2/teacher')?>" class="flhui" style="color:#065ed7;float:left;font-size:14px;margin:5px 0 0 10px;">返回</a>
</p>
<?php }}?>
</div>
</div>
