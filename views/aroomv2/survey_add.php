	...
	<div>
		...
		<fieldset class="relatefs">
			...
			<div style="margin-left: 20px;">
                    <input id="startdate" class="qisties" readonly="readonly" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'<?=Date('Y-m-d',SYSTIME)?>'});" value="<?=date('Y-m-d H:i',SYSTIME);?>"/>
                    <span class="sjzjf">&mdash;</span>
					<input id="enddate" class="qisties" readonly="readonly" type="text"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'startdate\')}'});" value="<?=date('Y-m-d H:i',SYSTIME+604800);?>"/>
			</div>
		</fieldset>
...
function submit(){
	...
	$.ajax({
		type:'post',
		url:'/aroomv2/survey/create.html',
		data:{
			'title':title_content,
			'content':content,
