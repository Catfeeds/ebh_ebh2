...
<div class="ter_tit">
当前位置 > <a href="<?= geturl('aroomv2/datasetting/announcement/add') ?>">公告管理</a> > 添加公告</div>
...
$(function(){
		...
		$("#savebtn").click(function(){
			if(submit_check('message')) {
				var url="<?= geturl('aroomv2/datasetting/announcement/add')?>";
				$.ajax({
					url:url,
					type: "POST",
					...
					success:function(data){
						if(data == 'success') {
							$.showmessage({
								...
								title:'添加公告',
								callback :function(){
									 document.location.href = "<?= geturl('aroomv2/datasetting') ?>";
								}
							});
						} else {
							$.showmessage({
