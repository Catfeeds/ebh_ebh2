...
<div class="ter_tit">
	...
	<div class="lefrig">
<div class="annuato" style="line-height:28px;padding-left:20px;border:solid 1px #cdcdcd;background:#fff;position: relative;">
...
<div id="icategory" class="clearfix" style="border:solid 1px #cdcdcd;background:#fff;">
	...
	<dd style="width:680px;">
		<div class="category_cont1">
			<div>
				<a <?php if(empty($classid)){?>class="curr"<?php }?> href="<?=geturl('aroomv2/astulog-0-0-0-0')?>">所有学生</a>
			</div>
			<?php foreach($classlist as $cl){?>
			<div>
				<a <?php if($classid==$cl['classid']){?>class="curr"<?php }?>href="<?=geturl('aroomv2/astulog-0-0-0-'.$cl['classid'])?>"><?=$cl['classname']?></a>
			</div>
			<?php }?>
		</div>
	</dd>
...
<tbody>
		...
		<tr>
			...
			<td style="width:300px;">
				<a title="班级作业" class="previewBtn" href="<?=geturl('aroomv2/astulog/astuloglist-0-0-0-'.$v['uid'])?>">听课监察</a>
			</td>
		</tr>
		<?php }}else{?>
...
$(function(){
	$('#searchbutton').click(function(){
		...
		var url = '<?=geturl('aroomv2/astulog-0-0-0-'.$classid)?>';
