...
<div class="ter_tit">
当前位置 > <a href="<?=geturl('aroomv2/setting/avatarold')?>">网校校徽</a> > 自定义校徽
</div>
...
<div class="work_menu">
						<ul>
							<li class="workcurrent"><a href="<?=geturl('aroomv2/setting/avatarold')?>"><span>自定义头像</span></a></li>
							<li><a href="<?=geturl('aroomv2/setting/roomlogo')?>"><span>系统头像</span></a></li>
						</ul>
					</div>
...
function preparexPhoto(id,callback,initpicurl){
	...
	window.xphoto = new UploadImageAndThumb({
		...
		sizearr:new Array('220_220'),
		sizemsgarr:new Array('课程图片尺寸比例为1:1')
	});
...
function msghandle(res){
	if(res && res.status == 0){
		$.ajax({
			url:'<?=geturl('aroomv2/setting/avatarupdate')?>',
			type:'post',
			data:{'op':'logo','destshowpath':baseUrl+res.data.thumb},
...
//准备一个xPhoto实例(用时调用)
function preparexPhoto(id,callback,initpicurl){
	var upurl = 'http://up.ebh.net/imghandler.html?type=avatar&subtype=customlogo';
	window.xphoto = new xPhoto({
		...
		upurl:encodeURIComponent(upurl),
		sizearr:new Array('220_220'),
		sizemsgarr:new Array('图片尺寸为220*220')
	});
	window.xphoto.render();
}
<?php } ?>
