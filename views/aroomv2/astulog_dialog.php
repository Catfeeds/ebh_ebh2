...
.datatab td {
	...
	<div class="ter_tit">
		<!-- 当前位置 > <a href="<?=geturl('aroomv2/astulog')?>">学生听课监察</a> > 听课监察 -->
		<div class="diles">
	...
	<div class="lefrig" style="border:solid 1px #cdcdcd;background:#fff;float:left;margin-top:15px;width:798px;">
...
<!-- <a href="<?=geturl('aroomv2/astulog')?>" class="tfanhui" style="left: 640px;_top:13px;position: absolute;width:90px;">返回上一页</a> -->
</div>
...
<tbody>
	...
	?>
		<tr>
			...
			<td width="15%"><?=secondToStr($ll['ctime'])?></td>
			<td width="15%"><?=secondToStr($ll['ltime'])?></td>
...
$(function(){
	$('#searchbutton').click(function(){
		var href = '<?=geturl('aroomv2/astulog/astuloglist_fordialog-0-0-0-'.$uid.'-'.$folderid.'-searchvalue')?>';
		href += '?starttime=<?=$starttime_str?>&endtime=<?=$endtime_str?>'
