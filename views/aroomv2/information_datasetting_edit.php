...
<div class="">
	<div class="ter_tit">
        当前位置 > <a href="<?= geturl('aroomv2/information/datasetting') ?>">公告管理</a> > 公告设置
    </div>
	<form id="upform">	
	<input type="hidden" name="infoid" value="<?= $send['infoid']?>" />
    <div class="gonggao mt15">
    	<textarea class="khstfd" name="message" id="message" maxlength="500" onblur="submit_check('message')"><?= $send['message'] ?></textarea>
...
$(function(){
		...
		$("#savebtn").click(function(){
			if(submit_check('message')) {
				var url="<?= geturl('aroomv2/information/datasetting/edit')?>";
				$.ajax({
					url:url,
					type: "POST",
