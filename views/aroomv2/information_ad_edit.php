...
<div class="ter_tit">
当前位置 > <a href="/aroomv2/information/ad.html">广告管理</a> > 修改广告</div>
...
<option value="<?=$cat['catid']?>" <?php if($ad['catid']==$cat['catid']){echo 'selected=selected';}?>><?=$cat['name']?></option>
<?php }?>
</select>
...
<script type="text/javascript" src="http://static.ebanhui.com/ebh/js/jquery/jquery-lightbox-0.5/jquery.lightbox-0.5.min.js"></script>
	...
	<a class="sn" id="a" href="<?=$ad['thumb']?>" style="<?=empty($ad['thumb'])?'display:none':'diplay:block'?>">
...
$("#savebtn").click(function(){
      if(submit_check()) {
        var url="<?= geturl('aroomv2/information/ad/edit')?>";
        $.ajax({
          url:url,
          type: "POST",
