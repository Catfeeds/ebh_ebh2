...
<div class="tiezitool">
<a class="hongbtn jiabgbtn marrig" onclick="window.location.href='<?=geturl('aroomv2/student/add-0-0-0-'.$classid)?>'" >添加学生</a>
<a class="hongbtn jiabgbtn <?=$room['isschool']=='7'?'marrig':'marrig'?>" onclick="window.location.href='<?=geturl('aroomv2/student/input')?>'" >批量导入</a>
<a class="hongbtn jiabgbtn <?=$room['isschool']=='7'?'marrig':''?>" onclick="window.location.href='<?=geturl('aroomv2/student/input_upgrade')?>'" >升班导入</a>
<?php if($room['isschool']==7){?>
<a class="hongbtn jiabgbtn" style="width:135px" onclick="window.location.href='<?=geturl('aroomv2/student/input_scb')?>'">可查询账号导入</a>
<?php }?>
</div>
</div>
...
<div id="icategory" class="clearfix">
	...
	<dd>
		<div class="category_cont1">
			<div>
				<a <?php if(empty($classid)){?>class="curr"<?php }?> href="<?=geturl('aroomv2/student-0-0-0-0')?>">所有学生</a>
			</div>
			<?php foreach($classlist as $cl){?>
			<div>
				<a <?php if($classid==$cl['classid']){?>class="curr"<?php }?>href="<?=geturl('aroomv2/student-0-0-0-'.$cl['classid']).'?rnd='.rand()?>"><?=$cl['classname']?></a>
			</div>
			<?php }?>
		</div>
	</dd>
...
<tbody>
		...
		<tr>
			...
			<a class="workBtn" href="<?=geturl('aroomv2/student/edit/'.$v['uid'])?>">编辑</a>
...
$(function(){
	$('#searchbutton').click(function(){
		...
		var url = '<?=geturl('aroomv2/student-0-0-0-'.$classid)?>';
