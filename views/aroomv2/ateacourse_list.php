...
$(function(){
	$('#searchbutton').click(function(){
		var href = '<?=geturl('aroomv2/ateacourse/course/courselist-0-0-0-'.$folderid.'-'.$uid)?>';
...
<div class="ter_tit">
	当前位置 > <a href="<?=geturl('aroomv2/ateacourse/course/'.$uid)?>">该教师课件查看</a> > 
	<?php if(!empty($uparr)){
				$upfoldercount = count($uparr);
				foreach($uparr as $k=>$upfolder){
				$index = $upfoldercount-$k-1;
			?>
			<a href="<?=geturl('aroomv2/ateacourse/course/courselist-0-0-0-'.$uparr[$index]['folderid'].'-'.$uid)?>"><?=$uparr[$index]['foldername']?></a> > 
			<?php }}?>
			<?= $folder['foldername'] ?>
</div>
...
<div class="annuato" style="line-height:28px;padding-left:20px;position: relative;">该教师在此课程共有课件 <span style="color:#0709FE;font-weight:bold"><?=$coursecount?></span> 个
		<a class="tfanhui" style="width:90px;" href="<?=geturl('aroomv2/ateacourse/course/'.$uid)?>">返回上一层</a></div>
<?php }?>
<?php if(!empty($subfolderlist)) { ?>
<div class="kejian">
	<ul class="liss">
	...
	<div class="showimg"><a href="<?=geturl('aroomv2/ateacourse/course/courselist-0-0-0-'.$subfolder['folderid'].'-'.$uid)?>" title="<?=$subfolder['foldername']?>"><img src="<?= empty($subfolder['img']) ? 'http://static.ebanhui.com/ebh/images/nopic.jpg' : $subfolder['img'] ?>" width="114" height="159" border="0" /></a></div>
	<span class="spne"><a href="<?= geturl('aroomv2/ateacourse/course/courselist-0-0-0-'.$subfolder['folderid'].'-'.$uid) ?>" style="text-decoration: none;" title="<?= $subfolder['foldername'] ?>"><?= ssubstrch($subfolder['foldername'],0,20) ?>(<?= $subfolder['coursewarenum'] ?>)</a></span>
	</li>
        <?php } ?>
