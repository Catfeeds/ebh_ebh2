...
<div class="ter_tit">
		当前位置 > <a href="<?=geturl('aroomv2/teacher')?>">教师管理</a>
		<div class="diles">
	<?php
		$q= empty($q)?'':$q;
		if(!empty($q)){
...
<div class="tiezitool">
<a class="hongbtn jiabgbtn marrig" onclick="window.location.href='<?=geturl('aroomv2/teacher/add')?>'" >添加教师</a>
<a class="hongbtn jiabgbtn" onclick="window.location.href='<?=geturl('aroomv2/teacher/input')?>'" >批量导入</a>
</div>
</div>
...
<tbody>
	<?php if(!empty($roomteacherlist)){
		...
		<td width="140px">
		<a class="workBtn" href="<?=geturl('aroomv2/teacher/edit/'.$value['uid'])?>">编辑</a>
	...
	else
	{
		...
		$.ajax({
			...
			// alert(data)
				...
				{
					$.showmessage({message:'密码修改失败！请使用6位以上密码'});
				}
			}
		});
	}
...
$(function(){
	$('#searchbutton').click(function(){
		var href = '<?=geturl('aroomv2/teacher')?>';
