...
class MychapterModel extends CModel{
	...
	*判断给定章节名称是否存在
	*/
	public function chapter_exists($chaptername,$crid=0,$pid=0,$chapterid=0) {
		if(empty($chapterid)) {
			$sql = 'select * from ebh_schchapters c where c.crid='.intval($crid).' and c.pid='.intval($pid)." and c.chaptername='".$this->db->escape_str($chaptername)."'";
		} else {
			$sql = 'select * from ebh_schchapters c where c.crid='.intval($crid).' and c.pid='.intval($pid).' and c.chapterid !='.intval($chapterid)." and c.chaptername='".$this->db->escape_str($chaptername)."'";
		}
	...
	*根据chapterid删除章节
	*/
	public function delete_byid($chapterid,$crid = 0) {
	...
	*/
	...
	//根据章节id批量将其排序加1
	function incorder($chapteridArr){
	...
	private function _updatechildren($pid = 0,$level = 0,$chapterpath = ''){
		...
		foreach ($children as $child) {
			$this->db->update('ebh_schchapters', array('level'=>$level+1,'chapterpath'=>$chapterpath.'/'.$child['chapterid']), array('chapterid'=>$child['chapterid']));
			$this->_updatechildren($child['chapterid'],$level+1,$chapterpath.'/'.$child['chapterid']);
		}
	}
	...
	public function getnodelist($crid, $versionid = 0) {
		...
		if(!empty($versionid))
		{
			...
			$version = $this->db->query($version_sql)->row_array();
	...
	*/
	public function moveit($param){
		...
		$thischapter = $this->db->query($sql)->row_array();
		...
		if (!empty($sameorder))
		{
			if($param['compare'] == '<')
				...
				$op = '+';
			$sqlAllforone = 'update ebh_schchapters set displayorder=displayorder'.$op.'1 where crid='.$param['crid'].' and displayorder'.$param['compare'].'='.$thischapter['displayorder'].' and level=1 and chapterid<>'.$thischapter['chapterid'];
			$this->db->query($sqlAllforone);
		}

		$sql2 = 'select chapterid,displayorder from ebh_schchapters';
		$wherearr[] = 'crid='.$param['crid'];
		$wherearr[] = 'displayorder'.$param['compare'].$thischapter['displayorder'];
		$wherearr[] = 'level=1';
		$sql2 .= ' where '.implode(' AND ',$wherearr);
		$sql2 .= ' order by '.$param['order'];
		$sql2 .= ' limit 1';
		$deschapter = $this->db->query($sql2)->row_array();
		if(empty($deschapter))
			return false;
		$this->db->update('ebh_schchapters',array('displayorder'=>$deschapter['displayorder']),array('chapterid'=>$thischapter['chapterid']));
		$this->db->update('ebh_schchapters',array('displayorder'=>$thischapter['displayorder']),array('chapterid'=>$deschapter['chapterid']));
		return TRUE;
	}
