...
class BindModel extends CModel{
	...
	public function doUnbind($type,$uid){
		...
		if($type=='qq'){//qq解绑
			...
			//删除openid unionid
			if($ret){
				$this->db->update("ebh_users",array('wxunionid'=>'','wxopenid'=>'','wxopid'=>''),array('uid'=>$uid));
			}
	 ...
	 * 检查微信绑定
	 */
	public function checkwxbind($openid){
	    $sql = 'select count(*) as count from ebh_binds b  left join ebh_users u on b.uid = u.uid where wxopid =\''.$openid.'\' OR  wxopenid =\''. $openid.'\' OR wxunionid =\''. $openid.'\' limit 1';
	    $row = $this->db->query($sql)->row_array();
	    return $row['count']>0 ? true : false;
	}
	...
	public function getwxuserbyopenid($openid){
        $sql = 'select u.uid, u.username,u.realname,u.sex,u.email,u.face, u.groupid, u.credit,u.logincount,u.dateline,u.lastlogintime,u.password,u.balance,u.lastloginip,u.status,u.allowip,u.schoolname,u.mysign,u.address,u.mobile 
                from ebh_binds b left join ebh_users u on b.uid = u.uid
                where u.wxopid =\''.$openid.'\' OR  u.wxopenid =\''. $openid.'\' OR u.wxunionid =\''. $openid.'\' limit 1';
        //log_message($sql);
