 ...
 * Time: 10:32
 */
class ChangeclassModel extends CModel{
    /**
     * 返回学生当前网校下所在班级信息
     * @param $uid
     * @param $crid
    ...
    public function myClassmates($classid, $uid, $pageindex = 1, $pagesize = 20) {
        ...
        $classmates = $this->db->query($sql)->list_array();
        if (empty($classmates)) {
            return false;
        }
        //获取关注信息
    ...
    public function follow($uid, $fuid) {
        ...
        $snsdb->begin_trans();
        //先验证 是否关注
        $ckfllow = $snsdb->query('select count(*) count from ebh_sns_follows where uid ='.$uid.' and fuid='.$fuid.' limit 1')->row_array();
        if(empty($ckfllow['count'])){
    ...
    public function changeClassBySelf($params, &$errcode) {
        ...
        //记录升班日志
        if (empty($changelogid)) {
            $newid = $this->db->insert('ebh_classlog', array(
                'pid' => $change['pid'],
                'crid' => $crid,
                'uid' => $uid,
                'sourceid' => $sourceid,
        ...
        if (empty($changelogid)) {
            ...
            //迁移健康数据
            $ret = $this->db->update('ebh_constitution', array(
                'cid' => $classid
            ), array(
                'uid' => $uid,
                'cid' => $sourceid
            ...
            //迁移健康评论数据
            $ret = $this->db->update('ebh_health_comment', array(
                'classid' => $classid
            ), array(
                'studentid' => $uid,
                'classid' => $sourceid
            ...
            $ret = $this->db->update('ebh_classlog', array(
                'classid' => $classid
            ), "`lid`=$newid");
            if ($ret === false) {
                $this->db->rollback_trans();
    ...
    public function getChangeInfo($uid, $crid, $classid) {
        ...
        $classids = $this->db->query($sql)->row_array();
        if (empty($classids)) {
    ...
    public function changeSingleStudent($crid, $sourceid, $classid, $user, $student_id) {
        ...
        $message = sprintf('根据学校教务管理安排，你所处的班级已由原先的“%s”更换至“%s”，你可以点击“更多模块”查看“我的班级”相关信息。',
            $this->db->escape_str($classes[$sourceid]['classname']),
            $this->db->escape_str($classes[$classid]['classname']));
        ...
        $this->db->begin_trans();
        //记录升班事件日志
        $newid = $this->db->insert('ebh_changeclass', $insert_params);
        if ($newid == 0) {
            return false;
        }

        //记录学生升班日志
        $new_log_id = $this->db->insert('ebh_classlog', array(
            'pid' => $newid,
            'crid' => $crid,
            'uid' => $student_id,
            'sourceid' => $sourceid,
        ...
        //发送学生升班消息
        $new_message_id = $this->db->insert('ebh_messages', array(
            'fromid' => $user['uid'],
            'toid' => $student_id,
            'sourceid' => $newid,
            'crid' => $crid,
        ...
        $redis->hIncrBy('msg_'.$student_id.'_'.$crid, 3);
        return $newid;
    }
    ...
    private function _changeByAdmin($crid, $sourceid, $classid, $user, &$errcode) {
        ...
        if ($student_id_arr = $this->db->query($sql)->list_field('uid')) {
    ...
    private function _changeByStudent($crid, $sourceid, $classids, $starttime, $endtime, $user, &$errcode) {
        ...
        $source_class_name = $this->db->escape_str($classes[$sourceid]['classname']);
        unset($classes[$sourceid]);
        $class_options = array();
        foreach ($classes as $classitem) {
            $class_options[] = '“' . $this->db->escape_str($classitem['classname']) . '”';
        }
        ...
        $student_id_arr = $this->db->query("SELECT `uid` FROM `ebh_classstudents` WHERE `classid`=$sourceid")->list_field('uid');
        if (!empty($student_id_arr)) {
            //发送学生自主升班消息
            foreach ($student_id_arr as $studentid) {
                if (!$this->db->insert('ebh_messages', array(
                    'fromid' => $uid,
                    'toid' => $studentid,
                    'sourceid' => $newid,
                    'crid' => $crid,
        ...
        if (!empty($student_id_arr)) {
            ...
            foreach ($student_id_arr as $stuent_id) {
                $redis->hIncrBy('msg_'.$stuent_id.'_'.$crid, 3);
            }
        }
        return $newid;
    }
    ...
    private function _modifyChangePlan($pid, $crid, $sourceid, $classids, $starttime, $endtime, $user, $is_reduce, &$errcode) {
        ...
        $this->db->begin_trans();
        //修改升班事件日志
        $ret = $this->db->update('ebh_changeclass', $up_params, "`pid`=$pid");
        if ($ret === false) {
            $errcode = 10002;
            return false;
        }
        ...
        if ($is_reduce) {
            ...
            $logs = $this->db->query($sql)->list_array();
            if (!empty($logs)) {
                foreach ($logs as $log) {
                    //迁移健康数据
                    $ret = $this->db->update('ebh_constitution', array(
                    ...
                    $ret = $this->db->update('ebh_classstudents', array(
                        'classid' => $sourceid
                    ), array(
                        'uid' => $log['uid'],
                        'classid' => $log['classid']
            ...
            foreach ($restore as $rk => $rv) {
                if ($rk > 0 && $rv > 0) {
                    if (!$this->db->simple_query("UPDATE `ebh_classes` SET `stunum`=`stunum`-$rv WHERE `classid`=$rk")) {
                        $this->db->rollback_trans();
                        $errcode = 10005;
                        return false;
                    }
                } elseif ($rk == 0) {
                    if (!$this->db->simple_query("UPDATE `ebh_classes` SET `stunum`=`stunum`+$rv WHERE `classid`=$sourceid")) {
                        $this->db->rollback_trans();
                        $errcode = 10006;
                        return false;
                    }
            ...
            if (!empty($log_id_arr)) {
                ...
                unset($log_id_arr);
                $ret = $this->db->simple_query("UPDATE `ebh_classlog` SET `status`=1 WHERE `lid` IN($log_id_arr_str)");
                if ($ret === false) {
                    $this->db->rollback_trans();
                    $errcode = 10007;
                    return false;
