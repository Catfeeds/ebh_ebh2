...
class UserModel extends CModel {
    ...
    public function login($username,$userpass,$iscoding = FALSE) {
        ...
        $sql = "select uid,username,groupid,logincount,lastlogintime,lastloginip,password,status,allowip from ebh_users where username=$username or email=$username or mobile=$username";
    ...
    public function getloginbyuid($uid,$userpass,$iscoding = FALSE) {
        $pwd = $iscoding ? $userpass : md5($userpass);
        $sql = "select u.freezebalance,u.uid, u.username,u.realname,u.sex,u.email,u.face, u.groupid, u.credit,u.logincount,u.dateline,u.lastlogintime,u.password,u.balance,u.lastloginip,u.status,u.allowip,u.schoolname,u.mysign,u.address,u.mobile from ebh_users u where u.uid=$uid";
        $user = $this->db->query($sql)->row_array();
        if(empty($user) || $user['password'] != $pwd || $user['status'] == 0) {
            return false;
        }
	...
	*/
	public function existsEmail($email){
        ...
        public function getuserbyuid($uid) {
            $sql = 'select u.freezebalance,u.uid, u.username,u.realname,u.sex,u.email,u.mobile,u.face, u.groupid, u.credit,u.logincount,u.password,u.balance,u.lastloginip,u.lastlogintime,u.status,u.allowip,u.schoolname,u.mysign  from ebh_users u where u.uid = '.$uid;
            return $this->db->query($sql)->row_array();
        }
        /**
         * 修改用户信息
