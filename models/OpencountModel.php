...
class OpencountModel extends CModel{
	...
	*/
	function update($param,$wherearr=array()) {
		...
		return $this->db->update('ebh_tempstudents',$setarr,$wherearr);
	...
	*/
	function delete($wherearr = array()){
		if(empty($wherearr))
			return FALSE;
		return $this->db->delete('ebh_tempstudents',$wherearr);
	}
	/**
	*根据编号获取订单详情
	*/
	...
	*/
	public function getopencountlist($param){
		...
		$wherearr[]= 't.status =1';
		if(!empty($param['q']))
			$wherearr[]= '  (t.username like \'%'. $this->db->escape_str($param['q']) .'%\' or c.crname like \'%' . $this->db->escape_str($param['q']) .'%\')';
		if(!empty($param['crid'])){
			if(empty($param['upid']))
