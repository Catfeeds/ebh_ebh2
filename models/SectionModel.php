...
class SectionModel extends CModel{
    ...
    public function getallsections($param = array()) {
        $sql = 'select s.sid,s.folderid,s.sname from ebh_sections s ';
        $wherearr = array();
        if(!empty($param['folderid']))
            $wherearr[] = 's.folderid='.$param['folderid'];
        if(!empty($param['crid']))
        ...
        if(!empty($wherearr))
            ...
            return FALSE;
        $sql .=' order by s.displayorder asc';
        $sectionlist = $this->db->query($sql)->list_array();
        $sectionarray = array();
        if (!empty($sectionlist))
        {
        	foreach ($sectionlist as $key => $value) {
        		$sectionarray[$value['folderid']][] = $value;
        	}
        }
        return $sectionarray;
    }
