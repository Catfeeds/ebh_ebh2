...
class ExamModel extends CModel {
	...
	* @param int $crid 教室编号
	*/
	public function getexamcount($crid,$uid=0){
        $count = 0;
        if(empty($crid)){
            return $count;
        }
	...
	*/
	public function getschexamlist($param) {
		if(empty($param['crid']))
			return FALSE;
		$sql = 'SELECT e.eid,e.title,e.uid,e.grade,e.limitedtime,e.score,e.dateline,e.status,e.answercount,e.quescount,e.cwid,e.classid,e.district,e.type,c.classname,c.stunum,cw.title as ti,u.username,u.realname FROM ebh_schexams e left join ebh_classes c on e.classid=c.classid left join ebh_coursewares cw on e.cwid = cw.cwid LEFT JOIN ebh_users u on e.uid = u.uid';
		$wherearr = array();
		$wherearr[] = 'e.crid='.$param['crid'];
		$isgrade = FALSE;
		if(!empty($param['classid'])) {
			if(!empty($param['grade']) && !empty($param['uid'])) {	//按照年级和科目获取作业
				...
				if(empty($fid_in)) {
//                    if($param['all']==1){
//                        $wherearr[] = '((e.classid in('.$param['classid'].') and e.grade = 0) or (e.grade in('.$param['grade'].') and e.district='.$param['district'].'))';
//                        $isgrade = true;
//                    }else{
...
//                    }
					...
					$isgrade = TRUE;
					$wherearr[] = '((e.classid in('.$param['classid'].') and e.grade = 0) or (e.folderid in'.$fid_in.' and e.grade in('.$param['grade'].') and e.district='.$param['district'].'))';
				}
			} else {
//                if($param['all']==1){
...
//                            if(!empty($param['classid'])){
//                                $wherearr[] = '(((e.classid in('.$param['classid'].') or e.grade in ('.$param['grade'].')) and e.folderid in ('.$param['folderid'].')) or e.uid ='.$param['myuid'].')';
//                            }else{
//                                $wherearr[] = '(( e.grade in ('.$param['grade'].') and e.folderid in ('.$param['folderid'].')) or e.uid ='.$param['myuid'].')';
//                            }
//                        }else{
//                            if(!empty($param['classid'])){
//                                $wherearr[] = '(e.classid in('.$param['classid'].') or e.grade in ('.$param['grade'].') or e.uid ='.$param['myuid'].')';
//                            }else{
//                                $wherearr[] = '(e.grade in ('.$param['grade'].') or e.uid ='.$param['myuid'].')';
//                            }
//                        }
//                    }else{
//                        if(!empty($param['folderid'])){
//                            if(!empty($param['classid'])){
//                                $wherearr[] = '((e.classid in('.$param['classid'].') and e.folderid in ('.$param['folderid'].')) or e.uid = '.$param['myuid'].')';
//                            }else{
//                                $wherearr[] = '(e.folderid in ('.$param['folderid'].') or e.uid = '.$param['myuid'].')';
//                            }
//                        }else{
//                            if(!empty($param['classid'])){
//                                $wherearr[] = '(e.classid in('.$param['classid'].') or e.uid = '.$param['myuid'].')';
//                            }else{
//                                $wherearr[] = 'e.uid = '.$param['myuid'];
//                            }
//                        }
//                    }
//                }else{
...
//                }
		...
		if(isset($param['type']) && (is_numeric($param['type']) || is_array($param['type'])) ){
		...
		if(!empty($eidlist) && !empty($param['classid'])) {	//年级作业的答题数需要调整
			$usql = 'select uid from ebh_classstudents where classid in('.$param['classid'].')';
	...
	public function getschexamlistcount($param) {
		...
		if(!empty($param['classid'])) {
			if(!empty($param['grade']) && !empty($param['uid'])) {	//按照年级和科目获取作业
				...
				if(empty($fid_in)) {
//                    if($param['all']==1){
//                        $wherearr[] = '(e.classid in('.$param['classid'].') or (e.grade in('.$param['grade'].') and e.district='.$param['district'].'))';
//                        $isgrade = TRUE;
//                    }else{
                        $wherearr[] = 'e.classid in('.$param['classid'].')';
//                    }
					...
					$isgrade = TRUE;
					$wherearr[] = '(e.classid in('.$param['classid'].') or (e.folderid in'.$fid_in.' and e.grade in('.$param['grade'].') and e.district='.$param['district'].'))';
				}
			} else {
//                if($param['all']==1){
//                    if(!empty($param['grade'])){
...
//                }
	...
	*/
	public function getFolderByGrade($crid,$grade,$uid,$district) {
		$sql = "select f.folderid,f.foldername from ebh_teacherfolders tf join ebh_folders f ".
				...
				"where f.crid=$crid and tf.tid=$uid and f.grade in ( ".$grade." ) and f.district=$district";
	...
	*/
	public function getschexamanswerlistbyeid($param) {
		if(empty($param['eid']) || empty($param['classid']))	//作业编号和班级编号不能为空
			return FALSE;
		$sql = 'SELECT ea.aid,ea.eid,ea.dateline,ea.completetime,ea.totalscore,ea.remark,ea.tid,ea.status,ea.uid,u.username,u.realname,u.sex,u.face FROM ebh_classstudents ct '.
		...
		$wherearr = array();
        if(isset($param['eastatus'])) {
            if ($param['eastatus'] == 1) {
                $wherearr[] = 'ea.status = ' . $param['eastatus'];
            } elseif ($param['eastatus'] == 2) {
                $wherearr[] = '(ea.status is null or ea.status = 0)';
            }
        }
	...
	*/
	public function getschexamanswerlistbygrade($param) {
		...
		$wherearr[] = 'ct.classid in('.$param['classid'].')';
		$wherearr[] = 'e.status = 1';
        if(!empty($param['eastatus'])){
            $wherearr[] = 'ea.status = '.$param['eastatus'];
        }
		$sql .= ' WHERE '.implode(' AND ',$wherearr);
	...
	*/
     ...
     *更加学生账号获取该学生的做作业信息数
     */
    public function getschexamscount($param){
        if(empty($param['crid']) && empty($param['classid']))
            return FALSE;
        $sql = 'SELECT count(*) as count FROM ebh_schexams e left join ebh_schexamanswers s on (e.eid=s.eid)';
        $wherearr = array();
        if(!empty($param['crid']))
            $wherearr[] = 'e.crid='.$param['crid'];
        if(!empty($param['classid'])) {
        ...
        if( is_numeric($param['type']) || is_array($param['type']) ){
            if(is_numeric($param['type'])){
                $wherearr[] = 'e.type='.$param['type'];
            }else if(!empty($param['type'])){
                $wherearr[] = 'e.type in ('.implode(',',$param['type']).')';
        ...
        if(!empty($param['eid']))
            $wherearr[] = 'e.eid='.$param['eid'];
        if(!empty($param['uid']))	//过滤某个教师布置的班级作业
            $wherearr[] = 's.uid='.$param['uid'];
        if(!empty($param['q']))
            $wherearr[] = 'e.title like \'%'.$this->db->escape_str($param['q']).'%\'';
        if(!empty($wherearr))
...
//        }
	...
	*/
	public function getschexambyeid($eid) {
		$sql = "SELECT e.eid,e.title,e.limitedtime,e.grade,e.district,e.classid,e.score,e.dateline,e.status,e.answercount,e.quescount,e.type,e.uid FROM ebh_schexams e WHERE e.eid=$eid";
		return $this->db->query($sql)->row_array();
	}
	/**
	*根据eid删除网校课件下的作业
	...
	*/
	public function getExamListByMemberid($param) {      
		if(empty($param['uid']))
			return FALSE;
		$sql = 'SELECT e.eid,e.crid,e.title,e.dateline,e.score,e.answercount,e.limitedtime,e.folderid,u.uid,u.username,u.face,u.sex,u.realname,u.groupid,a.aid,a.status astatus,a.dateline adateline,a.completetime,a.totalscore,f.foldername,c.title ctitle,e.cwid from ebh_schexams e '.
	...
	*/
	...
	public function getRoomExamCount($classid,$eid,$type=0){
		$sql = "select  count(*) count from ebh_schexamanswers a left join  ebh_classstudents as t on a.uid = t.uid left join ebh_schexams e on e.eid = a.eid where t.classid in( $classid ) and e.eid = $eid ";
		if($type==1){
			$sql.=' and a.tid !=0';
		}elseif($type==2){
			$sql.=' and a.tid=0';
	...
	*/
	public function getschexamlistbycwid($param) {
		...
		if(!empty($param['stuid'])){
			$sql = 'SELECT  e.eid,e.title,e.cwid,e.classid,e.grade,e.district,e.dateline,e.score,e.uid,e.answercount,  (select status from ebh_schexamanswers sa where sa.eid = e.eid AND sa.uid='.$param['stuid'].') as status,e.folderid   FROM ebh_schexams e ';
		}else{
	...
	获取多个老师的作业数，试题数
	*/
	public function getTeachersExamCount($param){
		$sql = 'select se.uid,count(*) examnum,sum(se.quescount) as examquesnum from ebh_schexams se';
		if(!empty($param['crid']))
			$wherearr[] = 'se.crid='.$param['crid'];
		if(!empty($param['uids']))
			$wherearr[] = 'se.uid in ('.$param['uids'].')';
		if(!empty($param['startdate']))
	...
	老师的作业数
	*/
	 public function getTexamcount($param) {
        $count = 0;
		$sql = 'SELECT COUNT(*) as count FROM ebh_schexams ';
		$wherearr = array();
		if (!empty($param['crid']))
            $wherearr[] = 'crid=' . $param['crid'];
        if (!empty($param['uid']))
     ...
     * 获取试卷已回答列表(认证考试专用)
     */
    public function getschexamansweredlist($param){
		if(empty($param['eid']) || empty($param['crid']))
			return FALSE;
		$sql = 'SELECT ea.aid,ea.eid,ea.dateline,ea.completetime,ea.totalscore,ea.remark,ea.tid,ea.status,ea.uid,u.username,ap.realname,u.sex,ap.photo FROM ebh_schexamanswers ea '.
		' JOIN ebh_examapplys ap ON ap.uid = ea.uid'.
		' LEFT JOIN ebh_schexams e ON e.eid = ea.eid'.
		' LEFT JOIN ebh_users u ON u.uid = ea.uid'.
		' LEFT JOIN ebh_examawards aw ON aw.aid = ea.aid';
		$wherearr = array();
		$wherearr[] = 'ea.eid='.$param['eid'];
		$wherearr[] = 'ea.status=1';
		$wherearr[] = 'ap.status=1';
		$wherearr[] = 'e.crid='.$param['crid'];
		if(isset($param['ispass'])){
			if ($param['ispass'] == 1){
				$wherearr[] = 'ea.totalscore>=90';
			}
			elseif ($param['ispass'] == 0){
				$wherearr[] = 'ea.totalscore<90';
			}
		}
		if(isset($param['isaward'])){
			if ($param['isaward'] == 1){
				$wherearr[] = 'aw.isaward=1';
			}
			elseif ($param['isaward'] == 0){
				$wherearr[] = '(aw.isaward is null or aw.isaward=0)';
			}
		}
		if(!empty($param['q']))
			$wherearr[] = '(u.username like \'%'.$this->db->escape_str($param['q']).'%\''.
                    ' or ap.realname like \'%'.$this->db->escape_str($param['q']).'%\')';
		$sql .= ' WHERE '.implode(' AND ',$wherearr);
     ...
     * 获取试卷已回答计数(认证考试专用)
     */
    public function getschexamansweredcount($param){
		if(empty($param['eid']) || empty($param['crid']))
			return FALSE;
        $count = 0;
		$sql = 'SELECT COUNT(*) count FROM ebh_schexamanswers ea'.
		' JOIN ebh_examapplys ap ON ap.uid = ea.uid'.
		' LEFT JOIN ebh_users u ON u.uid = ea.uid'.
		' LEFT JOIN ebh_examawards aw ON aw.aid = ea.aid';
		$wherearr = array();
     ...
     * 获取试卷导出列表(认证考试专用)
     */
    public function getschexamexcellist($param){
		if(empty($param['eid']) || empty($param['crid']))
			return FALSE;
		$sql = 'SELECT ap.realname,ap.sex,ap.namespell,ap.major,ap.stuid,ap.idcard,ap.mobile,ap.email,ea.totalscore FROM ebh_schexamanswers ea '.
		' JOIN ebh_examapplys ap ON ap.uid = ea.uid'.
		' LEFT JOIN ebh_schexams e ON e.eid = ea.eid'.
		' LEFT JOIN ebh_examawards aw ON aw.aid = ea.aid';
		$wherearr = array();
		...
		if(!empty($param['startdate']))
			$wherearr[] = 'ea.dateline>=' . $param['startdate'];
		if(!empty($param['enddate']))
			$wherearr[] = 'ea.dateline<=' . $param['enddate'];
	...
	*根据学生编号获取学校学生所在班级下的作业
	*/
	public function getExamcountByMemberid($param) {      
		if(empty($param['uid']))
			return FALSE;
		$sql = 'SELECT e.eid,e.crid,e.title,e.dateline,e.score,e.answercount,e.limitedtime,e.folderid,u.uid,u.username,u.face,u.sex,u.realname,u.groupid,a.aid,a.status astatus,a.dateline adateline,a.completetime,a.totalscore,f.foldername,c.title ctitle,e.cwid from ebh_schexams e '.
				'LEFT JOIN ebh_schexamanswers a on (e.eid = a.eid AND a.uid='.$param['uid'].') '.
     ...
     * 获取已答学生列表
     */
    public function getHomeworkAnswer($param){
        if(empty($param['eid'])){
            return false;
        }
        $sql = 'select sa.aid,sa.eid,sa.dateline,sa.completetime,sa.totalscore,sa.remark,sa.tid,sa.status,sa.uid
                from ebh_schexamanswers sa where sa.status=1 and sa.eid='.$param['eid'];
        if(!empty($param['order'])){
            $sql .= ' order by '.$param['order'];
        }
        $answers = $this->db->query($sql)->list_array();
        foreach($answers as $key=>$answer){
            //验证班级
            $sql1 = 'select classid from ebh_classstudents where uid ='.$answer['uid'];
            if(!empty($param['classid'])){
                $sql1 .= ' and classid='.$param['classid'];
            }
            $classid = $this->db->query($sql1)->row_array();
            if(empty($classid)){
                unset($answers[$key]);
                continue;
            }
            ...
            //验证学校和年级
            $sql2 = 'select classid from ebh_classes where status=0';
            if(!empty($param['classid'])){
                $sql2 .= ' and classid='.$param['classid'];
            }
            if(!empty($param['grade'])){
                $sql2 .= ' and grade='.$param['grade'];
            }
            if(!empty($param['crid'])){
                $sql2 .= ' and crid ='.$param['crid'];
            }
            $classinfo = $this->db->query($sql2)->row_array();
            ...
            //学生信息
            $userinfo = $this->db->query('select face,sex,username,realname from ebh_users where uid ='.$answer['uid'])->row_array();
            $answers[$key]['sex']=$userinfo['sex'];
            $answers[$key]['face']=$userinfo['face'];
            $answers[$key]['username']=$userinfo['username'];
            $answers[$key]['realname']=$userinfo['realname'];
        }
        return $answers;
