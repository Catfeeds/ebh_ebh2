...
class PitemsModel extends CPortalModel{
	...
	public function datafetch($catid=0,$bth = array(),$num = 5,$hasimg = false,$order = ''){
		$sql = 'select i.itemid,i.catid,i.subject,i.itemurl,i.thumb,i.note from ebh_pitems i';
		$wherearr = array();
		if(!empty($catid)){
			$wherearr[] = 'i.catid ='.$catid;
		}
		if(!empty($bth)){
			if(isset($bth['b'])){
				$wherearr[] = 'i.best='.intval($bth['b']);
			}
			if(isset($bth['t'])){
				$wherearr[] = 'i.top='.intval($bth['t']);
			}
			if(isset($bth['h'])){
				$wherearr[] = 'i.hot='.intval($bth['h']);
			}
		}
		...
		if(!empty($order)){
			$sql .= ' order by '.$this->portaldb->escape_str($order);
		}else{
			$sql .= ' order by lastpost desc,itemid desc';
		}
	...
	public function addviewnum($itemid, $num = 1) {
        ...
        $this->portaldb->update('ebh_pitems', array(), $where, $setarr);
    }
	
	public function setviewnum($itemid, $num = 1) {
		$where = 'itemid=' . $itemid;
