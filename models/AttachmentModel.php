...
class AttachmentModel extends CModel {
	...
	*/
    ...
    public function getAttachmentListByCwid($queryarr = array()) {
		...
		if(isset($queryarr['filterstatus']))
			$wherearr[] = 'a.status != '.$queryarr['filterstatus'];
	...
	*/
	public function addMultipleAtt($param){
		$attnum = count($param);
		$sql = 'insert into ebh_attachments (uid,crid,cwid,title,message,source,url,filename,suffix,size,status,dateline,sourceid,checksum,ispreview,apppreview) values ';
		foreach($param as $att){
			$uid = $att['uid'];
			$crid = $att['crid'];
			$cwid = $att['cwid'];
			$title = $this->db->escape_str($att['title']);
			$message = $this->db->escape_str(empty($att['message'])?'':$att['message']);
			$source = $att['source'];
			$url = $att['url'];
            ...
            $ispreview = $att['ispreview'];
            $apppreview = $att['apppreview'];
			$sql.= "($uid,$crid,$cwid,'$title','$message','$source','$url','$filename','$suffix',$size,$status,$dateline,$sourceid,'$checksum',$ispreview,$apppreview),";
		}
		$sql = rtrim($sql,',');
        // log_message($sql);
		$this->db->query($sql);
  ...
  public function deletebycwid($cwid){
	...
	*/
	public function getAttachByFolderid($param){
		...
		$wherearr[] = 'a.status != -1';
		$wherearr[] = '(cw.status != -2 and cw.status != -3)';
		$sql .= ' where '.implode(' AND ',$wherearr);
     ...
     * @return mixed
     */
	public function getAttachByChecksum($checksum) {
	    $sql = 'SELECT `attid`,`source`,`url`,`filename` FROM `ebh_attachments` WHERE `checksum`='.$this->db->escape($checksum);
	    return $this->db->query($sql)->row_array();
    }
    ...
    public function getIntro($attid) {
	    $sql = 'SELECT `url`,`source`,`suffix` FROM `ebh_attachments` WHERE `attid`='.intval($attid).' AND `status`=1';
	    return $this->db->query($sql)->row_array();
    }
