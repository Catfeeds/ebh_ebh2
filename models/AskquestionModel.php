...
class AskquestionModel extends CModel {
	...
	*/
    ...
    public function getallasklists($param) {
        ...
        $pagesize = empty($param['pagesize']) ? 10 : $param['pagesize'];
        $start = ($page - 1) * $pagesize;
        $sql = 'select q.qid,q.crid,q.uid,q.folderid,q.title,q.answercount,q.viewnum,q.hasbest,q.dateline,q.catpath,q.status,u.uid,u.sex,u.face,u.username,u.realname,f.foldername,q.status,q.message,q.shield,u.groupid,q.reward,q.imagesrc,q.audiotime,q.coverimg,q.audiosrc from ebh_askquestions q join ebh_users u on (q.uid = u.uid) left join ebh_folders f on (f.folderid = q.folderid)';
        $wherearr = array();
        if (!empty($param['crid']))
            $wherearr[] = 'q.crid=' . $param['crid'];
        if (!empty($param['uid']))
    ...
    public function getmyasklist($param) {
		...
		if (!empty($param['cridsis']))
			$sql .= ' AND q.crid = '.$param['cridsis'];
        if (!empty($param['order'])) {
            $sql .= ' order by ' . $param['order'];
        } else {
            $sql .= ' order by q.qid desc ';
    ...
    public function getallaskcount($param) {
        ...
        if(isset($param['answercount'])){
            $wherearr[] = $param['answercount'];
        }
        if (!empty($param['q']))
    ...
    public function getmyaskcount($param) {
		...
		if (!empty($param['aq']))
            $wherearr[] = '(u.username =\'' . $param['aq'] .'\' or u.realname =\'' . $param['aq']. '\')';
        if(!empty($param['Y'])){
            $wherearr[] = 'FROM_UNIXTIME(q.dateline,\'%Y\') = '.$param['Y'];
        }
        if(!empty($param['m'])){
            $wherearr[] = 'FROM_UNIXTIME(q.dateline,\'%c\') = '.$param['m'];
        }
        if(!empty($param['day'])){
            $wherearr[] = 'FROM_UNIXTIME(q.dateline,\'%e\') = '.$param['day'];
        }
		if (!empty($wherearr))
            $sql.= ' WHERE (' . implode(' AND ', $wherearr);
		if(!empty($param['qids']))
...
//        $sql .= ' ORDER BY a.isbest desc,a.aid desc';
    ...
    public function getasklistbyanswers($param) {
        ...
        $pagesize = empty($param['pagesize']) ? 10 : $param['pagesize'];
        $start = ($page - 1) * $pagesize;
        $sql = 'SELECT q.qid,q.title,q.hasbest,q.dateline,q.catpath,q.status,a.dateline as adateline,a.isbest,q.answercount,q.viewnum,f.foldername,u.uid,u.sex,u.face,u.username,u.realname,q.shield,u.groupid,q.reward,q.imagesrc,q.coverimg,q.audiosrc,q.audioname,q.audioname,q.message,q.audiotime FROM ebh_askquestions q ' .
...
//            $wherearr[] = 'a.uid=' . $param['uid'];
    ...
    public function getaskcountbyanswers($param) {
        $count = 0;
        if(!empty($param['group'])){
            $sql = 'select count(DISTINCT '.$param['group'].') count from ebh_askquestions q ';
        }else{
            $sql = 'SELECT count(*) count FROM ebh_askquestions q ' ;

        }
        $sql .= 'LEFT JOIN ebh_askanswers a ON (q.qid = a.qid) ' .
                'LEFT JOIN ebh_users u ON (u.uid = q.uid) ';
        $wherearr = array();
        if (!empty($param['crid']))
            $wherearr[] = 'q.crid=' . $param['crid'];
        ...
        if(!empty($param['Y'])){
            $wherearr[] = 'FROM_UNIXTIME(a.dateline,\'%Y\') = '.$param['Y'];
        }
        if(!empty($param['m'])){
            $wherearr[] = 'FROM_UNIXTIME(a.dateline,\'%c\') = '.$param['m'];
        }
        if(!empty($param['day'])){
            $wherearr[] = 'FROM_UNIXTIME(a.dateline,\'%e\') = '.$param['day'];
        }
        if (!empty($wherearr))
            $sql.= ' WHERE ' . implode(' AND ', $wherearr);
    //    $sql .= ' AND a.shield =0 AND q.shield =0 ';
    ...
    public function getasklistbyfavorit($param) {
        ...
        $pagesize = empty($param['pagesize']) ? 10 : $param['pagesize'];
        $start = ($page - 1) * $pagesize;
        $sql = 'SELECT q.uid,q.qid,q.title,q.hasbest,q.dateline,q.catpath,q.answercount,q.viewnum,q.status,f.foldername,a.aid,u.uid,u.sex,u.face,u.username,u.realname,q.shield,u.groupid,q.reward,q.imagesrc,q.coverimg,q.audioname,q.audiosrc,q.message,q.audiotime FROM ebh_askquestions q ' .
...
//    }
	...
	*/
	public function setBestT($param){
		...
		$count = $this->db->query($sql)->row_array();
		if($count['count']>0){
			return FALSE;//只能设一次最佳答案了2017.08.09
			$aarr['isbest'] = 0;
    ...
    */
    public function getcoursequestionslist($folderids = array(),$param = array()){
        ...
        $sql = 'select a.qid,a.dateline,a.title,a.answercount,a.viewnum,u.uid,u.username,u.realname,u.face,u.sex,f.foldername,a.shield,a.status,u.groupid,a.reward,a.audiotime,a.coverimg,a.imagesrc,a.audiosrc,a.cwid,a.hasbest,a.message from ebh_askquestions a  left join ebh_users u on a.uid = u.uid left join ebh_folders f on f.folderid = a.folderid where a.folderid in ('.$in.')';
        if(!empty($param['q'])){
    ...
    */
    ...
    public function getClassesAsk($classids,$param=array(),$folderids){
        ...
        $sql = 'select aq.qid,aq.dateline,aq.title,aq.answercount,aq.viewnum,aq.status,aq.message,aq.hasbest,aq.coverimg,aq.audiotime,u.sex,u.face,u.username,u.realname,u.uid,f.foldername,u.groupid,aq.reward from ebh_askquestions aq
	...
	*/
     ...
     * 获取网校学生提问人数明细
     */
    public function getQcountGroupByUid($param){
        $sql = 'select q.uid,count(*) as count,u.realname,u.username,u.face,u.sex from ebh_askquestions q LEFT JOIN ebh_users u on q.uid=u.uid';
        $where = ' where 1=1';
        if(!empty($param['crid'])){
            $where.=' and q.crid ='.$param['crid'];
        }
        $sql.=$where;
        $sql.=' group by q.uid order by count(*) desc';
//        p($sql);die;
        return $this->db->query($sql)->list_array();
    }
     ...
     * 获取网校学生回答问题人数明细
     */
    public function getAcountGroupByUid($param){
        $sql = 'select a.uid,count(*) as count,u.realname,u.username,u.face,u.sex from ebh_askanswers a LEFT JOIN ebh_askquestions q on a.qid=q.qid LEFT join ebh_users u on a.uid=u.uid';
        $where = ' where 1=1';
        if(!empty($param['crid'])){
            $where.=' and q.crid ='.$param['crid'];
        }
        $sql.=$where;
        $sql.=' group by a.uid order by count(*) desc';
        return $this->db->query($sql)->list_array();
    }
     ...
     * 获取网校回答人数
     */
    public function getAnswersCount($param,$species=true){
        if(empty($param['crid'])){
            return false;
        }
        if($species){
            $sql = 'select count(DISTINCT a.uid) count from  ebh_askanswers a LEFT JOIN ebh_askquestions q ON (q.qid = a.qid)';
        }else{
            $sql = 'select count(*) count from  ebh_askanswers a LEFT JOIN ebh_askquestions q ON (q.qid = a.qid)';
        }
        $where = ' where 1=1';
        if(!empty($param['crid'])){
            $where .=' and q.crid ='.$param['crid'];
        ...
        if(!empty($param['abegindate'])) {	//回答时间从
            $where .= ' and a.dateline>='.$param['abegindate'];
        }
        if(!empty($param['aenddate'])) {	//回答时间从
            $where .= ' and a.dateline<'.$param['aenddate'];
        }
        $sql.=$where;
        $count =  $this->db->query($sql)->row_array();
        return $count['count'];
	...
	根据aid获取回答者uid
	*/
	public function getAnswererByAid($aid,$joinQue=0){
        if (!$aid) {
            return false;
        }
        if ($joinQue) {
            $sql = 'select a.uid,q.qid,q.title from ebh_askanswers a left join ebh_askquestions q using(qid) where a.aid = '.$aid;
        } else {
            $sql = 'select uid,qid,message from ebh_askanswers where aid = '.$aid;
        }
		return $this->db->query($sql)->row_array();
	}
    /**
     * 给原有问答中的音频添加音频时间
     */
    public function updateAudio($param,$qid){
        if(empty($param) || empty($qid)){
            return false;
        }
        $sql = 'UPDATE ebh_askquestions SET audioname='.$this->db->escape($param['audioname']).',audiosrc='.$this->db->escape($param['audiosrc']).',audiotime='.$this->db->escape($param['audiotime']).' WHERE qid ='.intval($qid);
        $this->db->simple_query($sql);
    }
    /**
     * 给原有回答中的音频添加时间
     */
    public function updateAnswerAudioTime($param,$aid){
        if(empty($param) || empty($aid)){
            return false;
        }
        $sql = 'UPDATE ebh_askanswers SET audioname='.$this->db->escape($param['audioname']).',audiosrc='.$this->db->escape($param['audiosrc']).',audiotime='.$this->db->escape($param['audiotime']).' WHERE aid ='.intval($aid);
        $this->db->simple_query($sql);
    }
    /**
    ...
    public function updateQueStatusByQid($qid){
        ...
        return $this->db->update('ebh_askquestions', $setarr, $wherearr);
    }
    /**
     * 根据aid获取回答的详情
     */
     ...
     * 根据uid获取回答和解答的列表
     */
    public function getAnswerAndQuestionByUid($uid,$crid,$page = 1){
        if(empty($uid) || empty($page) || empty($crid)){
            return false;
        }
        $page = intval($page);
        $limit = (($page - 1) * 4).' ,4';
        $sql = 'SELECT c.qid,c.dateline FROM (SELECT a.qid,a.dateline FROM ebh_askanswers a JOIN ebh_askquestions aq on (aq.qid = a.qid) WHERE a.uid = '.intval($uid).' and a.shield  = 0 and aq.crid ='.intval($crid).' UNION ALL SELECT qid,dateline FROM ebh_askquestions WHERE uid = '.intval($uid).' and shield = 0 and crid = '.intval($crid).') AS c ORDER BY c.dateline DESC LIMIT '.$limit;
        ...
        if(!empty($lists)){
            ...
            if(!empty($qids)){
                $qsql = 'select qid,title from `ebh_askquestions` where qid in('.implode(',',$qids).')';
                $qlists = $this->db->query($qsql)->list_array();
                if(!empty($qlists)){
                    foreach ($lists as &$lt) {
                        foreach ($qlists as $qlist) {
                            if($qlist['qid'] == $lt['qid']){
                                $lt['title'] = $qlist['title'];
                            }
                        }
                    }
                    return $lists;
     ...
     * 根据uid获取回答和解答的列表数量
     */
    public function getAnswerAndQuestionCount($uid,$crid){
        if(empty($uid)){
            return false;
        }
        $sql = 'SELECT count(*) as count FROM (SELECT a.qid,a.dateline FROM ebh_askanswers a JOIN ebh_askquestions aq on (aq.qid = a.qid) WHERE a.uid = '.intval($uid).' and a.shield = 0 and aq.crid ='.intval($crid).' UNION ALL SELECT qid,dateline FROM ebh_askquestions WHERE uid = '.intval($uid).' and shield = 0 and crid ='.intval($crid).') AS c ';
        $count = $this->db->query($sql)->row_array();
        return $count['count'];
    }
}
