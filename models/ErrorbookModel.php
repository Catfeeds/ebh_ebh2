...
class ErrorbookModel extends CModel {
	...
	*学校学生我的错题本列表
	*/
	public function myscherrorbooklist($param = array(),$distinct = false) {
		if(empty($param['uid']))
			return FALSE;
		...
		if($distinct){
			$sql = 'SELECT distinct e.exid,e.eid,ex.title as etitle,q.ques,e.qid,e.dateline,q.quetype,q.falsenum,q.score,e.uid,e.erranswers,q.title,e.uid from ebh_schquestions q '.
					'join ebh_errorbook e on (q.qid=e.quesid) '.
					'join ebh_schexams ex on (ex.eid in (q.eid)) '.
					'join ebh_classstudents c on (e.uid=c.uid) '.
					'join ebh_classes cs on (cs.classid=c.classid) ';
	...
	*学校学生我的错题本列表记录总数
	*/
	public function myscherrorbooklistcount($param = array(),$distinct = false) {
		$count = 0;
		if(empty($param['uid']))
			return $count;
		$sql = 'SELECT count(*) count from ebh_schquestions q '.
				...
				'join ebh_classes cs on (cs.classid=c.classid) ';
		if($distinct){
			$sql = 'SELECT count(distinct(ex.eid)) count from ebh_schquestions q '.
					'join ebh_errorbook e on (q.qid=e.quesid) '.
					'join ebh_schexams ex on (ex.eid in (q.eid)) '.
					'join ebh_classstudents c on (e.uid=c.uid) '.
					'join ebh_classes cs on (cs.classid=c.classid) ';
	...
	*/
	public function getSchoolErrorBookList($param){
		...
		$sql.= ' where '.implode(' and ',$wherearr);
		$sql.= ' order by ex.dateline desc,q.falsenum desc ';
		if(!empty($param['limit'])) {
            $sql .= ' limit '.$param['limit'];
        } else {
			if (empty($param['page']) || $param['page'] < 1)
	...
	*/
	public function getSchErrorBookListByClassid($param){
		if(empty($param['classid']))
			return FALSE;
		$sql = 'SELECT er.exid,er.eid,er.qid,er.quesid,e.title etitle,er.dateline,er.erranswers,e.score,q.ques,q.quetype,q.falsenum,u.username,u.realname,q.title title,e.crid from ebh_errorbook er '.
	...
	*/
	...
	public function getTotalErrors($param = array()){
		$sql = 'select e.exid as examid,e.uid,e.qid,e.erranswers,e.type,u.username,u.realname,e.dateline,e.quesid,sa.aid,sa.tid,sa.status from ebh_errorbook e join ebh_users u on e.uid = u.uid left join ebh_schexamanswers sa on e.exid = sa.eid and e.uid = sa.uid';
		$whereArr = array();
	...
	/**
	 * [getSchexamsByEid 根据eid获取作业题目]
	 * @param  [type] $eid [description]
	 * @return [type]      [description]
	 */
    public function getSchexamsByEid($eid){
        $sql = 'select title from ebh_schexams';
        if(!empty($eid)){
            $sql.=' where eid ='.$eid;
        }
	 ...
	 * 根据exid获取作业的信息
	 */
	public function getexambyexid($exid){
		if(empty($exid)){
			return false;
		}
		$sql = 'select title,eid from ebh_schexams where eid in ('.$exid.')';
		return $this->db->query($sql)->list_array();
	}
}
?>