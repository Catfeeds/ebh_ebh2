...
class ClassroomModel extends CModel {
    ...
    function getroomlistbytid($tid) {
        $sql = 'select c.crid,c.crname,c.domain,c.upid,c.cface,c.summary,c.coursenum,c.teanum,c.stunum,c.fulldomain,c.uid from ebh_roomteachers rt ' .
    ...
    function getroomlistbyuid($uid) {
        $sql = 'select c.crid,c.crname,c.domain,c.fulldomain from ebh_roomusers rc ' .
...
/*
	...
	*/
	public function getsimpleclassroomlist(){
		$sql = 'select c.crid,c.domain,c.crname from ebh_classrooms c where c.isschool=7';
		return $this->db->query($sql)->list_array();
	}
	/**
	 * 获取所有网校列表
	...
	*/
	public function editclassroom($param){
		...
		if(isset($param['isdesign'])){
		    $setarr['isdesign'] = $param['isdesign'];
		}
...
//log_message($sql);
    ...
    */
     ...
     * @return mixed
     */
    public function getAdmins($crids) {
        $wheres  = array();
        if (is_array($crids)) {
            $crids = array_map('intval', $crids);
            $wheres[] = '`a`.`crid` IN('.implode(',', $crids).')';
        } else {
            $wheres[] = '`a`.`crid`='.intval($crids);
        }
        $sql = 'SELECT `a`.`crid`,`b`.`uid`,`b`.`username`,`b`.`realname`,`b`.`sex`,`b`.`face` FROM 
                `ebh_classrooms` `a` LEFT JOIN `ebh_users` `b` ON `b`.`uid`=`a`.`uid`
                WHERE '.implode(' AND ', $wheres);
        return $this->db->query($sql)->list_array('crid');
    }
