...
class PanModel extends CModel{
    ...
    */
    public function getClassroomPaninfo($crid,$isFromAdmin=0){
    	...
    	$info['totalpansize'] = 1024*1024*1024*1000;//默认1000G
    	//$info['noinfo'] = 1;
    	$info['defaultpansize'] = 1024*1024*1024;
    	$res = $this->getCridPanUsesize($crid);
		$info['usepansize'] = empty($res['sum']) ? 0 : $res['sum'];//byte
        return $info;
    }
    ...
    */
    public function setClassroomPaninfo($param){
        ...
        if (!$info) {//老师不存在，第一次赋值
            ...
            $setarr['totalpansize'] = 1000*1048576*1024;//G到byte
            if ($setarr['totalpansize'] < $setarr['usepansize']) {
                return 0;
            }
            $setarr['crid'] = $crid;
            $setarr['defaultpansize'] = $param['defaultpansize']*1024*1024*1024;
            $size = $this->getMaxCridPanUsesize($crid);//所有用户最大的使用量
            if ($setarr['defaultpansize'] < $size['maxsize']) {
                return -1;
            }
            if ($this->pandb->insert('pan_giveinfos', $setarr)) {
                return 1;
            } else {
                return 0;
            }
    ...
    */
    ...
    public function doPanGive($param,$seq=0) {
    	...
    	if (!$seq) {//第一次插入
    		...
    		$res = $this->getCridPanUsesize($crid);
    		$setarr['usepansize'] = empty($res['sum']) ? 0 : $res['sum'];//byte
    		//接下来算总的givepansize,默认都是1G,网校总人数*1G
    		$setarr['totalpansize'] = $param['totalpansize']*1048576*1024;//G到byte
    		if ($setarr['totalpansize'] < $setarr['usepansize']) {
    			return -1;
    		}
    		$setarr['crid'] = $crid;
    		...
    		$totalpansize = $param['totalpansize']*1048576*1024;//G到byte
