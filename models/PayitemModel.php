...
//		$param['folders'] = $this->db->escape($param['folders']);
	...
	*/
    ...
    /**
     * 网校平台下的所有课程服务项，包括：本校服务项与企业选课服务项
     * @param $crid
     * @return array
     */
    public function getPlatItems($crid) {
        $wheres = array(
            '`a`.`crid`='.$crid,
            '`a`.`status`=0',
            '`b`.`del`=0',
        ...
        $sql = 'SELECT `a`.`itemid`,`a`.`pid`,`a`.`sid`,`a`.`iname`,`a`.`iprice`,`a`.`imonth`,`a`.`iday` FROM `ebh_pay_items` `a` JOIN `ebh_folders` `b` ON `b`.`folderid`=`a`.`folderid` 
        ...
        $sql = 'SELECT `b`.`itemid`,`b`.`pid`,`b`.`sid`,`b`.`iname`,`a`.`price`,`month` FROM `ebh_schsourceitems` `a` JOIN `ebh_pay_items` `b` ON `b`.`itemid`=`a`.`itemid` 
                JOIN `ebh_folders` `c` ON `c`.`folderid`=`b`.`folderid` 
                LEFT JOIN `ebh_pay_sorts` `d` ON `d`.`sid`=`b`.`sid` 
                WHERE '.implode(' AND ', $wheres);
        $oRet = $this->db->query($sql)->list_array('itemid');
        if (!empty($oRet)) {
            $ret = $ret + $oRet;
            unset($oRet);
        }
