...
class PayorderModel extends CModel{
	...
	*/
	public function getOrderDetailListByOrderId($orderid) {
		$sql = "select d.isshare,d.sharefee,d.shareuid,d.detailid,d.orderid,d.itemid,d.fee,d.crid,d.folderid,d.oname,d.osummary,d.omonth,d.oday,d.rname,d.providercrid,d.comfee,d.roomfee,d.providerfee,d.cwid from ebh_pay_orderdetails d where d.orderid=$orderid";
		return $this->db->query($sql)->list_array();
	}
	
	public function getOrderList($param){
		$sql = 'select o.orderid,o.crid,o.ordername,o.payfrom,o.paytime,o.refunded,o.sourceid,o.dateline,o.totalfee,o.ordernumber,o.remark,cr.crname,u.username,u.realname,o.totalfee,o.status from ebh_pay_orders o left join ebh_classrooms cr on o.crid=cr.crid left join ebh_users u on u.uid=o.uid ';
		$wherearr = array();
	...
	*/
	public function getOrderDetailList($param) {
		if(empty($param['crid']) && empty($param['pid']) && empty($param['orderid']) && empty($param['uid']) && empty($param['shareuid'])) {	//至少需要有个参数
			return FALSE;
		}
		$sql = 'select pd.sharefee,pd.detailid,pd.crid,pd.uid,pd.fee,pd.rname,pd.oname,pd.osummary,pd.omonth,pd.oday,o.payfrom,o.paytime,o.dateline,u.username,u.realname,u.sex,u.face from ebh_pay_orderdetails pd '.
		...
		if(!empty($param['shareuid']))
			$wherearr[] = 'pd.shareuid='.$param['shareuid'];
		if(!empty($param['isshare']))
			$wherearr[] = 'pd.isshare='.$param['isshare'];
		if(!empty($param['pid']))
	...
	*/
	...
	//判断用户是否购买过课程
	public function checkIfHasBuyedCourese($uid){
		if(empty($uid)){
			return false;
		}
		$sql = "select count(*) as count from ebh_pay_orders";
		$where = " where uid = ".intval($uid).' and refunded = 0 and status = 1 and invalid = 0';
		$sql .= $where;
		$res = $this->db->query($sql)->row_array();
