...
class UpcontrolLib {
    ...
    function getcontrolhtml($name = "up", $type = 0, $valueparam = array(), $uptype = 'courseware', $flashparam = array() , $multifile = false) {
        ...
        if ($type == 0) {
            ...
            $filetyps = '*.ppt;*.excel;*.xls;*.wps;*.pdf;*.doc;*.docx;*.mp3;*.swf;*.avi;*.mpg;*.flv;*.zip;*.rar;*.7z;*.pptx;*.xlsx;*.rmvb;*.mp4;*.wav';
            $filedes = '文件格式';
        } elseif ($type == 3) {
            $filetyps = '*.ppt;*.excel;*.xls;*.wps;*.pdf;*.doc;*.docx;*.ebhp;*.avi;*.mpg;*.mp3;*.flv;*.zip;*.rar;*.7z;*.pptx;*.xlsx;*.rmvb;*.mp4;*.wav;*.mov';
            $filedes = '文件格式';
        } elseif ($type == 6) {
...
//	}
            ...
            if (!empty($filepathvalue)) {
                ...
                if ($type == 0) {
                    ...
                    $constr .= shortstr(empty($filenamevalue) ? $filepathvalue : $filenamevalue,30);
                    $constr .= '</span>';
					if($type != 3 && $type != 13){
        ...
        if (isset($valueparam['size'])) {
            $constr .= '<div><span id="' . $name . '_spanuploadbutton"></span></div>';
        } else {
            $constr .= '<div><span id="' . $name . '_spanuploadbutton"></span></div>';
        }
		if(empty($multifile)){
    ...
    function getpostpath($type = 0) {
        ...
        if ($type == 0 || $type == 3 || $type == 10 || $type == 11) {
            ...
            $scount = count($_UP['favicon']['server']);
            $spos = rand(0, $scount - 1);
            $uploadurl = $_UP['favicon']['server'][$spos];
        } elseif ($type == 13) {
            $scount = count($_UP['health']['server']);
            $spos = rand(0, $scount - 1);
            $uploadurl = $_UP['health']['server'][$spos];
        } else {
