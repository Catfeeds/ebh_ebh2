<?php
class Mail{
	public function sendemail($toarr, $subject, $message='', $from='') {
		...
		$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: ebh.net ".$mail['ver']."{$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=UTF-8{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
		...
		if($mail['mailsend'] == 1) {
			...
			fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." ebh.net\r\n");
			$lastmessage = fgets($fp, 512);
