...
class SMS{
    ...
    */
    ...
    public function send_dayu($mobile,$code,$sign = "e板会"){
        ...
        }elseif(!empty($result) && !empty($result->code) && ($result->code > 0)){
            //发送异常,账号超出限制
            log_message('result:'.var_export($result,true));
        }
    }
     ...
     * 提现申请短信发送
     */
    public function send_applycash($mobile,$info,$sign = "e板会"){
        if(empty($mobile)){
            return false;
        }
        $result = $this->_sendByMT4($mobile,$info,$sign);
        if( !empty($result) && !empty($result->result) && ($result->result->err_code !== '0') ){
            //发送失败写日志
            log_message('result:'.var_export($result,true));
        }elseif(!empty($result) && !empty($result->code) && ($result->code > 0)){
    ...
    private function _sendByMT4($mobile,$info,$sign){
        ...
        $req->setSmsType("normal");
        $req->setSmsFreeSignName($sign);
        $req->setSmsParam("{\"name\":\"".$info['name']."\",\"time\":\"".$info['time']."\",\"money\":\"".$info['money']."\"}");
        $req->setRecNum($mobile);
        $req->setSmsTemplateCode("SMS_33820164");
        return $c->execute($req);
    }
	...
	申请结算相关短信
	*/
	private $jsTemplateCodeArr = array(
			0=>'SMS_105920090',//身份审核短信：您好，${name} 网校申请身份验证，请尽快审核
			1=>'SMS_105850075',//您好，${name} 网校申请结算 ${price} 元，请尽快到审核平台进行审核。
			2=>'SMS_105870090',//您好，您的网校 ${name} 结算申请付款成功，结算金额将于1-7个工作日内到账，请注意查收。
			3=>'SMS_105775079');//您好，您的网校 ${name} 结算付款失败，请确认信息正确，结算申请的金额已返回网校余额，请至网校进行查询。
	public function jsSend($mobile,$paramarr,$index,$sign = 'e板会'){
		include_once "aliyun_dayu/TopSdk.php";
    	date_default_timezone_set('Asia/Shanghai');
    	$c = new TopClient;
    	$c->format = 'json';
    	...
    	$req->setSmsType("normal");
    	$req->setSmsFreeSignName($sign);
    	$req->setSmsParam(json_encode($paramarr));
    	$req->setRecNum($mobile);
    	$req->setSmsTemplateCode($this->jsTemplateCodeArr[$index]);
    	return $c->execute($req);
	}
}