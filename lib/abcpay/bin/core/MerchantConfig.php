...
class MerchantConfig
{
	...
	private static function bundle()
    {
        if (!self::$iIsInitialed)
        {
            ...
            {
            	//配置文件路径
                $tIniFile = substr(dirname(__FILE__), 0,-9).DIRECTORY_SEPARATOR.'TrustMerchant.ini';
            }
            ...
            $tTrustPayCertFile= substr(dirname(__FILE__), 0,-9).DIRECTORY_SEPARATOR. $tTrustPayCertFile;
            //生产环境用方法1读，测试环境用方法2读，
           // 方法1
            ...
            {
            	//日志路径
                self::$iLogPath = substr(dirname(__FILE__), 0,-9).DIRECTORY_SEPARATOR.self::getParameterByName('LogPath');
            }
    ...
    private static function bindMerchantCertificateByFile()
    {
		...
		//print_r($tMerchantCertFileArray);exit();
        ...
        for($i = 0; $i < self::$iMerchantNum; $i++)
        {
            //1、读取证书
            $tCertificate = array();
            if(openssl_pkcs12_read(file_get_contents(substr(dirname(__FILE__), 0,-9).DIRECTORY_SEPARATOR.$tMerchantCertFileArray[$i]), $tCertificate, $tMerchantCertPasswordArray[$i]))
            {
                //2、验证证书是否在有效期内
