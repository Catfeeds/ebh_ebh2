...
class Verify {
     ...
     */
    public function check($code, $key = 'verify') {
    	$verify = EBH::app()->getInput()->cookie($key);
    	$secode = authcode($verify,'DECODE');
        // 验证码不能为空
        if(empty($code) || empty($secode) || empty($verify)) {
            return false;
        }
        list($verify_code,$verify_time) = @explode("\t", $secode);
        // cookie 过期
        if(SYSTIME - $verify_time > $this->expire) {
            return false;
        }

        if(strtoupper($code) == $verify_code) {
         //   $this->reset && EBH::app()->getInput()->setcookie($key,null,-3600);
            return true;
        }
    ...
    /**
     * 输出验证码并把验证码的值保存在cookie中
     * 验证码保存到cookie的格式为： 'verify_code' => '验证码值', 'verify_time' => '验证码创建时间';
     * @access public     
     * @param string $key 要生成验证码的标识   
     * @return void
     */
    public function entry($key = 'verify') {
        // 图片宽(px)
        ...
        $verify_time = SYSTIME;  // 验证码创建时间
        $secode = $verify_code."\t".$verify_time;
        EBH::app()->getInput()->setcookie($key,authcode($secode,'ENCODE'),$this->expire);
