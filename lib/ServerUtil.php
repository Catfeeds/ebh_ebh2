...
class ServerUtil {
	...
	*/
	...
	/**
	* 获取直播服务器地址
	* 会根据onlines.config中的服务器地址进行处理，如果多个，则会随机取值
	* 取值策略可以根据此处调整
	*/
	public function getOnlineSource() {
		$serverlist = Ebh::app()->getConfig()->load('onlines');
        $source = 'chat.ebh.net';
		if(empty($serverlist))
			return $source;
		$scount = count($serverlist);
	...
	* 取值策略可以根据此处调整
	*/
	public function getZKM3u8CourseSource() {
		$serverlist = Ebh::app()->getConfig()->load('zkm3u8servers');
        $source = '';
		if(empty($serverlist))
			return $source;
		$scount = count($serverlist);
